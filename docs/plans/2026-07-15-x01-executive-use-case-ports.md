...
- [x] Define `ContextAssembler::assemble(TurnRequest, canonical_history)` returning one bounded `AssembledContext` with system prefix, recall, core memory, facts, skills, Dasein view, and Agora view in deterministic order.
- [x] Move injection reads/formatting from daemon-turn and `TurnPipeline` behind existing `MemoryService`, `TurnServices`, and a skill-catalog port; enforce the current item and byte bounds before returning.
- [x] Define legacy clear/list/resume/compact/create/switch operations behind a `LegacySessionUseCases` trait implemented over `SessionService`, `SessionStore`, and the session registry.
- [x] Migrate session RPC/routing code to those operations and prove resume/fork/replay/interrupt still use canonical Session/Turn/Item state.
- [x] Run context/session focused tests plus `session_lifecycle_commands`, `session_append_store`, and `turn_service_equivalence`; expected PASS.

**Task 5 evidence:** the request/history boundary, deterministic fragment order (`recall -> core memory -> facts -> skills -> Dasein -> Agora`), UTF-8-safe per-fragment/aggregate caps, and history bounding are implemented at `crates/executive/src/service/context_assembler.rs:13-227`. `TurnPipeline` replays model history from the shared canonical `SessionService` at `crates/executive/src/service/turn_pipeline.rs:422-445`. The complete compatibility contract and its registry/legacy-store/canonical adapters are at `crates/executive/src/service/legacy_session_service.rs:42-354`; legacy resume imports journal history once through the append-only canonical projection at `crates/executive/src/service/session_service.rs:61-105`, while clear and compact rotate to new canonical sessions instead of rewriting history. Session RPCs contain only port calls and lifecycle adaptation at `crates/executive/src/impl/daemon/handler/rpc/rpc_session.rs:16-280`; workspace routing delegates to the same port at `crates/executive/src/impl/daemon/handler/mod.rs:405-414`, and the concrete routing module was deleted. `context_assembler` (3), `session_use_case_port` (4), `session_lifecycle_commands` (1), `session_append_store` (3), and `turn_service_equivalence` (8) all pass; all-target Executive Clippy and architecture fitness pass at 98 findings with the resolved session-store exception removed.

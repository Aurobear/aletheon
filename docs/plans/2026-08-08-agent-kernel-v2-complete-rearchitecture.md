...
- Kernel 本轮固定为独立小型 crate；这里只继续验证其 public surface 应收缩到何种程度，物理合并不再是本次迁移变量；
...
| Gmail cursor/dedupe/message/goal proposal | Gmail extension | Gmail store | 只能提交 `GoalDraftProposal`/External Stimulus，不能创建 canonical GoalDraft |
| Canonical GoalDraft/activation decision | Application | Application GoalDraft store | Gmail/Robot/用户入口只能提交 proposal；采用后通过 Runtime command 执行 |
...
├── kernel/                 # 本轮固定的独立小型 execution enforcement boundary
...
2. `CapabilityBroker`；
3. `ResourceLedger`；
4. `TimeSource/Timer` ports 与 OS child supervision。

“Kernel”在 V2 中固定为小型、独立的 `kernel` crate。它不只是 Runtime 的内部实现：Corpus/Gmail outbound/Hardware 等 effect adapter 在 composition 时实现其 ports，Application/Owner resolution 由外层 verifier adapter 接入，Pi 也依赖其 process lease，因此本轮不得再把它并入 `runtime::execution`。未来若真实 resolved graph 证明只剩一个消费者，必须在 V2 稳定后另写 ADR；该 ADR 不属于本次迁移，也不得削弱 owner、ports、journal、禁止依赖和 authority 约束。不能为了名字保留空层，也不能借合并让 Runtime 绕过该边界。
...
duplicate AgentLoader     -> census 已证明 agent-definition 版本无生产 caller并删除；保留的生产版本迁出时精确命名 MarkdownAgentProfileLoader
...
- 删除 `TuiSessionManager`、旧 `application::agent::AgentRuntime` 及本纵向切片内已证明无调用者的 Session/Agent 重复 authority；Pi 仍依赖的 `SubAgentRuntime`/AgentControl delegate seam 保留到 Milestone 5 cutover；
...
- E7 先删 extension-specific Fabric rows/re-export，D6 再删 non-extension rows并唯一收口 shared root；达到允许的 primitives 后，由唯一 `AK2-25` PR 做机械 rename并一次清零 `fabric::` alias/re-export；
- `AK2-25` 通过后才由 `XRET-04/XRET-05` 删除已经为空的 Executive crate/facade 和 compatibility path；
...
AK2-11a Runtime generic AgentSupervisor + sealed DelegateBackendRegistry（RA-05；不迁 Pi）
AK2-11b Pi delegate/wait/cancel/recovery production cutover（唯一联合 PR E6-K6d）
AK2-11c post-smoke legacy SubAgent/registry/facade cleanup（RA-06）
...
`AK2-*` 仅是上位 roadmap umbrella，不是第二套实施 PR 编号，也不能拿来绕开专项 owner。AK2 与 `RA/K/D/APX/CGP/E/XRET` 的唯一规范映射、直接前置、writer cutover 和删除门集中维护在 [`Executive 完全拆解：专项计划索引与重复权威总账`](./2026-08-08-executive-decomposition-plan-series.md)；其他文档只能引用该表，不得复制一份容易漂移的 crosswalk。尤其 `AK2-11a -> AK2-11b -> AK2-11c` 分别由 `RA-05 -> E6-K6d -> RA-06` 交付，generic Runtime registry、Pi adapter 切流和 legacy 清退不得合成一个 Mega PR。

规则：

- 一个 PR 只迁移一个 owner 或一条纵向路径；
- mechanical rename、type ownership 和 behavior change 分开 commit；E7 与 D6 按 extension rows -> non-extension/shared root 的顺序把 surface 清到 rename-ready，随后由唯一 `AK2-25` 纯机械 PR 修改 crate/package/path/manifest/lock/docs；
- 每个 owner 迁移 PR 同时清理该 owner 范围内的重复词汇、blanket lint allow、无类型错误和 oversized constructor；不得在 `AK2-25` 混入业务迁移或语义重命名；
...
- 下列 “Kernel” gate 适用于本轮固定的独立小型 `kernel` crate；审计只决定 public surface、space/execd 等机制去留，物理合并只能在 V2 稳定后另开 ADR；
...
- [ ] Kernel 是本轮固定的独立小型 enforcement crate，public surface 已按调用/隔离审计收缩；任何未来物理合并仅由 V2 稳定后 ADR 决定；
...
本文件保持不可违反的总体边界；以下专项计划把最新 `dev` 的重复实现、Executive 逐项迁出、PR 切片、兼容 seam、能力保全和删除门细化为可独立审阅的 Draft plans PR：

- [`Executive 完全拆解索引与重复权威总账`](./2026-08-08-executive-decomposition-plan-series.md)；
- [`Runtime 权威收敛`](./2026-08-08-runtime-authority-consolidation.md)；
- [`Kernel Enforcement 收敛`](./2026-08-08-kernel-enforcement-consolidation.md)；
- [`领域权威与 Adapter 迁出`](./2026-08-08-domain-authority-and-adapter-extraction.md)；
- [`Application 与持久化/主机 Adapter 提取`](./2026-08-08-application-persistence-extraction.md)；
...
- [`DeepSeek 文件级实施计划`](./2026-08-09-agent-kernel-v2-deepseek-implementation-plan.md)。

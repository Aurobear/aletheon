...
| XF-011 | X12 | Session authority remained split: `CanonicalSessionStore` still implemented the public mutation port and startup recovery mutated its SQLite record directly, while Task/principal recovery facts were not fully journal-backed | Fabric Session contract、Executive Session authority/recovery/materializer、schema registry、status ledger | exactly one production `SessionAppendStore` implementation；principal/Task/recovery facts commit through EventSpine；schema-v5 replay and U-RESUME-001/002/004/006 pass |
| XF-012 | X12 | Agent settlement control evidence reused the public Session `TURN_EVENT_V1` schema even though the payload was not an `ItemRecord` | Fabric schema IDs、AgentControl settlement sink/tests、status ledger | dedicated Agent settlement schema；stale generation rejection remains durable；test proves settlement schema differs from public Turn schema |
| XF-013 | X12 | U-TUI-002/U-TUI-006 and A-DELETE-002 were bound to weak proxies: no cancellation wire assertion, incomplete `NO_COLOR`/line-mode journey, and README Stable rows lacked separately verified recovery anchors | Interact terminal/line-mode/key tests、README capability matrix、Metacog deletion contract、acceptance registry、status ledger | incremental command output plus real cancel request；no-color keyboard/line-mode core journey；every Stable row has existing code/E2E/recovery anchors checked mechanically |
| XF-014 | X12 | receipt classifier treated the aggregate `tool_errors` diagnostic counter as terminal failure even after the authoritative terminal, independent acceptance, workspace policy, resource cleanup and correlated evidence all passed | `tests/coding/harness/receipt.py`、runner/replay tests、status ledger | recovered tool errors remain separately counted；typed provider/runtime failures and failed host gates remain fail-closed；static harness suite；installed 20-task rerun |

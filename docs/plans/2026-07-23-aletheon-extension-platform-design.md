...
**日期：** 2026-07-23
**状态：** 完成；R0–R6 与 §21 已通过源码、全量测试和实际部署验收
**受众：** 实现者、审查者、发布验收人员
...
下表是最终权威状态。`已实现/已接线/已验证` 必须三项同时成立，
不得再用类型存在或 CLI 可解析代替完成。

| 阶段 | 已实现 | 已接线 | 已验证 | 备注 |
|---|---|---|---|---|
| Phase 0 | ✅ | — | ✅ | 契约、负向基线及完整测试输出已保存 |
| Phase 1 | ✅ | ✅ | ✅ | Package/Asset/Runtime/Capability 四层类型和只读旧类型投影进入生产编译图 |
| Phase 2 | ✅ | ✅ | ✅ | 安全 Inspector、事务 Store、应用服务、CLI 和 receipt 已贯通 |
| Phase 3 | ✅ | ✅ | ✅ | quarantine、previous-known-good、health/TUI 及五种 daemon 场景均已验证 |
| Phase 4 | ✅ | ✅ | ✅ | Provider 全生命周期通过稳定 Runtime Router 接入 daemon |
| Phase 5 | ✅ | ✅ | ✅ | 隔离子进程、协议边界、取消、超时、断路和清理均有真实辅助进程测试 |
| Phase 6 | ✅ | ✅ | ✅ | 完整 CLI、审批、工作区信任、迁移遥测、Metacog 事件和回滚已验证 |
| Phase 7 | ✅ | ✅ | ✅ | 安装二进制、真实 TUI、Sub-Agent Runtime、故障矩阵和清理收据通过 |

- ✅ 完成，❌ 未完成，⚠ 部分完成未接线
- "已接线" 指存在生产调用路径且不是仅单元测试调用
- "已验证" 指对应阶段门禁全部通过
...
```text
candidate commit:
96d426b0303cef702d18e6d5953586e36b932fd0

candidate / installed / running SHA-256:
a54751397f8124e7d9c32d01c7e82c9c9230c743c0d6c87e92a71ec1e9638e96

receipt directory:
.scenario-runs/extension-platform-96d426b-vVbtDS/
```

候选提交之后的 `362026dd4eeede2aa014d9b3a883a3ecad3ab7cd`
只收紧发布收据校验：所有 JSON/JSONL 收据现在必须引用同一个候选
commit；混合提交证据会被拒绝。生产验证入口位于
`scripts/libexec/aletheon/extension-acceptance.sh:83-130`，其正负向测试位于
`tests/production/extension_acceptance_static_test.sh:52-75`。

收据目录中的 `raw/` 保存构建、安装、workspace 测试、真实 TUI
typescript、daemon health、Profile quarantine、Runtime 数据库结果、
生命周期收据和 Metacog 原始事件。公开收据不包含凭据值。

### 23.2 R0–R6 关闭证据

| 修复阶段 | 实现与接线证据 | 验证证据 | 结论 |
|---|---|---|---|
| R0 | CLI 通过应用服务执行真实生命周期；状态矩阵见 §3.5 | `crates/aletheon/tests/extension_cli_gate.rs`；`raw/workspace-tests.log` | 完成 |
| R1 | Tar 类型 allowlist、重复路径、checksum、Asset 路径和受控 staging：`crates/corpus/src/extension/inspector.rs:35-205` | 十类归档位于 `tests/fixtures/extensions/archives/`；聚焦测试 `required_malicious_archive_fixtures_are_rejected_without_staging_output` | 完成 |
| R2 | 原子 Store 和 receipt：`crates/corpus/src/extension/store.rs`；统一应用服务：`crates/executive/src/application/extension_service.rs` | Store 单元测试、CLI gate、实际 `install/list/show` 生命周期收据 | 完成 |
| R3 | 候选 Profile、持久 quarantine、known-good 恢复和最终选择：`crates/executive/src/host/daemon/bootstrap/agents.rs:53-158` | 五种 daemon bootstrap 场景：同文件 `:325-424`；实际 quarantine/修复收据 `profile-quarantine.json` | 完成 |
| R4 | 稳定 Router：`crates/executive/src/application/extension_runtime_router.rs:19-115`；隔离 Provider：`crates/executive/src/extensions/runtime/subprocess.rs:107-155`、`:425-675` | 真实辅助进程覆盖协议错误、超长输出、秘密脱敏、挂起、取消、崩溃、越权和清理；实际 `runtime.generic` 任务成功 | 完成 |
| R5 | 生命周期事务：`crates/executive/src/application/extension_manage.rs`；daemon 自动回滚：`crates/executive/src/host/daemon/bootstrap/extensions.rs:69-126`；迁移遥测和通用事件：`crates/corpus/src/extension/store.rs` | 权限升级拒绝保留旧状态、工作区 hash-bound trust、v1.2 故障回滚到 v1.1、v1.3 恢复健康 | 完成 |
| R6 | 13 类收据和 20 条标准覆盖由 `scripts/libexec/aletheon/extension-acceptance.sh:18-130` 强制 | `bash scripts/aletheon.sh acceptance extension .scenario-runs/extension-platform-96d426b-vVbtDS` 通过 | 完成 |

### 23.3 验证命令与完整输出

所有 Rust 命令均通过 `scripts/cargo-agent.sh`，没有直接调用 Cargo。

```bash
bash scripts/cargo-agent.sh fmt --all -- --check
bash scripts/cargo-agent.sh test -p corpus extension:: --lib
bash scripts/cargo-agent.sh test -p executive application::extension_ --lib
bash scripts/cargo-agent.sh test -p executive \
  host::daemon::bootstrap::extensions::tests --lib
bash scripts/cargo-agent.sh test -p executive daemon_bootstrap_ --lib
bash scripts/cargo-agent.sh test -p aletheon --test extension_cli_gate
bash scripts/cargo-agent.sh test -p fabric --test extension_contract
bash scripts/cargo-agent.sh test --workspace -- --test-threads=4
bash tests/production/extension_acceptance_static_test.sh
```

workspace 命令完成 254 个 test result 分组，未出现 `FAILED` 或
`error: test failed`。完整 5,038 行输出保存在
`raw/workspace-tests.log`。并行全量运行曾暴露一个资源竞争型
`script_tool_execute_success` 抖动；单测通过，最终以四线程全量运行稳定
通过。串行全量运行曾使 `cross_domain_acceptance` 的固定 TTL 过期；
该 target 独立运行 6/6 通过，因此最终全量门禁采用有界四线程，而不是
掩盖失败或延长生产 TTL。
...
| §21 条目 | 权威收据 |
|---|---|
| 1、2、6、19 | `candidate-hash.json` |
| 3、4、5、7、11、12 | `cli-install.json` |
| 8 | `profile-quarantine.json`、`daemon-health.json` |
| 9 | `connector-failure.json`、`daemon-health.json` |
| 10 | `runtime-crash.json`、`subagent-runtime-task.json` |
| 13 | `upgrade-rollback.json`、`runtime-crash.json` |
| 14 | `metacog-events.jsonl` |
| 15 | `tui-tool-task.json`、`profile-quarantine.json`、`upgrade-rollback.json` |
| 16 | `candidate-hash.json`、`installed-hash.json`、`running-process-hash.json` |
| 17 | `tui-tool-task.json`、`subagent-runtime-task.json` |
| 18 | `running-process-hash.json`、`connector-failure.json`、`runtime-crash.json`、`cleanup.json` |
| 20 | `cleanup.json`；旧兼容类型仍保留，未绕过 §17.3 删除条件 |

### 23.5 实际部署结果

实际部署依次完成：

1. 通过正式脚本构建 release 候选，并用 `setup.sh --user --no-build`
   安装 binary、systemd socket 和 Bash/Zsh completions；
2. socket 激活用户 daemon，真实模型请求成功，`NRestarts=0`；
3. 安装并显式审批 `test.runtime`，v1.0、v1.1 正常；
4. 注入退出码 23 的 v1.2，daemon 自动恢复 v1.1，health 暴露
   `extension_rolled_back`；
5. 升级 v1.3 后，真实 TUI 接收中文并调用 `agent_list`；
6. `runtime.generic` 通过 `agent_spawn` 产生两条持久
   `succeeded` 记录，输出均为 `generic runtime acceptance succeeded`；
7. 注入未知 Tool 的 Profile 后，仅该 Profile 被 quarantine；删除故障
   Profile 并重启后 quarantine 为 `[]`，daemon 全程无 restart loop；
8. gbrain 连接故障保持 external degraded，但核心仍存活并接受任务；
9. purge 后扩展进程数和 activation 指针均为 0，daemon 仍 active。

因此 R0–R6 和 §21 的 20 项条件均已有实现、生产接线、负向测试及实际
部署证据。本规格的扩展平台范围可以标记为完成；§17.3 的历史兼容删除
仍是未来独立发布门禁，不属于本次“直接删除”。

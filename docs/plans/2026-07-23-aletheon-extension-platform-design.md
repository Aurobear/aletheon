...
下表是 R0 修正后的权威状态基线。后续修复不得跨行改写状态。

| 阶段 | 已实现 | 已接线 | 已验证 | 备注 |
|---|---|---|---|---|
| Phase 0 | ✅ | — | ✅ | 57 项单元/契约测试；daemon/TUI 收据待 R6 |
| Phase 1 | ✅ | ✅ | ✅ | 3 新文件 + From 投影，下游编译通过 |
| Phase 2 | ⚠ | ❌ | ❌ | Inspector/Manifest/Store 类存在；安全、事务、应用服务缺失 → R1/R2 |
| Phase 3 | ⚠ | ❌ | ❌ | 内存 quarantine 部分工作；previous-known-good、ready、TUI 缺失 → R3 |
| Phase 4 | ⚠ | ❌ | ❌ | Provider trait 已声明；生命周期和接线不完整 → R4 |
| Phase 5 | ⚠ | ❌ | ❌ | 子进程类可单行调用；沙箱、取消、协议校验缺失 → R4 |
| Phase 6 | ❌ | ❌ | ❌ | 仅 inspect/validate 执行逻辑，其余占位 → R0/R5 |
| Phase 7 | ❌ | ❌ | ❌ | 未执行 — 等待 R6 |

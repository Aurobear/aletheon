...
> 更新日期: 2026-07-13
> 基于 dev 分支 Clock 统一完成 + Turn-Path 收敛完成
> 本文档反映真实代码状态，非设计建议
...
| 时间统一 | **✅ Clock trait 覆盖 100%** — 132 处直接时间调用全部替换，仅剩 29 处有意保留的回退路径/测试代码 |
| Turn-Path 收敛 | **✅ 已提取共享 TurnPipeline** — daemon 路径通过 `TurnPipeline::run()`，exec 路径通过 `TurnService`，共享 `ReActLoop` + `AdmissionController` |
| dasein 跨服务依赖 | **无** — 仅依赖 fabric + kernel，已完成解耦 ✓ |
| bin side-channel | **已消除** — 仅依赖 executive + interact ✓ |

### 变更说明 (2026-07-13 Clock 统一完成 + Turn-Path 收敛)

**Clock 统一 (Phase 1-2):** 7 个领域 crate 从仅依赖 `fabric` 变为依赖 `fabric + kernel`。kernel 的 `Clock` trait 实现（`SystemClock`/`TestClock`）和 `Timer` 工具类被所有 crate 共享。132 处直接时间调用全部替换为 `Clock`/`Timer` 抽象，仅剩 29 处有意保留（`Timer` 自身实现、回退路径、IPC 传输层、测试辅助）。

**Turn-Path 收敛 (3 Stage):** 从 `DaemonTurnOrchestrator::execute_turn` 提取共享 `TurnPipeline`，消除 daemon 与 exec 两条路径的语义分裂。daemon 通过 `TurnPipeline::run()` 路由，exec 通过 `TurnService::submit()` 路由，两者共享相同的 `ReActLoop`、`AdmissionController`、工具执行语义。
...
| # | P0 项 | 当前状态 | 具体 Gap |
|---|-------|---------|----------|
| 1 | **唯一 Turn Execution Path** | ✅ 完成 | daemon 路径: `DaemonTurnOrchestrator` → `TurnPipeline::run()`；exec 路径: `ExecSessionBuilder` → `TurnService::submit()`。两者共享 `ReActLoop` + `AdmissionController`。`TurnPipeline` 从 orchestrator 提取为共享 Pre/Cognit/Post 管道。 |
| 2 | **拆 `handle_chat`** | ✅ 完成 | `chat.rs` (482 行 zombie) 已删除。`bridge/mod.rs` (空模块) 已删除。业务逻辑全部在 `DaemonTurnOrchestrator` + `TurnPipeline` 中。 |
...
| 6 | **Dasein lived time vs Kernel Chronos 边界** | ✅ 完成 | Clock 统一完成：dasein 所有 `Utc::now()`/`Instant::now()` 已替换为 `Clock`/`Timer` 抽象。watchdog `last_beat: Instant` → `MonoTime`，safe_mode `entered_at: Instant` → `MonoTime`。`TestClock::default()` 种子修复确保 0 不触发假过期。 |
...
| **P1** | `execute.rs` 拆出 event 转换函数 (230 行) | 降低单文件复杂度 | ✅ 已完成 (TurnPipeline 提取) |
...
### 8.2 ✅ 完成 (2026-07-13 — 100% 覆盖)

所有 132 处生产代码中的直接时间调用已替换为 `Clock`/`Timer` 抽象。仅剩 29 处有意保留：

| Crate | 保留数 | 原因 |
|-------|--------|------|
| fabric (Timer 自身) | 2 | `chronos.rs:39,48` — `tokio::time` 是 `Timer` 的实现基础 |
| fabric (回退路径) | 12 | `event_log.rs`, `envelope.rs`, `kernel_bus.rs`, `request_response.rs`, `in_process.rs`, `error.rs`, `debug_bus.rs` — `clock=None` 时回退到系统时间 |
| fabric (IPC 测试/传输) | 7 | `unix_socket.rs`, `unix_socket_transport.rs`, `io_uring.rs`, `mailbox.rs`, `debug.rs` |
| corpus (测试辅助) | 8 | `executor.rs` (并发测试需真实时间), `task_tools.rs` (测试便利) |

**逐 crate 最终注入 (PR-1~8 + follow-up):**

| Crate | 注入方式 | 替换前 | 保留 (有意) | Tests |
|-------|---------|--------|-------------|-------|
| agora | `Workspace::new(session_id, clock)` | 1 | 0 | 53 pass |
| metacog | `DefaultMetaRuntime` → pipeline → leaf | 10 | 0 | 17 pass |
| cognit | `CognitCore::new(clock)` → 子模块 | 1 | 0 | 2 pass |
| interact | `TuiApp` + ACLX context | 0 | 0 | 109 pass |
| corpus | `ToolContext.clock` + driver injection | 19 | 8 (test) | 368 pass |
| mnemosyne | `MemoryPipeline` → 所有 backend | 33 | 0 | 175 pass |
| dasein | `SelfField::new(clock)` → 子层 + watchdog/safe_mode 字段类型变更 | 29 | 0 | 279 pass |
| executive | `ServicePorts.clock` → handler/host/supervisor + struct 字段变更 | 32 | 0 | 313 pass |
| fabric | KernelEventBus/EventLog 可选 clock + 回退路径 | 17 | 17 (回退/传输) | 395 pass |
| kernel (residual) | `task_group.rs` 注入 | 1 | 0 | 132+ pass |
...
**✅ 已完成:**
- Clock 统一 (132 → 29 有意保留)
- Turn-Path 收敛 (daemon + exec 共享 `TurnPipeline`/`TurnService` + `ReActLoop`)
- Space 生命周期 (Phase 1/2a/2b/2c: 内核托管 fork → release → 继承)
- Agora 接口精简 (publish/update 降级为后端方法)
- Mnemosyne MemoryService facade
...
| 问题 | 严重度 | 影响 |
|------|--------|------|
| `fork_space` 从未被调用 | ✅ Phase 2b + 2c 已修复 | `ProcessTable::spawn()`（kernel-owned，`table.rs`）在 `spec.parent` 命中时调用 `space_manager.fork_space(parent_space, child_pid)` 继承父空间 bindings；parentless spawn（如主 agent）仍 mint 新根空间。Phase 2c：子 agent **生产路径**（`SubAgentSpawner`）现在通过 `set_shared_tables()` 共享 `ServicePorts.process_table`/`operation_table`，且 AgentTool 闭包读取 `CoreSystems.main_agent_process_id` 共享槽，经 `spawn_tracked_with_parent(..., Some(main_pid))` 传入真实 process parent，生产路径 fork-on-spawn 已触发 |
...
| **P1** | ✅ Phase 2c done：`SubAgentSpawner` 新增 `set_shared_tables()`，bootstrap（`init.rs`）在既有异步注入块里改为共享 `ServicePorts.process_table`/`operation_table`；`CoreSystems` 新增 `main_agent_process_id` 共享槽（`Arc<Mutex<Option<ProcessId>>>`），`DaemonTurnOrchestrator::ensure_main_agent` 写入、AgentTool 闭包读取，经新增 `spawn_tracked_with_parent(..., Some(main_pid))` 把真实 process parent 传给 `ProcessTable::spawn()`。Space 子系统现已端到端匹配设计 `Final(2).md` §9.5。唯一遗留项：`SpaceManager` **trait** 提升（`release`/`upsert_binding` → `Arc<dyn>`），推迟（YAGNI：单一实现、无 `dyn` 消费者） | 生产环境子 agent 真正继承父空间（fork-on-spawn 端到端生效） |

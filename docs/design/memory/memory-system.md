...
If daemon process crashes during Core Memory update (`append`/`replace`/`rethink`), block values may be partially written, or Recall Memory and Core Memory states may be inconsistent.

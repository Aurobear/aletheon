...
- **LoopDetector 已实现**（现行路径 `crates/corpus/src/security/loop_detector.rs`；旧 `crates/fabric/src/security/loop_detector.rs` 已退役）— 停滞检测 + 连续失败检测，自动阻断循环，对应"停下来"的触发条件
...
| 问题 | 关联文档 | 优先级 |
|------|----------|--------|
| L2 确认流程的 TUI 实现 | `../corpus/security.md` | P1 |
| 何时"主动出击" vs 等待指令 | 需新设计文档（ProactiveBehavior） | P1 |
| Agent 对自身行为的反思/修正机制 | `../cognit/cognitive-engine.md` | P2 |
...
| 多设备同步的数据加密 | `../corpus/platform.md` §5 | P2 |
...
| 跨设备记忆同步的冲突解决 | `../corpus/platform.md` §2 | P2 |
| 记忆污染检测（Agent 写入错误信息） | 需新设计文档 | P2 |
| ArchivalMemory 的自动索引和清理 | `../mnemosyne/memory-system.md` | P2 |
...
- **Agent 间冲突检测**为 ⬜ Planned（`../corpus/platform.md` §3）
- **Per-Agent LoopDetector 隔离**为 ⬜ Planned（`../corpus/security.md` §4.13）
...
- **DKMS 打包方案**在 `../corpus/platform.md` 中设计完成
- **自动降级机制已实现** — `IpcManager` 自动探测是否需要内核模块（`../corpus/platform.md`）
...
- **瓶颈检测**（`../dasein/perception.md`）已实现 — CPU/内存/IO/网络瓶颈检测，含升级建议
- **eBPF 命名源**当前只提供 `/proc`/`/sys` 回退；真实 probe/ring buffer 未实现
...
| FUSE 实际挂载、生命周期与性能 | ⬜ Planned | 从当前 stub 演进为可验收挂载 |
...
- **Android 平台适配仍是目标设计** — 当前 workspace 没有 Android target/backend
- **能力受限时的降级策略需要随未来 Android host contract 一并设计和验收**
- **Android 沙箱行为未验证** — 不应从 Linux 的 ProcessBackend 推断 Android production fallback
...
| 6 | 性能与资源 | 🟡 进行中 | 混合推理代码存在但未接入生产路由，FUSE 仅 stub，L3 Archival 非默认路径 |
| 7 | Android 碎片化 | ⬜ 未开始 | 当前 workspace 无 Android target/backend，也无安装态测试环境 |

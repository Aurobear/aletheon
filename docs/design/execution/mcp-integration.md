...
> OS-Agent 通过 MCP (Model Context Protocol) 接入第三方工具服务器，实现工具系统的开放扩展。

**模块编号:** 03 (MCP 子系统)
**关联模块:** [tool-system.md](tool-system.md), [sandbox.md](sandbox.md), [shared/traits.md](../shared/traits.md)
**最后更新:** 2026-06-07
...
MCP (Model Context Protocol) 是 Anthropic 提出的开放协议，允许外部进程以标准化方式向 AI Agent 暴露工具、资源和提示词。OS-Agent 作为 MCP 客户端，通过 `McpConnectionManager` 统一管理多个 MCP 服务器的生命周期。
...
| `McpToolWrapper` | 将 MCP 工具适配为 OS-Agent 的 `Tool` trait |
...
MCP 定义三种传输方式，OS-Agent 目前仅实现 stdio：
...
MCP 服务器通过 `tools/list` RPC 方法暴露可用工具。OS-Agent 在连接建立后调用此方法获取工具列表。
...
MCP 的 `inputSchema` (JSON Schema) 直接映射到 OS-Agent 的 `Tool::input_schema()` 返回值。处理要点：
...
MCP 服务器可通过 `notifications/tools/list_changed` 通知工具列表变更。OS-Agent 监听此通知并重新拉取工具列表，更新 `ToolRegistry`。
...
MCP 工具名限制 64 字节。OS-Agent 多服务器场景下：
- 截断后名称语义丢失
- SHA1 哈希后缀不可读
- 多服务器碰撞概率高
...
**McpToolWrapper** — 将 MCP 工具适配为 OS-Agent 的 Tool trait，将 `tools/call` 调用代理到远程服务器。
...
MCP 协议支持 `type: "image"`（base64）和 `type: "resource"`（URI 引用）content block。OS-Agent 的 `ToolResultContent` 枚举已定义对应变体：Text, Image, FileRef, Resource。
...
**已实现的文件:** `crates/agent-core/src/mcp/` (mod.rs, client.rs, config.rs, transport.rs, wrapper.rs)
...
**代码路径:** `crates/agent-core/src/mcp/` (6 files: mod.rs, client.rs, config.rs, transport.rs, wrapper.rs, auth.rs)

...
|-----------|--------|---------------|-------|
| AgentFs (in-memory API) | ✅ Implemented | `crates/agent-core/src/fuse/filesystem.rs` | In-memory virtual filesystem with read/write/readdir, pause support |
| FsNode types | ✅ Implemented | `crates/agent-core/src/fuse/filesystem.rs` | Directory/File/DynamicFile node types |
| FUSE module | ✅ Implemented | `crates/agent-core/src/fuse/mod.rs` | Module entry point |
| FuseMount | ✅ Implemented | `crates/agent-core/src/fuse/mount.rs` | `fuse3::path::PathFileSystem` integration behind `fuse` feature flag; stub mode when feature disabled |
| PathFileSystem impl | ✅ Implemented | `crates/agent-core/src/fuse/mount.rs` | `AgentFs` implements `fuse3::path::PathFileSystem` (lookup, getattr, read, write, readdir) behind `#[cfg(feature = "fuse")]` |
| StateProvider trait | ✅ Implemented | `crates/agent-core/src/fuse/provider.rs` | Abstraction for data sources (`get_sensor_data`, `get_context`, `get_log`, `get_agent_status`) |
| LiveStateProvider | ✅ Implemented | `crates/agent-core/src/fuse/provider.rs` | Reads live system state from `/proc` (loadavg, meminfo, diskstats, net/dev) |
| MockStateProvider | ✅ Implemented | `crates/agent-core/src/fuse/provider.rs` | In-memory mock with pre-settable responses and call recording for testing |
| ControlsValidator | ✅ Implemented | `crates/agent-core/src/fuse/controls.rs` | Write validation for `/controls/` (toggle "0"/"1", TOML syntax check, allowlist) |
...
- **挂载点权限**: `/mnt/agent/` 应由 agentd 创建和管理，挂载时使用 `allow_other` 或 `allow_root` 选项。
- **缓存策略**: `context/` 和 `sensors/` 的内容变化频繁，不应启用页缓存 (direct_io)。
- **日志轮转**: `logs/` 下的文件应支持大小限制和轮转，避免 FUSE 返回无限大的文件。
- **优雅关闭**: agentd 停止时应执行 `fusermount -u /mnt/agent/`，确保挂载点清理。
- **systemd 集成**: FUSE 挂载应作为 agentd.service 的一部分，或独立为 agent-fuse.service。
...
**Code location:** `crates/agent-core/src/fuse/` (5 files: mod.rs, filesystem.rs, mount.rs, provider.rs, controls.rs)

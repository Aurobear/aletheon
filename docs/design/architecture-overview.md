...
│   └── net-agent.toml + .md
├── crates/
│   ├── agent-core/               # 核心库 (~17K 行)
│   │   └── src/
│   │       ├── engine.rs         # ReAct 循环
│   │       ├── message.rs        # ContentBlock 类型
│   │       ├── config.rs         # TOML 配置
│   │       ├── provider_registry.rs
│   │       ├── llm/              # LLM 提供者 (OpenAI, Anthropic)
│   │       ├── tool/             # 工具系统 (9 个内置工具)
│   │       ├── security/         # 安全层 (策略、循环检测、审计、回滚)
│   │       ├── sandbox/          # 沙箱 (bubblewrap, process, noop)
│   │       ├── memory/           # 记忆系统 (L1/L2/L3, compressor)
│   │       ├── perception/       # 感知层 (proc, inotify, journald, bridge)
│   │       ├── orchestration/    # 多 Agent (委托、选择、DiGraph)
│   │       ├── hook/             # Hook 系统 (21 事件, 3 层配置)
│   │       ├── mcp/              # MCP 客户端 (stdio 传输已实现)
│   │       ├── plugin/           # 插件系统 (命令运行时)
│   │       ├── inference/        # 推理路由 (本地/云端)
│   │       ├── session/          # 会话管理 (SQLite, JSONL)
│   │       ├── ipc/              # IPC 层 (Unix Socket, io_uring 存根)
│   │       ├── fuse/             # FUSE 文件系统 (/mnt/agent/)
│   │       ├── learning/         # 学习系统 (pattern/outcome/rule)
│   │       └── platform/         # 平台适配器 (Linux, Android)
│   ├── agentd/                   # Daemon 入口 (~0.4K 行)
│   │   └── src/
│   │       ├── main.rs           # 启动、配置加载、感知桥接线
│   │       ├── handler.rs        # 请求处理、引擎初始化
│   │       └── server.rs         # (死代码，未使用，被 JsonRpcAdapter 替代)
...
```
/etc/argos/config.toml     # 系统配置
~/.argos/config.toml        # 用户配置 (覆盖系统)
.argos/config.toml          # 项目配置 (覆盖用户)
```

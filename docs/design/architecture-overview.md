...
> The vertical order above is illustrative; the parentheticals are the authoritative build
> dependencies. The five leaf subsystems (`mnemosyne`/`agora`/`corpus`/`cognit`/`metacog`)
> now depend on **`fabric` only** — RFC-018 D4 is resolved: `mnemosyne`'s old
> `cognit`/`corpus` coupling was removed (the shared `CompactorTrait` + tool-output pruner
> moved to `fabric::compaction`, and `LlmProvider` to `fabric::llm_types`), and `dasein` is
> cognit-free. Only `dasein` and `executive` compose other subsystems.

...
```
aletheon-abi          (no deps -- pure interfaces)
    ^
    |
aletheon-comm         (depends on abi)
    ^
    |
aletheon-memory       (depends on abi)
    ^
    |
aletheon-body         (depends on abi)
    ^
    |
aletheon-self         (depends on abi, body, comm, memory)
    ^
    |
aletheon-brain        (depends on abi, body)
    ^
    |
aletheon-runtime      (depends on abi, brain, body, self)
    ^
    |
aletheond             (depends on runtime)
aletheon-cli          (depends on body for TUI)

aletheon-meta         (depends on abi -- future self-evolution)
```
...
| Crate | 角色 | 核心内容 |
|-------|------|----------|
| `aletheon-abi` | 契约层 | 零实现，纯 Trait 定义和共享类型（message, tool, sandbox, llm_types, memory, event, genome） |
| `aletheon-comm` | 通信层 | EventBus、Unix Socket、io_uring、优先队列、消息路由 |
| `aletheon-memory` | 记忆层 | SQLite 后端：episodic、semantic、procedural、self_memory、MemoryRouter |
| `aletheon-body` | 执行层 | 工具、沙箱、MCP、感知、平台适配、驱动、UI、ACIX |
| `aletheon-self` | 主体场 | 身份、边界、关切、叙事、冲突、注意力、连续性、变异、Hook、安全、容错 |
| `aletheon-brain` | 认知层 | 推理、规划、批判、反思、学习、LLM 桥接、推理路由 |
| `aletheon-runtime` | 编排层 | ReAct 循环、会话、编排、记忆管道、插件、自动化、守护进程 |
| `aletheon-meta` | 自演化层 | MetaRuntime、Morphogenesis、Genome、候选生成、沙箱测试、迁移 |
| `aletheond` | 守护进程 | 入口点、配置加载、Unix Socket 服务、JSON-RPC |
| `aletheon-cli` | 客户端 | CLI/TUI、三种模式（单消息/TUI/REPL） |
...
```
User → aletheon-cli → Unix Socket → aletheond handler
  → Engine.run_turn()
    → Hook: PreLLMCall

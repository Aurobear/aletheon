...
cognit        (depends on fabric)
    ^
    |
dasein         (depends on fabric, corpus, cognit, mnemosyne)
    ^
    |
executive      (depends on fabric, cognit, corpus, mnemosyne, dasein, metacog)
    ^
    |
aletheond             (binary in executive crate)
...
| `executive` | 编排层 | ReAct 循环、会话、编排、记忆管道、插件、自动化、守护进程（aletheond binary）、Hook 系统 |

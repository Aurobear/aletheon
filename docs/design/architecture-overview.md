...
base         (depends on abi)
    ^
    |
memory       (depends on abi)
    ^
    |
corpus         (depends on abi)
    ^
    |
dasein         (depends on abi, body, comm, memory)
    ^
    |
cognit        (depends on abi, body)
    ^
    |
runtime      (depends on abi, brain, body, self)
    ^
    |
daemon             (depends on runtime)
cli          (depends on body for TUI)

metacog         (depends on abi -- future self-evolution)
```
...
| Crate | 角色 | 核心内容 |
|-------|------|----------|
| `base` | 契约层 | 零实现，纯 Trait 定义和共享类型（message, tool, sandbox, llm_types, memory, event, genome） |
| `base` | 通信层 | EventBus、Unix Socket、io_uring、优先队列、消息路由 |
...
| `runtime` | 编排层 | ReAct 循环、会话、编排、记忆管道、插件、自动化、守护进程 |
| `metacog` | 自演化层 | MetaRuntime、Morphogenesis、Genome、候选生成、沙箱测试、迁移 |
| `daemon` | 守护进程 | 入口点、配置加载、Unix Socket 服务、JSON-RPC |
| `cli` | 客户端 | CLI/TUI、三种模式（单消息/TUI/REPL） |
...
```
User → cli → Unix Socket → daemon handler
  → Engine.run_turn()
    → Hook: PreLLMCall

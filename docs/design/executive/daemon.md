...
`crates/aletheon/src/host/exec.rs:56-111`；Unix server 位于
`crates/aletheon/src/daemon/server.rs:622-708`。
...
Turn 的唯一接口定义于
`crates/aletheon/src/daemon/turn_engine.rs:14-22`。RPC handler 只做协议转换、
身份/上下文解析和 use-case 调用。

## 5. Host 模式

`aletheon` 可以按环境选择前台、systemd 或 container host。Systemd host 的
READY/WATCHDOG 行为位于 `crates/aletheon/src/composition/daemon_bootstrap/services.rs:31-131`。
...
并启动 `execd`：
`crates/aletheon/src/composition/daemon_bootstrap/security.rs:22-37`。

...
**已实现的文件:** `crates/corpus/src/tools/mcp/` (mod.rs, client.rs, config.rs, transport.rs, wrapper.rs)
...
**代码路径:** `crates/corpus/src/tools/mcp/` (6 files: mod.rs, client.rs, config.rs, transport.rs, wrapper.rs, auth.rs)

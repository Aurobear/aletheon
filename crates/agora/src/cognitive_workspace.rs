...
use crate::{AgoraOperation, AgoraProposal, AgoraService, WorkspaceCommitPermit};
use std::sync::Arc;
...
impl application::evaluation_projection::EvaluationProjectionSink
    for AgoraEvaluationProjectionSink
{
...
#[async_trait::async_trait]
impl runtime::agent_admission::CognitiveTaskAdmissionPort for CognitiveWorkspaceCoordinator {
    async fn bind_before_launch(
        ...
        .await
        .map(|_| ())
        .map_err(|error| ::contracts::AgentControlError {
            kind: match &error {
                CognitiveWorkspaceError::Reconciliation(_) => {
                    ::contracts::AgentControlErrorKind::Conflict
                }
                CognitiveWorkspaceError::Service(_) => ::contracts::AgentControlErrorKind::Runtime,
            },
...
impl CognitiveWorkspaceCoordinator {
    ...
    pub async fn commit_artifact_at(
        ...
        space: AgoraSpaceId,
        expected_version: u64,
        artifact: ::contracts::cognitive_workflow::CognitiveArtifactEnvelope,
        owner: ProcessId,
    ) -> Result<u64, CognitiveWorkspaceError> {
    ...
    pub async fn record_stage_decision_at(
        ...
        expected_version: u64,
        task_node_id: CognitiveTaskNodeId,
        decision: ::contracts::cognitive_workflow::StageDecision,
        owner: ProcessId,
    ) -> Result<u64, CognitiveWorkspaceError> {
        self.commit_operation_at(
            space,
...
mod tests {
    use super::*;
    use ::contracts::cognitive_workflow::{CognitiveStage, CognitiveTaskStatus};
    ...
    async fn root_spaces_are_isolated_and_role_projection_is_receipted() {
        let agora: Arc<dyn AgoraService> = Arc::new(crate::AgoraRegistry::new(Arc::new(
    ...
    async fn role_handoff_changes_authority_without_copying_peer_history() {
        use ::contracts::cognitive_workflow::CognitiveRoleProfile;
        let agora: Arc<dyn AgoraService> = Arc::new(crate::AgoraRegistry::new(Arc::new(
            kernel::chronos::TestClock::default(),
        )));
        let coordinator = CognitiveWorkspaceCoordinator::new(agora);
    ...
    async fn blocked_checkpoint_survives_restart_and_resumes_same_task() {
        use ::contracts::cognitive_workflow::{ClarificationState, CognitiveCheckpoint};
        let root = tempfile::tempdir().unwrap();
        let path = root.path().join("agora.db");
        let owner = ProcessId::new();
        let clarification_id = ClarificationId::new();
        {
            let persistence = Arc::new(crate::SqliteAgoraPersistence::open(&path).unwrap());
            let agora: Arc<dyn AgoraService> =
                Arc::new(crate::AgoraRegistry::new_with_persistence(
                    persistence,
        ...
        let persistence = Arc::new(crate::SqliteAgoraPersistence::open(&path).unwrap());
        let agora: Arc<dyn AgoraService> = Arc::new(crate::AgoraRegistry::new_with_persistence(
            persistence,

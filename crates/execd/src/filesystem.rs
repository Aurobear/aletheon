//! Workspace-confined filesystem operations for execd.
...
impl WorkspaceRoots {
    pub fn from_env() -> std::io::Result<Self> {
        let encoded = std::env::var("ALETHEON_EXECD_WORKSPACE_ROOTS").map_err(|_| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "ALETHEON_EXECD_WORKSPACE_ROOTS must be set",
            )
        })?;
        let configured: Vec<PathBuf> = serde_json::from_str(&encoded).map_err(|e| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!("invalid execd workspace roots: {e}"),
            )
        })?;
    ...
    fn new(configured: Vec<PathBuf>) -> Result<Self, String> {
        if configured.is_empty() {
            return Err("execd requires at least one workspace root".into());
        }
        ...
        if roots.iter().any(|r| !r.is_dir()) {
            return Err("execd workspace roots must be directories".into());
        }
        Ok(Self { roots })
    }
...
pub fn handle_fs(
    ...
    let result = match method {
        ...
        "fs/applyPatch" => apply_patch(params, workspace, runtime),
...
fn apply_patch(
    ...
    if request.patch.trim_start().starts_with("*** Begin Patch") {
        let patch = platform::structured_patch::parse_structured_patch(&request.patch)
            .map_err(|error| (INVALID_PARAMS, format!("invalid structured patch: {error}")))?;
        let result = execute_structured_patch(
            &patch,
            &working_dir,
            workspace,
            request.write_roots.as_deref(),
    ...
    let patch = platform::structured_patch::parse_unified_diff(&request.patch)
        .map_err(|error| (INVALID_PARAMS, format!("invalid unified patch: {error}")))?;
    let result = execute_structured_patch(
        &patch,
        &working_dir,
        workspace,
...
fn execute_structured_patch(
    patch: &platform::structured_patch::StructuredPatch,
    working_dir: &Path,
    workspace: &WorkspaceRoots,
    write_roots: Option<&[PathBuf]>,
    runtime: &tokio::runtime::Runtime,
) -> platform::structured_patch::StructuredPatchResult {
    use platform::structured_patch::{
        AppliedOperation, FailedOperation, FileChangeSummary, PatchOperation, StructuredPatchResult,
    };

    let roots = write_roots.unwrap_or(&workspace.roots);
    let scope_roots = roots
        .iter()
        ...
        .map(platform::HostPath::new)
        .collect::<Vec<_>>();
    let filesystem = platform::open_filesystem(platform::FilesystemScope {
        roots: scope_roots,
        readable_paths: vec![],
        access: platform::FilesystemAccess::ReadWrite,
        symlink_policy: platform::SymlinkPolicy::WithinRoot,
    });
        ...
        Err(error) => {
            return StructuredPatchResult {
                ...
                failed: patch
                    .operations
                    .iter()
                    .map(|operation| FailedOperation {
                        op_type: patch_operation_name(operation).into(),
                        path: patch_operation_path(operation).into(),
    ...
    for operation in &patch.operations {
        let outcome: Result<(AppliedOperation, FileChangeSummary), (String, Option<usize>)> =
            runtime.block_on(async {
                match operation {
                    PatchOperation::AddFile { path, content } => {
                        let target = platform::HostPath::new(working_dir.join(path));
                        match filesystem.metadata(&target).await {
                            Ok(_) => return Err((format!("target '{path}' already exists"), None)),
                        ...
                        create_parent(&*filesystem, &target).await?;
                        filesystem
                    ...
                    PatchOperation::UpdateFile {
                        ...
                        let source = platform::HostPath::new(working_dir.join(path));
                        let bytes = filesystem
                            .read(&source)
                            .await
                            .map_err(|error| (error.to_string(), None))?;
                        ...
                        let expected = format!("{:x}", Sha256::digest(&bytes));
                        let existing =
                            String::from_utf8(bytes).map_err(|error| (error.to_string(), None))?;
                        let modified =
                            platform::structured_patch::apply_patch_hunks(&existing, hunks)
                                .map_err(|(error, applied)| (error, Some(applied)))?;
                        let result_path = move_to.as_ref().unwrap_or(path);
                        let destination = platform::HostPath::new(working_dir.join(result_path));
                        create_parent(&*filesystem, &destination).await?;
                        filesystem
                            .atomic_write(platform::AtomicWrite {
                                path: destination,
                                content: modified.as_bytes().to_vec(),
                                expected_sha256: move_to.is_none().then(|| expected.clone()),
                                mode: None,
                            })
                            .await
                            .map_err(|error| (error.to_string(), None))?;
        ...
        match outcome {
            ...
            Err((error, hunks_applied_before_failure)) => {
                result.failed.push(FailedOperation {
                    op_type: patch_operation_name(operation).into(),
                    path: patch_operation_path(operation).into(),
                    error,
                    hunks_applied_before_failure,
                });
            }
        }
    }
...
async fn create_parent(
    filesystem: &dyn platform::FilesystemHost,
    target: &platform::HostPath,
) -> Result<(), (String, Option<usize>)> {
    if let Some(parent) = target.native().parent() {
        filesystem
...
fn patch_operation_name(operation: &platform::structured_patch::PatchOperation) -> &'static str {
    use platform::structured_patch::PatchOperation;
    match operation {
...
fn patch_operation_path(operation: &platform::structured_patch::PatchOperation) -> &str {
    use platform::structured_patch::PatchOperation;
    match operation {
        PatchOperation::AddFile { path, .. }
        | PatchOperation::DeleteFile { path }
        | PatchOperation::UpdateFile { path, .. }
        | PatchOperation::AppendFile { path, .. } => path,
    }
}
...
mod tests {
    ...
    fn all_filesystem_methods_preserve_workspace_confinement() {
        ...
        for method in ["fs/list", "fs/metadata", "fs/walk", "fs/remove", "fs/open"] {
            ...
            let response = handle_fs(method, &value, &roots, &files, &runtime()).unwrap();
            assert!(
    ...
    fn symlink_escape_is_denied_for_new_operations() {
        ...
        for method in ["fs/list", "fs/metadata", "fs/open"] {
            ...
            let response = handle_fs(method, &params, &roots, &files, &runtime()).unwrap();
            assert!(
                matches!(response.result, ResponseResult::Err { ref error } if error.code == FS_ACCESS_DENIED)
            );
        }
    ...
    #[test]
    fn unified_patch_is_applied_without_an_ambient_patch_process() {
        let temp = tempfile::tempdir().unwrap();

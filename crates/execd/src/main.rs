...
fn main() -> io::Result<()> {
    let expected_secret = std::env::var("ALETHEON_EXECD_SECRET").map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            "ALETHEON_EXECD_SECRET must be set",
        )
    })?;
    if expected_secret.is_empty() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "ALETHEON_EXECD_SECRET must not be empty",
        ));
    }
...
fn dispatch(
    ...
    match req.method.as_str() {
        ...
        method if method.starts_with("fs/") => {
            filesystem::handle_fs(method, &req.params, workspace_roots, file_manager, rt)
                .map(|mut r| {
                    r.id = req.id.clone();
                    r
                })

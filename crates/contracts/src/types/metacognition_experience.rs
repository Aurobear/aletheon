...
impl DomainId {
    ...
    pub fn new(raw: &str) -> Result<Self, &'static str> {
        ...
        if trimmed.is_empty() {
            return Err("domain id must not be empty");
        }
        if trimmed.len() > 64 {
            return Err("domain id exceeds 64 characters");
        }
        if trimmed.chars().any(|c| c.is_control()) {
            return Err("domain id contains control characters");
        }
        if !trimmed
            .chars()
            .all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_')
        {
            return Err("domain id contains invalid characters");
        }

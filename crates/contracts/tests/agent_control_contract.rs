...
fn request_validation_enforces_all_bounds() {
    ...
    invalid.task = "x".repeat(contracts::agent_control::MAX_AGENT_TASK_BYTES + 1);
    ...
    assert!(AgentWaitRequest {
        caller_root_agent_id: root,
        agent_id: AgentId::new(),
        timeout_ms: contracts::agent_control::MAX_AGENT_WAIT_MS + 1,
    }
    .validate()
    .is_err());
    ...
    assert!(AgentListRequest {
        caller_root_agent_id: root,
        status: None,
        limit: contracts::agent_control::MAX_LIST_ITEMS + 1,
    }
    .validate()
    .is_err());
}

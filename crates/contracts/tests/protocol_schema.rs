...
fn typed_requests_round_trip_at_the_supported_version() {
    ...
    for request in [
        ...
        ClientRequest::ReadEvents(EventSubscription {
            session_id: session_id.clone(),
...
fn memory_gateway_request_validation_is_bounded_and_typed() {
    ...
    observation.content = "x".repeat(contracts::protocol::memory::MAX_MEMORY_CONTENT_BYTES + 1);
...
fn versioned_mutations_round_trip_explicit_identity_tuples() {
    ...
    for request in vec![
        ...
        ClientRequest::Approval(ApprovalRequest {
            ...
            turn_id,
            operation_id,
            approval_id: contracts::ApprovalId::new(),
            version: 1,
...
fn daemon_compatibility_requests_own_method_and_parameter_names() {
    ...
    let scoped_chat = ClientRpcRequest::chat_for(
        "hello",
        contracts::SessionId("session-1".into()),
        &workspace,
    )
    .to_json_rpc(Some(71))
    .unwrap();
    ...
    let mode = ClientRpcRequest::mode_switch("plan")
        .to_json_rpc(Some(1))
        .unwrap();
    assert_eq!(mode["params"]["mode"], "plan");

    let interrupt = ClientRpcRequest::interrupt("user_cancelled")
        .to_json_rpc(Some(1))
        .unwrap();

...
use ::contracts::{
    AdmissionError, AdmissionRequest, CapabilityAuthority, CapabilityCall, CapabilityRequest,
    CapabilityScope, ExecutionPermit, InvocationControl, PermitId, PrincipalId, RevokeReason,
...
use kernel::capability::{
    CapabilityInvoker, DefaultCapabilityInvoker, StubToolExecutor, ToolExecutor,
};
...
async fn allow_all_admits_everything() {
    ...
    let req = AdmissionRequest {
        ...
        principal: ::contracts::PrincipalId("test-agent".into()),
        capability: ::contracts::CapabilityId("shell.execute".into()),
        action: "run".into(),
    ...
    assert!(permit.is_valid_at(::contracts::MonoTime(0)));
}
...
async fn settle_then_settle_again_fails() {
    ...
    let permit = ctrl
        .admit(AdmissionRequest {
            operation_id: Default::default(),
            process_id: Default::default(),
            principal: ::contracts::PrincipalId("test".into()),
            capability: ::contracts::CapabilityId("test".into()),
            action: "test".into(),
...
async fn sandbox_required_maps_to_passed_in_allow_all() {
    ...
    let req = AdmissionRequest {
        ...
        process_id: Default::default(),
        principal: ::contracts::PrincipalId("test".into()),
        capability: ::contracts::CapabilityId("dangerous.tool".into()),
        action: "execute".into(),
...
async fn a_cap_003_exhausted_budget_rejects_before_external_executor() {
    ...
    request.authority.budget = Some(::contracts::BudgetRequest {
        max_tokens: Some(1),
        max_cost_micro: None,
    });

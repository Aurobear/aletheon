...
use ::contracts::{
    BudgetRequest, CapabilityAuthority, CapabilityCall, CapabilityRequest, CapabilityResult,
    CapabilityScope, ExecutionPermit, ExitReason, InvocationControl, LeaseRequest, OperationKind,
    OperationRequest, PrincipalId, ProcessSignal, RevokeReason, SandboxRequirement, SpawnSpec,
    UsageReport,
};
use async_trait::async_trait;
use kernel::capability::{CapabilityInvoker, DefaultCapabilityInvoker, ToolExecutor};
...
struct Scenario {
    ...
    process: kernel::ProcessHandle,
    operation: kernel::OperationHandle,
    ...
    fn request(&self, cancel: CancellationToken) -> CapabilityRequest {
        CapabilityRequest {
            ...
            authority: CapabilityAuthority {
                ...
                thread_id: ::contracts::ThreadId("scenario".into()),
...
async fn tool_failure_settles_once_and_parent_cancel_cancels_descendants() {
    ...
    scenario
        .kernel
        .cancel_operation(scenario.operation.id, ::contracts::CancelReason::User)
        .await
        .unwrap();
    assert!(scenario
        .kernel
...
async fn reconstructed_kernel_rejects_terminal_operation_and_identity_ids() {
    ...
    assert!(reconstructed
        ...
        .is_err());
    assert!(reconstructed
        .bind_os_process_id(stale_process, ::contracts::OsProcessId(42))
        .await
        .is_err());
}

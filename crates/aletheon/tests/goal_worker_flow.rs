...
#[async_trait]
impl ChannelTurnApplicationPort for NoopTurn {
...
fn goal_id_from_text(message: &OutboundMessage) -> i64 {
    let gateway::channel::MessageContent::Text { text } = &message.content else {
        panic!("expected text")
    };
...
fn harness(outcomes: Vec<Result<RuntimeResult, RuntimeFailure>>) -> Harness {
    ...
    let router = ChannelRouter::new(
        SqliteChannelProjectionStore::new(ChannelStore::open(&channel_path).unwrap()),
        Arc::new(NoopTurn),
    );
...
async fn success_event_and_outbox_are_persisted_before_bounded_notification() {
    ...
    let gateway::channel::MessageContent::Text { text } = &sent[0].content else {
        panic!("expected text")
    };
...
async fn retry_backoff_notification_contains_ids_without_raw_error() {
    ...
    notify(&h, &outcome).await;
    let gateway::channel::MessageContent::Text { text } =
        &h.transport.sent.lock().unwrap()[0].content
    else {
        panic!("expected text")
    };

...
fn config_accepts_nonzero_bounded_tree_budget_and_storage_limits() {
    ...
    let parsed: aletheon::config::AppConfig =
...
fn spawn(root: AgentId, parent: Option<AgentId>, profile: &str) -> AgentSpawnRequest {
    AgentSpawnRequest {
        ...
        parent_process_id: parent.map(|_| ::contracts::ProcessId::new()),
...
async fn a_agent_003_lease_transitions_settle_once_and_expose_content_free_metrics() {
    ...
    assert_eq!(admission.metrics().resident_agents, 0);
    assert!(lease
        .settle(&::contracts::AttemptUsage::default())
        .await
        .is_err());
}
...
fn a_agent_003_service_reserves_policy_before_creating_any_child_resource() {
    // AgentControl's spawn path is split into the Runtime child-spawn module;
    // inspect both halves in execution order while keeping the Kernel mailbox

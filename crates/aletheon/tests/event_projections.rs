use ::contracts::{
    EnvelopeV2, EnvelopeV2Delivery, EnvelopeV2Target, ItemId, ItemPayload, ItemRecord, NamespaceId,
    SchemaId, SessionId, TurnId, SESSION_SCHEMA_VERSION,
};
...
use runtime::read_model::{EventProjection, EventProjectionSink};
use runtime::{
    EventId, EventIdentity, EventPayload, EventPosition, EventTreeId, EventVisibility,
    ParentEventId, SpineEvent, TreeSequence,
};
...
fn default_projection_set_reports_lag_and_poison_without_stopping_peers() {
    ...
    let projections = adapters_sqlite::projection_set::DefaultEventProjectionSet::in_memory();

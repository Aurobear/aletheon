...
use ::contracts::{
    AgentBudget, AgentContextFork, AgentControlError, AgentId, AgentMessageDeliveryState,
    AgentMessageKind, AgentProfileId, AgentResult, AgentSendRequest, AgentSpawnRequest,
    AgoraSpaceId, CapabilityCall, CapabilityResult, Clock, EnvelopeV2, EnvelopeV2Delivery,
    EnvelopeV2Target, ItemId, ItemPayload, ItemRecord, NamespaceId, PermitId, PrincipalId,
    RuntimeId, SchemaId, SessionId, SpawnSpec, TurnId, UsageReport, WorkspaceContent,
...
use runtime::read_model::EventProjection;
use runtime::{
    EventId, EventIdentity, EventPayload, EventSpine, EventTreeId, EventVisibility, SpineEvent,
    UnsequencedEvent,
};
...
fn agent_spawn_request(
    root: AgentId,
    parent: ::contracts::ProcessId,
    label: &str,
) -> AgentSpawnRequest {
    ...
    request.delegator_authority = Some(::contracts::AgentDelegationAuthority::new(
...
fn append_real_lifecycle_receipts(
    ...
    let session = ::contracts::SessionRecord {
...
async fn run_agent_lifecycle(
    ...
    clock: Arc<TestClock>,
    root_process: ::contracts::ProcessId,
    ...
    let repository = Arc::new(SqliteAgentRunProjection::open(&repository_path)?);
    let memory = Arc::new(mnemosyne::AgentMemoryVault::open(&memory_path)?);
    let spine = Arc::new(adapters_sqlite::event_spine::SqliteEventSpine::open(
        &event_path,
    )?);
    let projections = Arc::new(
        adapters_sqlite::projection_set::DefaultEventProjectionSet::open(&projection_path)?,
    );
    ...
    let reopened_repository = Arc::new(SqliteAgentRunProjection::open(&repository_path)?);
    ...
    let promotion_request = mnemosyne::MemoryPromotionRequest {
        ...
        root_content: ::contracts::ContentId(Uuid::from_u128(710)),
        broadcast: ::contracts::BroadcastEpoch(11),
        selected_candidate: ::contracts::ContentId(Uuid::from_u128(711)),
    ...
    let reopened_spine = adapters_sqlite::event_spine::SqliteEventSpine::open(&event_path)?;
...
struct CountingDasein {
    inner: Arc<dyn agora::conscious_core_ports::DaseinWorkspacePort>,
    enabled: bool,
    modulations: AtomicUsize,
}

#[async_trait]
impl agora::conscious_core_ports::DaseinWorkspacePort for CountingDasein {
    async fn modulate_salience(
        &self,
        candidate: &::contracts::WorkspaceCandidate,
    ) -> anyhow::Result<::contracts::SalienceVector> {
        if self.enabled {
    ...
    async fn integrate_broadcast(
        &self,
        broadcast: &::contracts::WorkspaceBroadcast,
    ) -> anyhow::Result<agora::conscious_core_ports::DaseinIntegration> {
        self.inner.integrate_broadcast(broadcast).await
    }

    async fn self_view(&self) -> anyhow::Result<::contracts::StructuredSelfView> {
...
#[async_trait]
impl ::contracts::ConsciousProcessor for FeedbackProcessor {
    fn id(&self) -> ::contracts::ProcessorId {
        ::contracts::ProcessorId("acceptance-feedback".into())
    }

    async fn on_broadcast(
        &self,
        broadcast: ::contracts::WorkspaceBroadcast,
        context: ::contracts::ProcessorContext,
    ) -> ::contracts::ProcessorResponse {
        let source = ::contracts::ProcessId(Uuid::from_u128(802));
        let now = self.clock.mono_now();
        ::contracts::ProcessorResponse {
            processor: self.id(),
            source_epoch: context.source_epoch,
            health: ::contracts::ProcessorHealth::Healthy,
            candidates: vec![::contracts::WorkspaceCandidate {
                schema_version: ::contracts::WORKSPACE_SCHEMA_V1,
                id: ::contracts::ContentId(Uuid::new_v5(
                    &Uuid::from_u128(803),
                ...
                source,
                turn: None,
                content: WorkspaceContent::Prediction(::contracts::PredictionFrame {
                    statement: "bounded recurrent response".into(),
                    horizon_ms: 1_000,
                }),
                confidence: 0.8,
                salience: ::contracts::SalienceVector {
                    urgency: 0.6,
                    goal_relevance: 0.8,
                    self_relevance: 0.7,
                    novelty: 0.6,
                ...
                provenance: ::contracts::WorkspaceProvenance {
                    producer: source,
                    operation: None,
                    source_refs: vec![
                ...
                visibility: ::contracts::VisibilityScope::Session,
                dependencies: broadcast.winner_ids.clone(),
                created_at: now,
                expires_at: Some(::contracts::MonoDeadline::after(now, 10_000)),
            }],
            acknowledgements: broadcast
                .winner_ids
                .iter()
                .map(|content_id| ::contracts::ProcessorAck {
...
pub async fn run_ablation(root: &Path, config: AblationConfig) -> anyhow::Result<AblationRun> {
    ...
    let inner: Arc<dyn agora::conscious_core_ports::DaseinWorkspacePort> =
    ...
    agora::conscious_core_ports::ConsciousCandidatePort::submit_candidate(
        &coordinator,
        agora::conscious_core_ports::CandidateSubmission {
            candidate: ::contracts::WorkspaceCandidate {
                schema_version: ::contracts::WORKSPACE_SCHEMA_V1,
                id: ::contracts::ContentId(Uuid::from_u128(805)),
                space: space.clone(),
                source: owner,
                turn: None,
                content: WorkspaceContent::Observation(::contracts::WorkspaceObservation {
            ...
            cause: agora::conscious_core_ports::CandidateCause::ExternalObservation { event_ref },
        },
    )
    .await?;
...
pub async fn run(root: &Path) -> anyhow::Result<HarnessRun> {
    ...
    let bounded_candidate = |id: u128, source: u128, label: &str| {
        ...
        candidate.id = ::contracts::ContentId(Uuid::from_u128(id));
        candidate.source = ::contracts::ProcessId(Uuid::from_u128(source));
    ...
    anyhow::ensure!(
        matches!(
            bounded_pool.admit(first_bounded.clone(), ::contracts::MonoTime(100)),
            agora::AdmissionOutcome::Accepted { .. }
        ),
        "bounded fixture first admission failed"
    );
    let duplicate_delivery_rejected = matches!(
        bounded_pool.admit(first_bounded, ::contracts::MonoTime(100)),
        agora::AdmissionOutcome::Duplicate { .. }
    );
    bounded_pool.admit(
        bounded_candidate(502, 601, "second"),
        ::contracts::MonoTime(100),
    );
    ...
    let memory_candidate_is_private = mnemonic_admitted
        && recalled.iter().all(|candidate| {
            matches!(
                ...
                ::contracts::VisibilityScope::PrivateProcess { .. }
            )
        });

    let action_loop = registry.resolve(space.clone(), owner, owner).await?;
    let mut call = CapabilityCall {
        operation_id: ::contracts::OperationId(Uuid::from_u128(11)),
        process_id: owner,
    ...
    let forged_outcome_denied = action_loop
        .observe_outcome(
            &aletheon::wiring::application::governed_capability::SelectedActionContext {
                ...
                attribution: ::contracts::WorkspaceAttribution::User,
            },
            &call,
            &CapabilityResult {
    ...
    let outcome: SelectedActionOutcomeReceipt = action_loop
        .observe_outcome(
            ...
            &CapabilityResult {
                ...
                audit_id: Some(::contracts::AuditEventId(Uuid::from_u128(13))),
                patch_delta: None,
                served_from_cache: false,
            },
        )

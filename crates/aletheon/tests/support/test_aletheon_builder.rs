...
impl TestAletheonBuilder {
    ...
    pub async fn build(self) -> TestAletheon {
        ...
        let coordinator = aletheon::host::session::test_composition::compose_from_session_store(
            kernel.clone(),
            store.clone(),
...
mod tests {
    use super::*;
    use ::contracts::{ItemPayload, SpawnSpec, TurnMetrics, TurnResult, TurnStop};
    use runtime::turn_policy::TurnPolicy;

    fn request(session: &str, process_id: ::contracts::ProcessId) -> ::contracts::TurnRequest {
    ...
    async fn build_creates_fully_wired_instance() {
        ...
        assert!(store_snapshot
            .load_items(&::contracts::SessionId("nonexistent".into()), None)
            .await
            .is_ok());
    ...
    async fn submit_simple_turn_writes_items_and_settles_operation() {
        ...
        let items = test
            .store
            .load_items(&::contracts::SessionId("builder-test".into()), None)
            .await
            .unwrap();

...
struct FakeTurnExecutor {
    ...
    intents: Mutex<Vec<::contracts::contract::command::ClientIntent>>,
}

#[async_trait::async_trait]
impl ChannelTurnApplicationPort for FakeTurnExecutor {
    async fn execute(&self, request: &ChannelTurnRequest) -> anyhow::Result<String> {
        ...
        self.intents
            .lock()
            .await
            .push(::contracts::contract::command::ClientIntent::v1(
                ::contracts::contract::command::ClientSurface::Gateway,
                request.principal.clone(),
                request.correlation_id.clone(),
                ::contracts::contract::command::ClientCommand::SubmitPrompt(prompt.clone()),
            ));
...
async fn owner_text_invokes_executor_once() {
    ...
    assert!(matches!(
        &intents[0].command,
        ::contracts::contract::command::ClientCommand::SubmitPrompt(prompt)
            if prompt.session_id.is_none()
    ));
...
async fn executor_failure_inbox_retryable_cursor_unchanged() {
    ...
    #[async_trait::async_trait]
    impl ChannelTurnApplicationPort for FailingExecutor {
        async fn execute(&self, _request: &ChannelTurnRequest) -> anyhow::Result<String> {

...
impl RuntimeProcessRegistrationPort for TestRuntimeProcessRegistration {
    ...
    ) -> Result<RuntimeProcessId, ::contracts::AgentControlError> {
        Ok(RuntimeProcessId {
...
fn input_with_inbox(
    ...
    let input = AgentRuntimeInput {
        ...
        delegation_authority: ::contracts::AgentDelegationAuthority::new(
            Some(policy),
        ...
        memory_context: mnemosyne::AgentMemoryContext::verified(
            process,
            agent,
            ::contracts::AgentTaskId(format!("pi-test-{label}")),
            "sha256:pi-test",
        )
        .unwrap(),
        request,
...
"#).unwrap();
    ...
    let config = aletheon::config::CodingRuntimeConfig {
        enabled: true,
    ...
    let registry = AgentExecutionRegistry::default();
    registry
        .register_manifested(
...
"#,
    ...
    let runtime = PiDelegateBackend::prepare(
        &config,
        Arc::new(FixtureSandbox {
            script,
            restrict_network: false,
...
"#,
    ...
    assert_eq!(error.kind, ::contracts::AgentControlErrorKind::Terminal);

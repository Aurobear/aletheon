...
use ::contracts::{
    BroadcastEpoch, CapabilityAuthority, CapabilityCall, CapabilityRequest, CapabilityResult,
    CapabilityScope, ConsciousArbitrationMode, ContentId, FieldDecisionKind, FieldDecisionReason,
    ...
    async fn authorize(&self, call: &CapabilityCall) -> Result<AuthorizedInvocation> {
        ...
        Ok(AuthorizedInvocation {
            authority: CapabilityAuthority {
                ...
                thread_id: ::contracts::ThreadId("session-conscious".into()),
...
fn call() -> CapabilityCall {
    CapabilityCall {
        operation_id: ::contracts::OperationId(Uuid::from_u128(10)),
...
fn ac_f_3_modulation_trace_round_trips_all_causal_fields() {
    ...
    let space = ::contracts::AgoraSpaceId("acceptance-field-trace".into());
    let expected = [
        (
            ConsciousArbitrationMode::Observe,
    ...
    let decoded: Vec<agora::ConsciousTraceEvent> = serde_json::from_slice(&encoded).unwrap();

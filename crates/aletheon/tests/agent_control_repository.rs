...
fn record(root: AgentId, parent: Option<AgentId>, created_at_ms: i64) -> AgentRunRecord {
    ...
    AgentRunRecord {
        ...
        resumability: ::contracts::RuntimeResumability::Never,
        recovery: None,
    }
}
...
async fn repository_reopens_and_preserves_parent_edge() {
    ...
    run.request_hash = SqliteAgentRunProjection::request_hash(&run.request).unwrap();
    let agent = run.agent_id();

    SqliteAgentRunProjection::open(&path)
        .unwrap()
        .create(&run)
        .await
        .unwrap();
    let reopened = SqliteAgentRunProjection::open(&path).unwrap();
...
async fn repository_migrates_pre_workspace_rows_without_losing_runs() {
    ...
    let repository = SqliteAgentRunProjection::open(&path).unwrap();
...
async fn duplicate_identity_and_request_hash_mismatch_are_conflicts() {
    let repository = SqliteAgentRunProjection::in_memory().unwrap();
    let root = AgentId::new();
    let run = record(root, None, 10);
    repository.create(&run).await.unwrap();
    let duplicate = repository.create(&run).await.unwrap_err();
    assert_eq!(duplicate.kind, ::contracts::AgentControlErrorKind::Conflict);
    ...
    assert_eq!(mismatch.kind, ::contracts::AgentControlErrorKind::Conflict);
}
...
async fn transitions_are_compare_and_swap_and_terminal_results_survive() {
    ...
    let conflict = repository
        ...
        .await
        .unwrap_err();
    assert_eq!(conflict.kind, ::contracts::AgentControlErrorKind::Conflict);
...
async fn root_list_is_bounded_filtered_and_deterministic() {
    let repository: Arc<dyn AgentRunProjection> =
        Arc::new(SqliteAgentRunProjection::in_memory().unwrap());
    let root = AgentId::new();
...
async fn message_reference_is_persisted_before_sequence_is_returned() {
    ...
    let payload = |content: &str| ::contracts::AgentMessagePayload {

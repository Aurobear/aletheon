...
impl TestSkillAdmin {
    fn new(
        loader: Arc<Mutex<corpus::SkillLoader>>,
        _cached_prefix: Arc<Mutex<String>>,
        _config_prompt: String,
    ) -> Self {
        Self { loader }
    }
}
...
fn setup_with_rollback(
    ...
    let skills = Arc::new(TestSkillAdmin::new(
...
async fn admin_surface_executes_confirmed_dual_runtime_rollback() {
    ...
    let manifest = platform::DeploymentManifest {
    ...
    let rollback = Arc::new(platform::DeploymentRollbackService::filesystem(
        manifest_path,
    ));
...
async fn skill_reload_refreshes_catalog_without_exposing_it_in_prefix() {
    ...
    assert!(prefix.lock().await.is_empty());
...
async fn transient_approval_and_shutdown_are_owned_by_admin_service() {
    ...
    let principal_id = ::contracts::PrincipalId::local_uid(1001);
    let connection_id = ::contracts::ConnectionId::new();
    let thread_id = ::contracts::ThreadId("thread-a".into());
    ...
    let service = AdminService::new(AdminResources {
        runtime: test_runtime(),
        skills: Arc::new(TestSkillAdmin::new(
    ...
    assert!(service
        .resolve_transient_approval(TransientApprovalRequest {
            ...
            connection_id,
            approval_id,
            decision: ::contracts::protocol::client::TransientApprovalDecision::ApproveForSession,
            scope_hint: None,
        })
        .await
        .unwrap());
...
async fn disconnect_cancels_only_connection_owned_pending_approvals() {
    let pending = PendingApprovals::default();
    let disconnected = ::contracts::ConnectionId::new();
    let live = ::contracts::ConnectionId::new();
    let owner = ApprovalOwner::new(
        ::contracts::PrincipalId::local_uid(1001),
        ::contracts::ThreadId("thread-a".into()),
    );
...
async fn approval_with_closed_consumer_reports_terminal_non_delivery() {
    ...
    let owner = ApprovalOwner::new(
        ::contracts::PrincipalId::local_uid(1001),
        ::contracts::ThreadId("thread-closed".into()),
    );
    let (sender, receiver) = oneshot::channel();
    let approval_id = pending
        .insert(
...
async fn scoped_session_grant_survives_reopen_and_never_widens_to_tool_grant() {
    ...
    let principal = ::contracts::PrincipalId("owner".into());
    let thread = ::contracts::ThreadId("thread".into());
    let hint = ::contracts::protocol::client::TransientApprovalScopeHint {

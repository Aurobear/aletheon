...
impl Fixture {
    ...
    fn router(&self) -> ChannelRouter {
        ...
        store
            ...
            .unwrap();
        let adapter = Arc::new(ApprovalRepositoryPort::new(self.repo.clone()));
        let callback = Arc::new(DaemonChannelApprovalCallbackAdapter::new(
            adapter.clone(),
    ...
    fn callback(&self, message: &str, sender: &str, action: String, time: i64) -> ProviderEnvelope {
        ProviderEnvelope {
            message: InboundMessage {
                ...
                message_id: gateway::channel::MessageId(message.into()),

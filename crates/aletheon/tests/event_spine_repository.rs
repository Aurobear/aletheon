...
use runtime::{
    EventId, EventIdentity, EventPayload, EventSpine, EventTreeId, EventVisibility, ParentEventId,
    TreeSequence, UnsequencedEvent,
};
...
fn allocates_monotonic_tree_order_under_concurrency() {
    ...
    let workers: Vec<_> = (0..16)
        .map(|_| {
            ...
            thread::spawn(move || {
                let candidate = event(tree, EventId::new());
                for attempt in 0..20 {
                    match store.append(candidate.clone()) {
                        Ok(receipt) => return receipt,
                        Err(error)
                            if error.to_string().contains("event spine overloaded")
                                && attempt < 19 =>
                        {
                            // SQLite's one-second busy timeout is a production
                            // backpressure boundary, but a thread-heavy test
                            // can transiently hit it while another worker holds
                            // the write lock. Retry only that typed overload;
                            // all schema/parent errors remain fatal.
                            thread::sleep(std::time::Duration::from_millis(10));
                        }
                        Err(error) => panic!("append failed: {error}"),
                    }
                }
                unreachable!("retry loop returns on success or panic")
            })
        })
        .collect();

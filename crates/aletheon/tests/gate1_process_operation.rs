...
//! | `sub_agent_created_in_process_table` | AgentHostAdapter uses shared ProcessTable |
...
async fn main_agent_exists_in_process_table() {
    ...
    let handle = kernel
        .spawn_process(SpawnSpec {
            namespace: ::contracts::NamespaceId("gate-1".into()),
            ..SpawnSpec::default()
        })
        .await
...
fn agent_control_is_the_only_subagent_process_owner() {
    // The former generic SubAgent facade is retired after caller-zero. Keep
    // the deletion assertion source-relative so this test does not resurrect
    // or compile against a compatibility module that no longer exists.
    assert!(!std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("src/core/sub_agent.rs")
        .exists());
...
async fn every_turn_has_operation_id() {
    ...
    let handle = kernel
        .spawn_process(SpawnSpec {
            namespace: ::contracts::NamespaceId("gate-1-turn".into()),
...
async fn process_exit_transitions_state_and_operation_is_terminal() {
    ...
    let handle = kernel
        .spawn_process(SpawnSpec {
            namespace: ::contracts::NamespaceId("exit-test".into()),
            ..SpawnSpec::default()
        })
        .await
        .unwrap();
    ...
    assert_eq!(
        exit.reason,
        ::contracts::ExitReason::Cancelled("terminated".into())
    );
    assert_eq!(
        kernel.inspect_process(handle.id).await.unwrap().state,
        ProcessState::Exited

use application::health::{ComponentHealth, HealthClass, HealthRegistry};
...
fn unix_server_retains_peer_credential_gate() {
    // The peer-credential gate lives on the host UnixServer (M8.2 host
    // UnixServer + ConnectionDispatcher seam); daemon/server.rs no longer
    // owns socket/peer-auth.
    let server = include_str!("../../aletheon/src/host/unix_server.rs");
    ...
    // Optional degradation remains admission-ready; only required unready
    // components fail the installed health probe.
    assert!(script.contains("\"ready\": 0, \"degraded\": 0, \"unready\": 2"));
}
...
fn live_storage_probe_reports_free_bytes_and_backup_marker_age() {
    ...
    let health = HealthRegistry::production_ready();
    platform::storage_health::refresh_storage(&health, directory.path(), 1, true, 60);
    let snapshot = health.snapshot();

...
async fn setup() -> (
    ...
    let goal_executor = Arc::new(DaemonChannelGoalApplicationPort::new(goals.clone()));
    let goal_commands = Arc::new(DaemonChannelGoalCommandAdapter::new(goal_executor, None));
    let router = ChannelRouter::new(
        SqliteChannelProjectionStore::new(channels),
        Arc::new(NoTurn),
    )
    .with_goal_command_port(goal_commands);

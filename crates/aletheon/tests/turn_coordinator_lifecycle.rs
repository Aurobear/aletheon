...
async fn coordinator_owns_turn_operation_and_ordered_canonical_items() {
    ...
    submitted.execution_target = ::contracts::ExecutionTargetSelection::robot(
        "robot-1",
            ...
            move |request, _| async move {
                *capture.lock().await = Some(request.operation_id);
                    ...
                    .append(
                        ...
                        ::contracts::ItemRecord {
                            ...
                            payload: ItemPayload::TaskProjection {
                                fact: ::contracts::TaskProjectionFact::default(),
                            },
                        },
                    )
                    .await?;
                Ok(TurnExecution {
                    ...
                    projection: None,
                    context_projection: Some(::contracts::ContextProjectionReceipt {
                        space: ::contracts::AgoraSpaceId("session-a".into()),
                        broadcast_epoch: Some(::contracts::BroadcastEpoch(2)),
                        workspace_version: Some(3),
                        dasein_version: ::contracts::dasein::SelfVersion(4),
                        content_ids: vec![::contracts::ContentId(uuid::Uuid::from_u128(5))],
                    }),
    ...
    let operation = kernel
        ...
        .await
        .unwrap();
    assert_eq!(operation.kind, ::contracts::OperationKind::Turn);
    ...
    assert!(matches!(
        ...
        ItemPayload::UserMessage {
            execution_target: ::contracts::ExecutionTargetSelection {
                target: ::contracts::ExecutionTarget::Robot { device_id, .. },
                source: ::contracts::ExecutionTargetSource::UserCommand,
            },
            ..
    ...
    let events = event_spine
        .read_tree(
            runtime::EventTreeId::for_root_session("success"),
            EventReadFilter {
                limit: 10,
                ..Default::default()
            },
    ...
    assert!(events
        .iter()
        .skip(2)
        .all(|event| event.schema.0 == ::contracts::SchemaId::TURN_EVENT_V1));
    for (index, event) in events.iter().enumerate() {
        assert_eq!(
            event.position.sequence,
            runtime::TreeSequence(index as u64 + 1)
        );
    }
}
...
async fn general_robot_general_turn_starts_remain_ordered_and_target_scoped() {
    ...
    let targets = [
        ...
        .unwrap(),
        ::contracts::ExecutionTargetSelection::general(
            ::contracts::ExecutionTargetSource::UserCommand,
        ),
    ];
...
async fn concurrent_general_and_robot_turns_do_not_share_target_or_settlement() {
    ...
    general.execution_target = ::contracts::ExecutionTargetSelection::default();
    let mut robot = request("concurrent-robot", robot_process.id);
    robot.execution_target = ::contracts::ExecutionTargetSelection::robot(
        "robot-1",
        ::contracts::types::embodiment::ExecutionEnvironment::Simulation,
...
impl cognit::harness::CognitiveSession for SeedCapturingSession {
    async fn run_turn(
        &mut self,
        request: TurnRequest,
        services: &dyn ::contracts::TurnServices,
        _events: &dyn ::contracts::TurnEventSink,
...
#[async_trait]
impl ::contracts::TurnServices for EmptyServices {
    async fn recall(
        &self,
        _request: ::contracts::RecallRequest,
    ...
    async fn invoke(&self, call: ::contracts::CapabilityCall) -> ::contracts::CapabilityResult {
...
async fn daemon_then_exec_restart_projects_prior_canonical_context() {
    ...
    for policy in [TurnPolicy::daemon(), TurnPolicy::exec()] {
        ...
        .submit(
            request("restart", process.id),
            &::contracts::NoopTurnEventSink,
        )
        .await
        .unwrap();
    }
...
async fn deadline_cancels_a_never_ending_turn_and_settles_exactly_once() {
    ...
    turn.deadline = Some(::contracts::MonoDeadlineMillis(60));
    let captured_operation = Arc::new(tokio::sync::Mutex::new(None::<::contracts::OperationId>));
    ...
    assert!(
        matches!(
            record.state,
            ::contracts::OperationState::Cancelled | ::contracts::OperationState::Failed
        ),
...
async fn deadline_forcibly_drops_an_uncooperative_runner_after_bounded_grace() {
    ...
    assert_eq!(operation.state, ::contracts::OperationState::Cancelled);
    assert_eq!(
        operation.exit,
        Some(OperationExitReason::Cancelled(
            ::contracts::CancelReason::DeadlineExceeded
        ))
    );
}
...
async fn user_cancel_on_deadline_bearing_turn_preserves_user_reason() {
    ...
    turn.deadline = Some(::contracts::MonoDeadlineMillis(10_000));
    let (started_tx, started_rx) = tokio::sync::oneshot::channel();
...
async fn connection_scoped_cancel_touches_only_that_connections_turn() {
    ...
    let conn_a = ::contracts::ConnectionId::new();
    let conn_b = ::contracts::ConnectionId::new();
...
async fn daemon_shutdown_cancels_and_settles_every_active_turn() {
    ...
    assert_eq!(
        operation.exit,
        Some(OperationExitReason::Cancelled(
            ::contracts::CancelReason::Shutdown
        ))
    );
}
    ...
    tokio::time::timeout(std::time::Duration::from_secs(1), async {
        loop {
            ...
            if coordinator.active_turn_count().await == 0
                && operation.state == ::contracts::OperationState::Failed
            {
                break;
            }
            tokio::task::yield_now().await;

...
struct TerminalFailingStore {
    inner: adapters_sqlite::session::canonical_store::CanonicalSessionStore,
}
...
impl SessionProjectionStore for TerminalFailingStore {
    ...
    async fn item_by_id(
        ...
        session: &SessionId,
        id: &::contracts::ItemId,
    ) -> anyhow::Result<Option<::contracts::ItemRecord>> {
...
async fn create_session_on_first_turn() {
    ...
    assert_eq!(session.status, ::contracts::SessionStatus::Active);
}
...
async fn event_spine_sequence_monotonic_across_turns() {
    ...
    let events = test
        .event_spine
        .read_tree(
            runtime::EventTreeId::for_root_session("mono-seq"),
            ...
            |_request, _cancel| async move {
                Ok(TurnExecution {
                    ...
                    context_projection: Some(::contracts::ContextProjectionReceipt {
                        space: ::contracts::AgoraSpaceId("test-space".into()),
                        broadcast_epoch: Some(::contracts::BroadcastEpoch(1)),
                        workspace_version: Some(2),
                        dasein_version: ::contracts::dasein::SelfVersion(3),
                        content_ids: vec![::contracts::ContentId(uuid::Uuid::from_u128(1))],
                    }),
                    evaluation_artifacts: Default::default(),
                })
            },

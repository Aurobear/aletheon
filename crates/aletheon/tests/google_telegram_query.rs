...
use gateway::ports::ExternalAccountDirectory;
use gateway::ports::{ChannelChatAdapter, ChannelTurnApplicationPort, ChannelTurnRequest};
...
#[async_trait]
impl ChannelTurnApplicationPort for Turn {
    async fn execute(&self, request: &ChannelTurnRequest) -> anyhow::Result<String> {
        let prompt = &request.prompt;
        self.calls
            .lock()
            .await
            .push((request.principal.0.clone(), prompt.content.clone()));
...
fn router(path: &std::path::Path, turn: Arc<Turn>, accounts: Vec<String>) -> ChannelRouter {
    ...
    let chat = ChannelChatAdapter::new(
        turn,
        ::contracts::WorkspacePolicy::from_resolved_roots(path.to_path_buf(), Vec::new()).unwrap(),
    )
    .with_read_port(Arc::new(ExternalReadPreprocessor::new(Arc::new(Accounts(
        accounts,
    )))));
    registry.register(Arc::new(ChatHandler::new(Arc::new(chat))));
...
fn account_binding_and_revocation_survive_repository_restart() {
    ...
    assert_eq!(restarted.state, application::ExternalIdentityState::Revoked);
    assert_eq!(grant.state, application::GrantState::Revoked);

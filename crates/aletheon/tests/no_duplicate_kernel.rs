...
fn executive_has_no_duplicate_kernel_authority() {
    ...
    assert!(workspace
        ...
        .is_file());
    assert!(workspace.join("crates/runtime/src/mailbox.rs").is_file());
    assert!(
        !workspace
            .join("crates/contracts/src/ipc/mailbox.rs")
            .exists(),
        "Runtime delegation must remain the unique mailbox owner"
    );
    assert!(workspace

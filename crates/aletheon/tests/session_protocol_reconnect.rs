...
async fn fixture() -> (Arc<dyn SessionAppendStore>, SessionService, SessionId) {
    ...
    store
        ...
        .await
        .unwrap();
    let service = SessionService::new(store.clone(), Arc::new(runtime::ActiveTurnRegistry::new()));
...
fn item_events(events: &[ClientEvent]) -> Vec<&::contracts::protocol::client::ItemEvent> {
    events
        .iter()
...
async fn live_and_durable_item_phases_share_one_reconnect_cursor() {
    ...
    store
        ...
        .await
        .unwrap();
    let service = SessionService::with_protocol_store(
        store.clone(),
        Arc::new(runtime::ActiveTurnRegistry::new()),
        Arc::new(
            adapters_sqlite::session::SqliteSessionProtocolEventStore::open(&journal_path).unwrap(),
        ),
    );
    ...
    drop(service);
    let reopened = SessionService::with_protocol_store(
        store,
        Arc::new(runtime::ActiveTurnRegistry::new()),
        Arc::new(
            adapters_sqlite::session::SqliteSessionProtocolEventStore::open(&journal_path).unwrap(),
...
async fn a_session_001_interruption_at_each_item_boundary_replays_the_same_task_snapshot() {
    ...
    let payloads = [
        ...
        ItemPayload::InferenceReceipt {
            receipt: ::contracts::types::inference_receipt::InferenceTerminalReceipt {
                ...
                usage: ::contracts::InferenceUsage::reported(100, 20, Some(75), Some(25), Some(0)),
    ...
    for (index, payload) in payloads.into_iter().enumerate() {
        ...
        let before = SessionService::with_protocol_store(
            store.clone(),
            Arc::new(runtime::ActiveTurnRegistry::new()),
            Arc::new(
        ...
        .unwrap();
        let replayed_store =
            aletheon::host::session::test_composition::compose_in_memory_session_store(Arc::new(
                CanonicalSessionStore::open(&canonical_path).unwrap(),
            ));
        let after = SessionService::with_protocol_store(
            replayed_store,
            Arc::new(runtime::ActiveTurnRegistry::new()),
            Arc::new(
                adapters_sqlite::session::SqliteSessionProtocolEventStore::open(
    ...
    let sessions = SessionService::new(store, Arc::new(runtime::ActiveTurnRegistry::new()))
...
async fn u_resume_001_daemon_restart_preserves_goal_plan_budget_and_checkpoint() {
    ...
    let reopened_store = aletheon::host::session::test_composition::compose_session_store(
        reopened_read,
        reopened_spine,
...
async fn u_resume_002_restart_recovery_marks_unsettled_commands_lost() {
    ...
    let store =
        aletheon::host::session::test_composition::compose_in_memory_session_store(read_model);
    ...
    let recovery =
        runtime::turn_recovery::scan_incomplete_turns(store.as_ref(), hardening.compaction_v2)
            .await
            .unwrap();
    assert_eq!(recovery.incomplete_turns.len(), 1);

    let snapshot = SessionService::new(store, Arc::new(runtime::ActiveTurnRegistry::new()))
        .protocol_read_snapshot(&session_id)
        .await
        .unwrap();
    ...
    assert_eq!(snapshot.tasks[0].phase, ::contracts::TaskPhase::Failed);
    assert_eq!(snapshot.activities.len(), 1);

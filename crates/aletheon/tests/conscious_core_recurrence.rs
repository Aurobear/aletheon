            ...
            .map(|id| {
                ...
                WorkspaceCandidate {
                    ...
                    content: WorkspaceContent::Observation(WorkspaceObservation {
                        ...
                        attribution: ::contracts::WorkspaceAttribution::Cognit,
                    }),
                    confidence: 1.0,
...
impl AgoraService for RejectCommitAgora {
    async fn view(&self, request: AgoraViewRequest) -> anyhow::Result<agora::AgoraView> {
        self.inner.view(request).await
    }
    async fn propose(&self, proposal: agora::AgoraProposal) -> anyhow::Result<Uuid> {
        self.inner.propose(proposal).await
    }
    async fn commit(
        &self,
        _id: Uuid,
        _permit: agora::WorkspaceCommitPermit,
    ) -> anyhow::Result<agora::CommitReceipt> {
        anyhow::bail!("injected attention commit failure")
    }
    async fn reject(&self, id: Uuid, reason: agora::RejectReason) -> anyhow::Result<()> {
    ...
    async fn changes_since(
        ...
        space: AgoraSpaceId,
        version: u64,
    ) -> anyhow::Result<Vec<agora::AgoraCommit>> {

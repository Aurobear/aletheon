...
impl Clock for TestClock {
    fn wall_now(&self) -> ::contracts::WallTime {
        ::contracts::WallTime(50_000)
    }
    fn mono_now(&self) -> ::contracts::MonoTime {
        ::contracts::MonoTime(50_000)
    }
}
...
async fn advisory_warning_is_approval_ready_and_duplicate_call_is_idempotent() {
    ...
    let first = h
        ...
        .unwrap();
    assert!(
        matches!(first, AttemptCoordinationOutcome::Succeeded { ref goal, .. } if goal.wait_reason == Some(::contracts::GoalWaitReason::ExternalEvent { key: "approval required".into() }))
    );
    let duplicate = h
        .coordinator
...
async fn verifier_service_error_blocks_and_is_never_approval_ready() {
    ...
    assert_eq!(
        goal.wait_reason,
        Some(::contracts::GoalWaitReason::ExternalEvent {

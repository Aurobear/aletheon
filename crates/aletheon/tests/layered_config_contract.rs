...
use aletheon::config::{
    ...
    ConfigSourceKind, EnvironmentCredentialResolver, PrefixCacheCapability, SessionWriterMode,
    Transport,
};
...
fn project_cannot_grant_itself_network_authority() {
    ...
    assert_eq!(
        ...
        corpus::security::network_policy::NetworkDefaultAction::Deny
    );
...
#[test]
fn session_writer_mode_is_typed_and_unknown_values_fail_closed() {
    let runtime = layer(
        ConfigSourceKind::System,
        "/etc/aletheon/config.toml",
        "[bootstrap]\nsession_writer='runtime'",
    );
    let loaded = merge_layers([runtime]).unwrap();
    assert_eq!(
        loaded.value.bootstrap.session_writer,
        SessionWriterMode::Runtime
    );

    let error = ConfigLayer::from_toml(
        ConfigSource::new(ConfigSourceKind::System, "/etc/aletheon/config.toml"),
        "[bootstrap]\nsession_writer='typo'",
    )
    .unwrap_err()
    .to_string();
    assert!(error.contains("/etc/aletheon/config.toml"), "{error}");
...
"#,
    ...
    .unwrap();
    let _: &aletheon::config::TelegramChannelConfig = &config.telegram;
    let _: &aletheon::config::CodingRuntimeConfig = &config.pi_runtime;
...
fn legacy_supplemental_memory_keys_are_read_but_never_reemitted() {
    let memory: mnemosyne::supplemental_memory::MemoryConfig = toml::from_str(
        r#"
backend = "sqlite"
data_dir = "/tmp/memory"
[gbrain]
...
fn checked_in_lejurobot_deepseek_flash_uses_the_openai_transport() {
    for relative_path in [
        ...
        let expected_evaluation_mode = if path.ends_with("default.toml") {
            ::contracts::EvaluationMode::Enforce
        } else {

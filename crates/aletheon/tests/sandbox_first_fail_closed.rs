...
use kernel::capability::{CapabilityInvoker, DefaultCapabilityInvoker, StubToolExecutor};
use std::sync::Arc;
...
impl AdmissionController for SandboxRequiredAdmissionController {
    async fn admit(&self, request: AdmissionRequest) -> Result<ExecutionPermit, AdmissionError> {
        Ok(ExecutionPermit {
            ...
            expires_at: ::contracts::MonoDeadline::after(::contracts::MonoTime(0), 60_000),
...
async fn sandbox_decision_required_makes_permit_valid_at_false() {
    ...
    assert!(
        !permit.is_valid_at(::contracts::MonoTime(0)),

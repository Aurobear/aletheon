...
impl Fixture {
    ...
    fn resolve(
        ...
        let repo = adapters_sqlite::approval_repository::ApprovalRepository::open(&self.path)?;
        Ok(repo.resolve(
            draft.approval.id,
...
fn telegram_owner_confirmation_is_durable_and_wrong_or_email_identity_cannot_activate() {
    ...
    assert!(f
        ...
        .is_err());
    let repo = adapters_sqlite::approval_repository::ApprovalRepository::open(&f.path).unwrap();
    assert!(repo
        .resolve(
            draft.approval.id,
            0,
...
impl ChannelTurnApplicationPort for NoTurn {
    async fn execute(&self, _: &ChannelTurnRequest) -> anyhow::Result<String> {
...
struct CaptureTransport {
    sent: AsyncMutex<Vec<gateway::channel::OutboundMessage>>,
}
...
impl ChannelTransport for CaptureTransport {
    ...
    async fn send(&self, message: &gateway::channel::OutboundMessage) -> anyhow::Result<String> {
...
fn callback(id: &str, sender: &str, action: String, now_ms: i64) -> ProviderEnvelope {
    ProviderEnvelope {
        message: gateway::channel::InboundMessage {
            channel_id: gateway::channel::ChannelId("telegram".into()),
            message_id: gateway::channel::MessageId(id.into()),
            conversation_id: gateway::channel::ConversationId("42".into()),
            sender_id: gateway::channel::ExternalSenderId(sender.into()),
            content: gateway::channel::MessageContent::Text {
                text: String::new(),
            },
            timestamp_ms: now_ms,
...
async fn telegram_review_has_confirm_edit_reject_and_replayed_confirm_is_idempotent() {
    ...
    let approval_adapter = Arc::new(ApprovalRepositoryPort::new(approval_repo));
    let resolvers = Arc::new(ApprovalResolverRegistry::new());
    resolvers.register(ApprovalCategory::ActivateGoal, gmail_resolver);
    let callback_port = Arc::new(DaemonChannelApprovalCallbackAdapter::new(
        approval_adapter.clone(),
        resolvers,
    ));
    let mut router = ChannelRouter::new(
        SqliteChannelProjectionStore::new(channels),
        Arc::new(NoTurn),
    )
    .with_approval_callback_port(callback_port)
    .with_approval_delivery_port(approval_adapter);
    let transport = CaptureTransport::default();
    assert!(router
        .notify_approval(
            &transport,
            gateway::channel::ConversationId("42".into()),
            &draft.approval,
            101,
        )
        .await

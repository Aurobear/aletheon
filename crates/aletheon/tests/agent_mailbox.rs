...
async fn repository_rejects_invalid_schema_oversize_and_conflicting_settlement() {
    ...
    assert!(repository
        ...
        .await
        .is_err());
    let oversized = payload(&"x".repeat(::contracts::agent_control::MAX_AGENT_MESSAGE_BYTES + 1));
    assert!(repository
...
fn envelope(payload: AgentMessagePayload, priority: u8) -> EnvelopeV2 {
    EnvelopeV2::new(
        ...
        ::contracts::NamespaceId("mailbox-test".into()),
...
async fn normal_overload_never_drops_the_reserved_high_priority_signal() {
    ...
    assert!(matches!(
        ...
        runtime::mailbox::DeliveryReceipt::Rejected { .. }
    ));

...
use ::contracts::{
    CapabilityAuthority, CapabilityCall, CapabilityRequest, CapabilityResult, CapabilityScope,
...
async fn streaming_flag_path_forwards_progress_and_preserves_settled_result() {
    ...
    assert!(matches!(
        stream.try_recv(),
        Some(Ok(::contracts::ipc::TurnEventV1::ToolProgress {
            name,
            call_id,
            kind,
            payload,
...
async fn model_arguments_cannot_replace_capability_workspace() {
    ...
    let provider = RegistryAuthorityProvider::new(
        ...
        ::contracts::ThreadId("thread-1".into()),

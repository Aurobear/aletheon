...
use metacog::coding_evidence_adapter::{CodingEvidenceAdapter, DefaultCodingEvidenceAdapter};
use metacog::coding_rubric::{coding_rubric_v1, CODING_RUBRIC_V1};

use ::contracts::types::coding_job::{
...
use ::contracts::types::metacognition_evaluation::{DimensionValue, RubricId};
use ::contracts::types::metacognition_experience::ExperienceOutcome;
...
fn make_success_report(job_id: uuid::Uuid) -> CodingJobReport {
    CodingJobReport {
        ...
        goal_id: ::contracts::GoalId(1),
        attempt_id: ::contracts::AttemptId::new(),
...
) -> ::contracts::types::metacognition_evaluation::EvaluationReport {
    ...
    for gate in &mut report.gates {
        match gate.name.as_str() {
            "verification_evidence" => {
                ...
                if !gate.passed {
                    gate.evidence
                        .push(::contracts::types::metacognition_evidence::EvidenceId(
                            "ev-gate-v".to_string(),
                        ));
                }
            }

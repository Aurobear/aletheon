...
struct ScriptedLlm {
    ...
    seen: Mutex<Vec<(Vec<::contracts::Message>, Vec<ToolDefinition>)>>,
    block: bool,
}
...
fn input(cancel: CancellationToken) -> AgentRuntimeInput {
    ...
    AgentRuntimeInput {
        ...
        memory_context: mnemosyne::AgentMemoryContext::verified(
            process,
            root,
            ::contracts::AgentTaskId("test-task".into()),
        ...
        workspace_id: ::contracts::AgoraSpaceId(format!("agent:{}", root.0)),
        root_workspace_id: ::contracts::AgoraSpaceId(format!("root:{}", root.0)),
        root_process_id: process,
        inbox: AgentRuntimeInbox::empty(),
        request,
        cancellation: cancel,
        runtime_process: Arc::new(runtime::NoopRuntimeProcessRegistration),
...
fn cognitive_binding(
    ...
    CognitiveTaskRuntimeBinding {
        space: ::contracts::AgoraSpaceId("root-task".into()),

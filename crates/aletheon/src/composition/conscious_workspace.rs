...
fn bound_recent_modulations(
    mut events: Vec<agora::ConsciousTraceEvent>,
    requested_limit: usize,
) -> (Vec<agora::ConsciousTraceEvent>, usize) {
...
pub struct ConsciousFieldDiagnostics {
    ...
    pub indicators: metacog::evaluation::FieldMetricIndicators,
    pub modulations: Vec<agora::ConsciousTraceEvent>,
...
use crate::adapters::conscious::{
    AgentAdapter, CorpusProcessor, MetacogProcessor, MnemosyneProcessor,
};
use crate::composition::agent_control::AgentCandidateSubmissionPort;
use ::kernel::capability::governed::{GovernedActionLoop, GovernedActionLoopResolver};
use agora::conscious_core_ports::{
    CandidateAdmissionStatus, CandidateCause, CandidateSubmission, ConsciousCandidatePort,
    ConsciousCoreConfig, ConsciousCycleReceipt, DaseinWorkspacePort, ProcessorRegistration,
};
...
#[async_trait]
pub trait ConsciousTurnPort:
    GovernedActionLoopResolver + application::conscious::ConsciousObservationPort
{
...
struct ProjectedActionProposal {
    content: ::contracts::ActionProposalFrame,
    confidence: f32,
}
...
impl ConsciousWorkspaceBatchPlanner {
    ...
    fn metric_ref(&self, epoch: ::contracts::BroadcastEpoch) -> String {
        self.coordinator
...
fn reorder_trace_event(
    ...
    epoch: ::contracts::BroadcastEpoch,
    metric_ref: &str,
) -> agora::ConsciousTraceEvent {
    agora::ConsciousTraceEvent::FieldModulation {
        mode,
...
impl cognit::harness::BatchPlanner for ConsciousWorkspaceBatchPlanner {
    async fn plan(&self, calls: Vec<CapabilityCall>) -> anyhow::Result<CapabilityBatchPlan> {
        ...
        let priorities: Vec<f32> = candidates
            ...
            .collect();
        let ordered_call_ids = cognit::harness::linear::stable_priority_order(
            &calls
                .iter()
                .zip(priorities.iter())
...
/// Narrow consumer port for conscious-field diagnostics used by the daemon
/// transport (health RPC).
#[async_trait]
pub trait ConsciousWorkspaceRegistryPort: Send + Sync {
...
#[async_trait]
impl ConsciousWorkspaceRegistryPort for ConsciousWorkspaceRegistry {
    fn field_diagnostics(
        &self,
        space: &AgoraSpaceId,
        requested_limit: usize,
    ) -> anyhow::Result<Option<ConsciousFieldDiagnostics>> {
        ConsciousWorkspaceRegistry::field_diagnostics(self, space, requested_limit)
    }
}
        ...
        let processors: Vec<(Arc<dyn ConsciousProcessor>, Vec<&str>, VisibilityScope)> = vec![
            ...
            (
                ...
                VisibilityScope::PrivateProcess {
                    process: crate::adapters::conscious::processor_source(space, "mnemosyne"),
                },
            ),
            (
...
impl AgentCandidateSubmissionPort for ConsciousWorkspaceRegistry {
    async fn submit_agent_candidate(
        ...
        recipient: ProcessId,
        root: ProcessId,
    ) -> anyhow::Result<agora::conscious_core_ports::CandidateSubmissionReceipt> {
...
#[async_trait]
impl application::conscious::ConsciousObservationPort for ConsciousWorkspaceRegistry {
    async fn observe_turn(
        &self,
        observation: application::conscious::ConsciousTurnObservation,
    ) -> anyhow::Result<application::conscious::ConsciousTurnReceipt> {
        let application::conscious::ConsciousTurnObservation {
            space,
            owner,
            root,
            operation,
        ...
        Ok(application::conscious::ConsciousTurnReceipt {
            operation_id: receipt.operation_id,
            depth: receipt.depth,
            opened_at: receipt.opened_at,
            broadcast: receipt.broadcast,
            dasein_transition: receipt.dasein_transition,
            processors: receipt
                .processors
                .into_iter()
                .map(|status| application::conscious::ConsciousProcessorStatus {
                    processor: status.processor,
                    health: status.health,
                    source_epoch: status.source_epoch,
                    admitted_candidates: status.admitted_candidates,
...
mod diagnostic_tests {
    ...
    fn event(index: usize) -> agora::ConsciousTraceEvent {
        agora::ConsciousTraceEvent::Prediction {
    ...
    fn batch_reorder_builds_complete_causal_modulation_trace() {
        let call = CapabilityCall {
            operation_id: ::contracts::OperationId(Uuid::from_u128(41)),
        ...
        let decision = CapabilityBatchDecision {
            ...
            reason: FieldDecisionReason::Selected,
            priority: 0.75,
            broadcast_epoch: Some(::contracts::BroadcastEpoch(9)),
        };
        let trace = reorder_trace_event(
            ConsciousArbitrationMode::Observe,
            &call,
            &decision,
            ::contracts::BroadcastEpoch(9),
            "metric:9",
        );
        let agora::ConsciousTraceEvent::FieldModulation {
            mode,
            decision,
            reason,
            operation_id,
    ...
    fn same_turn_projection_reorders_by_distinct_host_confidence() {
        let readout = ConsciousFieldReadout {
            epoch: ::contracts::BroadcastEpoch(10),
            care_action: None,
            concern_urgency: 0.8,
            salience: max_salience(),
            precision: 0.8,
        ...
        assert_eq!(
            cognit::harness::linear::stable_priority_order(&priorities).unwrap(),

...
pub use crate::adapters::agent_memory::{
    context_projection_receipt, MemoryRecordingAgentEventSink,
};
pub use crate::adapters::conscious::agent_projection::{
...
/// The fixture constructor is excluded from production composition.
pub struct AgentHostAdapter {
    kernel: Arc<KernelRuntime>,
    clock: Arc<dyn Clock>,
    repository: Arc<dyn AgentRunProjection>,
    admission: Arc<dyn AgentAdmissionPort>,
    /// Fixture launcher catalog retained only for integration tests. Production composition uses Runtime's DelegateBackendRegistry
    /// and constructs this service without a second catalog.
    runtimes: Option<Arc<dyn FixtureRuntimeCatalog>>,
...
impl AgentHostAdapter {
    ...
    ///
    /// The borrowing variant remains available for integration fixtures
    ...
    pub fn facades(self: &Arc<Self>) -> AgentHostFacades {
        let host_control: Arc<dyn AgentControlPort> =
            Arc::new(RuntimeAgentControlFacade::new(self.clone()));
        AgentHostFacades {
            control: Arc::new(application::agent::AgentService::new(host_control)),
            effects: self.clone(),
...
/// Narrow integration-fixture launcher catalog. Production selection and lifecycle use
/// `RuntimeAgentSupervisor`; installed composition never constructs this port.
pub trait FixtureRuntimeCatalog: Send + Sync {
...
impl AgentHostAdapter {
    ...
    pub fn new_fixture(
        ...
        repository: Arc<dyn AgentRunProjection>,
        admission: Arc<dyn AgentAdmissionPort>,
        runtimes: Arc<dyn FixtureRuntimeCatalog>,
        event_spine: Arc<dyn EventSpine>,
    ) -> Self {
    ...
    /// Runtime is the production selection authority. The fixture registry is available only to isolated integration fixtures
    /// constructed without a Runtime supervisor.

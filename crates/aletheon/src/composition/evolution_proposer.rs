...
use adapters_sqlite::{
    approval_repository::{ApprovalCreate, ApprovalRepository},
    goal::ObjectiveStore,
};
use sha2::Digest;
use std::{
...
pub struct GovernedEvolutionProposer {
    ...
    evidence: Arc<SqliteCapabilityRollupProjectionSink>,
    clock: Arc<dyn ::contracts::Clock>,
    state: Arc<dyn application::evolution::EvolutionProposalStore>,
}
...
mod tests {
    ...
    async fn requires_distinct_durable_profiles_and_only_creates_pending_approval() {
        ...
        let evidence = Arc::new(
            SqliteCapabilityRollupProjectionSink::open(temp.path().join("eval.db")).unwrap(),
        );
        ...
        let approval = proposer
            ...
            .unwrap()
            .unwrap();
        assert_eq!(approval.status, ::contracts::ApprovalStatus::Pending);
...
impl GovernedEvolutionProposer {
    ...
    pub fn propose(
    ...
    ) -> anyhow::Result<Option<::contracts::ApprovalSnapshot>> {
        if receipt.decision != metacog::VerificationDecision::Adopt {
            return Ok(None);
        }
        if let Some(id) = self.state.pending_approval(receipt.mutation_id)? {
            return Ok(self.approvals.lock().unwrap().get(id)?);
        }
        let evidence = self
            .evidence
        ...
        if evidence.len() < 2 || profiles.len() < 2 {
            self.state
                .park_insufficient_evidence(receipt.mutation_id, self.clock.wall_now().0)?;
            return Ok(None);
        }
        ...
        self.state.record_pending_approval(
            receipt.mutation_id,
            approval.id,
            &evidence_digest,
            now,

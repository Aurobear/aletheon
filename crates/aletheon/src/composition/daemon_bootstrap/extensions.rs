...
pub(super) struct ExtensionExecutableRuntime {
    ...
    agent_host:
        RwLock<Option<std::sync::Weak<dyn crate::composition::agent_control::AgentHostEffects>>>,
...
impl PackageRuntimeStaging {
    ...
    ) -> Option<Arc<dyn crate::composition::agent_control::AgentRuntimeLauncher>> {
...
impl ExtensionExecutableRuntime {
    ...
    pub async fn publish(
        ...
        let launcher: Arc<dyn crate::composition::agent_control::AgentRuntimeLauncher> =
...
pub async fn reconcile_extension_snapshot(
    ...
    for activation in activations
        .iter()
        .filter(|activation| activation.enabled || activation.health == "quarantined")
    {
        // Quarantine deliberately persists `enabled = false`, but recovery must
        // probe the selected package as a candidate without first granting it
        // durable authority. The resolver only admits enabled candidates.
        let mut candidate_activation = activation.clone();
        candidate_activation.enabled = true;
        match admit_activation(
            &resolver,
            compiler,
            publisher,
            &resolved,
        ...
        .await
        {
            Ok((candidate_resolved, candidate_snapshot)) => {
                if !activation.enabled || activation.health == "quarantined" {
                    let mut recovered = activation.clone();
                    recovered.enabled = true;
                    recovered.health = "healthy".into();
                    recovered.quarantine_reason = None;
                    store.write_activation(&recovered)?;
                    append_recovery_evidence(
                        &store,
                        "successful_recovery",
                        &recovered.package_id,
                        "reenabled",
                        recovered.current.as_deref(),
                    )?;
                }
            ...
            Err(candidate_error) => {
                ...
                {
                    let mut rollback = candidate_activation.clone();
...
mod tests {
    ...
    async fn recover(
        ...
        let publisher = Arc::new(
            crate::composition::daemon_bootstrap::extension_publisher::DaemonExtensionRuntimePublisher::new(

use crate::composition::daemon_bootstrap::approval_gate::bootstrap_workspace_trust_resolver;
use crate::composition::daemon_bootstrap::request_ports::corpus_security_ports;
...
async fn daemon_bootstrap_keeps_untrusted_repo_files_usable_without_loading_executable_config() {
    use application::workspace_trust::{
        ClientMode, ExecutableConfigSource, WorkspaceTrustDecision,
    };
    ...
    let workspace = ::contracts::WorkspacePolicy::from_resolved_roots(canonical.clone(), vec![])
    ...
    let decision = resolver
        .evaluate(
            ::contracts::PrincipalId("headless-daemon".into()),
    ...
    for source in ExecutableConfigSource::all() {
        if crate::host::workspace_trust::source_is_granted(&decision, source) {
...
async fn daemon_self_field_bootstrap_replays_before_new_transitions() {
    ...
    assert!(matches!(
        ...
        ::contracts::SubsystemHealth::Healthy
    ));

...
use crate::daemon::context_working_set::ContextWorkingSet;
use crate::daemon::DaemonConfig;
use crate::host::domain::{MemoryGroup, SecurityGroup, SessionGroup};
...
pub(super) struct AgentServices {
    ...
    pub agent_host: Arc<dyn crate::composition::agent_control::AgentHostEffects>,
    pub agent_recovery: crate::composition::agent_control::AgentRecoveryReport,
    ...
    pub agent_repository: Arc<dyn crate::composition::agent_control::AgentRunProjection>,
    ...
    pub agent_live_runs: Arc<crate::composition::agent_control::LiveAgentRuns>,
    pub capability_rollups: Arc<adapters_sqlite::SqliteCapabilityRollupProjectionSink>,
...
pub(super) struct RuntimeLauncherBinding {
    ...
    pub launcher: Arc<dyn crate::composition::agent_control::AgentRuntimeLauncher>,
...
pub(super) async fn build_agent_services(
    ...
    pi_backend: Option<Arc<crate::host::runtime::PiDelegateBackend>>,
    tools: Arc<Mutex<corpus::tools::tools::ToolRegistry>>,
    agent_profiles_for_tools: HashMap<String, ::contracts::AgentProfile>,
    agent_profile_catalog: Arc<crate::host::runtime::AgentProfileRegistry>,
    ...
    let capability_rollups = Arc::new(adapters_sqlite::SqliteCapabilityRollupProjectionSink::open(
    ...
    let agent_repository: Arc<dyn crate::composition::agent_control::AgentRunProjection> =
        Arc::new(runtime::RuntimeAgentRunProjection::new(
            sql_agent_projection,
            runtime_agent_supervisor.clone(),
        ));
    let agent_facades = crate::composition::agent_control::AgentHostAdapter::new_runtime_only(
        kernel.clone(),
        clock.clone(),
    ...
    let runtime_agent_backend = Arc::new(
        crate::composition::agent_control::RuntimeObservedAgentBackend::compatibility(&agent_host),
    );
    runtime_agent_supervisor.set_observed_backend(runtime_agent_backend.clone());
    let has_native_runtime = runtime_bindings
        .iter()
        .any(|binding| binding.id.0 == crate::host::runtime::NATIVE_COGNIT_RUNTIME_ID);
    for binding in runtime_bindings {
        ...
        let runtime_backend: Arc<dyn runtime::DelegateBackend> = Arc::new(
            crate::composition::agent_control::RuntimeObservedAgentBackend::pinned(
                &agent_host,
                binding.launcher,
            ),
        );
    ...
    let pi_backend_id =
        runtime::DelegateBackendId(adapters_agent_backend::PI_CODER_RUNTIME_ID.to_owned());
    if let Some(pi_backend) = pi_backend {
        ...
        runtime_agent_supervisor
            .register_backend_with_manifest(
                pi_backend_id,
                pi_backend,
                crate::host::runtime::pi_manifest().clone(),
            )
            .map_err(|error| anyhow::anyhow!(error.to_string()))?;
    }
    ...
    let agent_cleanup = crate::composition::agent_control::AgentCleanupCoordinator::new(
        agent_repository.clone(),
        Arc::new(
            crate::host::runtime::worktree_recovery::VerifiedAgentWorktreeReclaimer::default(),
        ),
    )
    ...
    let role_workflow_factory = if has_native_runtime {
        ...
        Some(Arc::new(
            agora::cognitive_role_workflow::RoleWorkflowFactory::new(
                ...
                ::contracts::RuntimeId(crate::host::runtime::NATIVE_COGNIT_RUNTIME_ID.into()),
                profiles,
                10 * 60 * 1_000,
            )?,
        ))
...
pub(super) struct TurnServices {
    ...
    pub session_gateway: Arc<dyn crate::daemon::handler::ports::SessionProjectionPort>,
    pub session_memory: Arc<dyn crate::daemon::handler::ports::SessionMemoryProjectionPort>,
    pub turn_orchestrator: Arc<crate::daemon::DaemonTurnOrchestrator>,
    pub approved_apply: Option<Arc<crate::adapters::approved_apply::ApplyCoordinator>>,
    pub lifecycle_registry: Arc<runtime::lifecycle_contributors::LifecycleRegistry>,
    pub evaluation_service: Arc<application::evaluation::EvaluationService>,
...
pub(super) async fn build_turn_services(
    ...
    model_router: Arc<crate::daemon::model_router::ModelRouter>,
    domains: &crate::host::domain::DomainServices,
    security_group: &SecurityGroup,
    memory_group: &MemoryGroup,
    memory_gateway: Arc<mnemosyne::MemoryGatewayService>,
    session_group: &SessionGroup,
    capability_resources: crate::daemon::handler::tool_executor::CapabilityResources,
    conscious_registry: Arc<crate::composition::conscious_workspace::ConsciousWorkspaceRegistry>,
    context_assembler: Arc<application::turn::context::ContextAssembler>,
    apply_objective_store: Arc<std::sync::Mutex<adapters_sqlite::goal::ObjectiveStore>>,
    agent_live_runs: Arc<crate::composition::agent_control::LiveAgentRuns>,
    capability_rollups: Arc<adapters_sqlite::SqliteCapabilityRollupProjectionSink>,
    role_workflow_factory: Option<Arc<agora::cognitive_role_workflow::RoleWorkflowFactory>>,
    canonical_event_spine: Arc<adapters_sqlite::event_spine::SqliteEventSpine>,
    event_projections: Arc<adapters_sqlite::projection_set::DefaultEventProjectionSet>,
    agent_profile_registry: Arc<crate::host::runtime::AgentProfileRegistry>,
    active_profile: Arc<Mutex<String>>,
    runtime: Arc<Mutex<crate::host::cognitive_runtime::AletheonCognitiveRuntime>>,
    ...
    let memory_evaluation_projection = crate::adapters::memory_projection::MemoryProjection::new(
        canonical_event_spine.clone(),
    ...
    > = vec![
        Arc::new(
            crate::adapters::goal_evaluation::GoalEvaluationProjectionSink::new(
                canonical_event_spine.clone(),
                apply_objective_store.clone(),
            ),
        ),
        Arc::new(
            crate::adapters::evaluation::AgentEvaluationProjectionSink::new(
                canonical_event_spine.clone(),
            ),
        ),
        Arc::new(
            crate::adapters::memory_projection::MemoryEvaluationProjectionSink::new(
    ...
    if let Some(dasein) = self_field.lock().await.dasein_handle() {
        evaluation_sinks.push(Arc::new(
            crate::composition::dasein_workspace::DaseinEvaluationProjectionSink::new(
                dasein,
                clock.clone(),
            ),
        ));
    ...
    let coordinator = Arc::new(
        application::turn::coordinator::TurnCoordinator::from_resources(
            application::turn::coordinator::TurnCoordinatorResources {
                clock: kernel.clock(),
                timer: Arc::new(crate::host::runtime::turn_operations::KernelTurnTimer::system()),
                operations: Arc::new(
                    crate::host::runtime::turn_operations::KernelTurnOperations::new(
                        kernel.clone(),
                    ),
                ),
                identities: Arc::new(
                    adapters_sqlite::session::turn_identity::CanonicalTurnIdentityAdapter,
                ),
                session: Arc::new(
                    adapters_sqlite::session::turn_session_port::SessionAppendTurnPort::new(
                        session_store.clone(),
                    ),
                ),
        ...
        .with_host_acceptance(Arc::new(
            crate::adapters::evaluation::AgoraEvaluationAcceptance::new(domains.agora()),
        )),
    );
    let workspace_checkpoint = Arc::new(
        crate::host::workspace_checkpoint::WorkspaceCheckpointService::new(
            Arc::new(adapters_sqlite::checkpoint::SqliteCheckpointStore::open(
                data_dir.join("workspace-checkpoints.sqlite"),
            )?),
            Arc::new(
                crate::host::workspace_checkpoint::KernelWorkspaceLeasePort::new(
        ...
        .with_safety_guard(Arc::new(
            crate::host::workspace_checkpoint::LiveAgentRewindGuard(agent_live_runs),
        ))
        .with_events(event_bus.clone(), Some(canonical_event_spine.clone())),
    );
    let session_service = Arc::new(
        runtime::session_service::SessionService::with_protocol_store(
            session_store,
            coordinator.active_index(),
            Arc::new(
                adapters_sqlite::session::SqliteSessionProtocolEventStore::open(
    ...
    let session_gateway = Arc::new(
        crate::daemon::session_projection::CanonicalSessionProjection::new(session_service.clone()),
    );
    let session_memory = Arc::new(
        crate::daemon::session_projection::CanonicalSessionMemoryProjection::new(
            core_memory,
            recall_memory,
        ),
    );
    let projection: Arc<dyn application::turn::post_turn::PostTurnProjection> = Arc::new(
        crate::adapters::post_turn::ProductionPostTurnProjection::new(
            crate::adapters::post_turn::PostTurnProjectionResources {
                ...
                runtime: post_turn_runtime_port(
                    ...
                    clock.clone(),
                    Arc::new(
                        crate::composition::evolution_proposer::GovernedEvolutionProposer::new(
                            apply_objective_store.clone(),
                            memory_group.approval_repository.clone(),
                            capability_rollups.clone(),
                            clock.clone(),
                            Arc::new(
                                adapters_sqlite::evolution::SqliteEvolutionProposalStore::open(
    ...
    let runtime_ports =
        super::turn_runtime::compose_turn_runtime(super::turn_runtime::TurnRuntimeResources {
            corpus: domains.corpus(),
    ...
    let active_profile_port: Arc<dyn application::turn::ports::ActiveAgentProfilePort> =
        Arc::new(super::turn_runtime::ProductionActiveAgentProfile::new(
            active_profile.clone(),
            agent_profile_registry.clone(),
        ));
    let pipeline = Arc::new(crate::host::turn_pipeline::TurnPipeline::new(
        crate::host::turn_pipeline::TurnPipelineResources {
            notification: Some(Arc::new(
                crate::daemon::turn_engine::SharedMpscTurnNotificationPort(
            ...
            canonical_sessions: session_service.clone(),
            projection,
            hooks: runtime_ports.hooks,
            storm: runtime_ports.storm,
            models: runtime_ports.models,
            self_policy: runtime_ports.self_policy,
            approvals: runtime_ports.approvals,
            capabilities: runtime_ports.capabilities,
            sessions: runtime_ports.sessions,
            config: runtime_ports.config,
            observability: runtime_ports.observability,
    ...
    let turn_orchestrator = Arc::new(crate::daemon::DaemonTurnOrchestrator::compose(
        kernel.clone(),
        shared_notify_tx.clone(),
    ...
    let approved_apply = if pi_runtime.enabled && pi_work_allowed {
        Some(Arc::new(
            crate::adapters::approved_apply::ApplyCoordinator::new(
                apply_objective_store,
                memory_group.approval_repository.clone(),
                kernel.clone(),
                clock.clone(),
                crate::adapters::approved_apply::ApplyCoordinatorConfig {
                    worktree_base: pi_runtime.worktree_base.clone(),
                    timeout: std::time::Duration::from_secs(60),
                },
                Arc::new(platform::worktree::GitManagedWorktreeCleaner),
                Arc::new(platform::temporary_artifact::HostTemporaryArtifactStore),
            )?
            .with_memory_projection(
                crate::adapters::memory_projection::MemoryProjection::new(
                    canonical_event_spine.clone(),
                    event_projections.clone(),
                ),
            ),

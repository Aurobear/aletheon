...
use crate::host::admin_service::{AdminRuntimePort, ModeChange};
use crate::host::cognitive_runtime::AletheonCognitiveRuntime;
use crate::host::request_use_cases::{
    CareWeight, CognitiveRuntimePort, ReflectionMemoryPort, ReflectionStats, RetentionAdminPort,
    RuntimeStatus, SelfStatus, SelfStatusPort, SupplementalMemoryStatus,
    SupplementalMemoryStatusPort,
};
use application::turn::ports::TurnConfigPort;
use application::turn::post_turn::{PostTurnOutcome, PostTurnRuntimePort};
...
pub(super) fn post_turn_runtime_port(
    ...
    clock: Arc<dyn ::contracts::Clock>,
    proposer: Arc<crate::composition::evolution_proposer::GovernedEvolutionProposer>,
...
impl PostTurnRuntimePort for PostTurnDomainAdapter {
    async fn post_evolution(&self, outcome: &PostTurnOutcome) -> anyhow::Result<()> {
        let summary = self
            ...
            .await
            .post_evolution(
                &application::turn::post_turn::bounded_summary(&outcome.input, 100),
                &outcome.output,
...
fn evidence_proposal_has_priority(
    summary: Option<&crate::host::evolution_coordinator::EvolutionSummary>,
) -> bool {
...
impl TurnConfigPort for TurnConfigAdapter {
    async fn config(&self) -> application::turn::settings::TurnRuntimeSettings {
        let runtime = self.config_source.lock().await;
        let config = runtime.config();
        application::turn::settings::TurnRuntimeSettings {
            ...
            automatic_multi_agent_for_coding: config.multi_agent.automatic_for_coding,
...
mod tests {
    ...
    use crate::host::evolution_coordinator::EvolutionSummary;

...
use crate::host::domain::MemoryGroup;
...
pub(super) fn compose_gateway(
    ...
    config: &MemoryConfig,
    read_only_destinations: impl IntoIterator<Item = String>,
    ...
    if let Some(manager) = mcp {
        let supplemental_router = Arc::new(
            adapters_gbrain::McpSupplementalBindingNegotiator::new(
                manager,
            ...
            .with_read_only_destinations(read_only_destinations),
        );
        gateway = gateway

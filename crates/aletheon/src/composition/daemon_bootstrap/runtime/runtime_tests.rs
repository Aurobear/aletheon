...
mod goal_runtime_tests {
    ...
    impl InferencePort for NoopInference {
        async fn capabilities(
            ...
            cognit::ports::inference::ModelCapabilities,
            cognit::ports::inference::InferenceError,
        > {
            Ok(cognit::ports::inference::ModelCapabilities {
                provider_id: None,
                transport: None,
        ...
        async fn complete(
            &self,
            _request: cognit::ports::inference::CoreInferenceRequest,
        ) -> Result<::contracts::LlmResponse, cognit::ports::inference::InferenceError> {
        ...
        async fn stream(
            &self,
            _request: cognit::ports::inference::CoreInferenceRequest,
        ) -> Result<::contracts::LlmStream, cognit::ports::inference::InferenceError> {
            Err(anyhow::anyhow!("unused test inference").into())
        }
    }
    ...
    impl CapabilityService for NoopCapability {
        async fn invoke(
            &self,
            _context: Option<kernel::capability::governed::CapabilityExecutionContext>,
            call: ::contracts::CapabilityCall,
    ...
    async fn disabled_goal_runtime_registers_nothing() {
        ...
        let registrations = register(GoalRuntimeConfig::default(), app).await.unwrap();
    ...
    async fn same_provider_can_back_distinct_runtime_ids() {
        ...
        let registrations = register(config, app).await.unwrap();
        assert_eq!(registrations.len(), 2);
        assert!(registrations
            .iter()
            .any(|(id, _)| id.0 == "deepseek-worker"));
        assert!(registrations
            .iter()
            .any(|(id, _)| id.0 == "escalation-reviewer"));
    }
    ...
    async fn distinct_providers_register_worker_and_reviewer() {
        ...
        assert_eq!(
            ids,
            vec![
                ::contracts::RuntimeId("deepseek-worker".into()),
                ::contracts::RuntimeId("escalation-reviewer".into())
            ]
        );
        assert_eq!(registrations.len(), 2);
    }
    ...
    async fn agent_profiles_resolve_model_tools_and_frontmatter_limits() {
        ...
        let result = super::load_agent_profiles(
            ...
            &definitions,
            &definitions,
            &crate::config::CognitiveRuntimeConfig::default(),
    ...
    async fn wildcard_profile_expands_runtime_catalog_and_exposes_deferred_tools() {
        ...
        let visible = vec![::contracts::ToolDefinition {
            name: "file_read".into(),
            description: "read".into(),
            input_schema: serde_json::json!({"type":"object"}),
        }];
        let mut catalog = visible.clone();
        catalog.push(::contracts::ToolDefinition {
    ...
    async fn agent_profile_token_overrides_feed_adaptive_context_limits() {
        ...
        let definitions = vec![::contracts::ToolDefinition {
            name: "file_read".into(),
            description: "read".into(),
            input_schema: serde_json::json!({"type":"object"}),
        }];
        let mut profiles = crate::config::AgentProfilesConfig::default();
        profiles.overrides.insert(
            "compact".into(),
            crate::config::ProfileOverride {
    ...
    async fn agent_profile_config_rejects_unknown_default_and_override() {
        ...
        let mut unknown_default = crate::config::AgentProfilesConfig {
    ...
    async fn agent_tool_registration_profile_can_reference_agent_spawn() {
        ...
        let mut definitions: Vec<::contracts::ToolDefinition> = vec![::contracts::ToolDefinition {
            name: "file_read".into(),
            description: "read".into(),
            input_schema: serde_json::json!({"type": "object"}),
        }];
    ...
    async fn agent_tool_registration_unknown_tool_rejected() {
        ...
        let definitions: Vec<::contracts::ToolDefinition> = vec![::contracts::ToolDefinition {
            name: "file_read".into(),
            description: "read".into(),
            input_schema: serde_json::json!({"type": "object"}),
        }];

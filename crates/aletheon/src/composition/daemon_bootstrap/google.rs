...
use adapters_google::gmail::{load_gmail_ingress_policies, GmailGoalEventIngress};

pub(super) type ConfiguredGoogleReadTools = (
    Arc<GoogleIntegration>,
    adapters_google::GoogleSyncHandle,
    Arc<std::sync::Mutex<adapters_google::GoogleSyncStore>>,
...
pub(super) fn register_configured_external_read_tools(
    ...
    storage_quota: Option<platform::storage_quota::StorageQuota>,
    config: Option<&crate::config::ResolvedGoogleIntegration>,
    ...
    let gmail_enabled = repository
        .has_active_scope(application::ExternalCapabilityId::new("mail.read").unwrap())?;
    let calendar_enabled = repository
        .has_active_scope(application::ExternalCapabilityId::new("calendar.read").unwrap())?;
    let mut scopes = vec![
        application::ExternalCapabilityId::new("identity.openid").unwrap(),
        application::ExternalCapabilityId::new("identity.email.read").unwrap(),
        application::ExternalCapabilityId::new("mail.read").unwrap(),
        application::ExternalCapabilityId::new("calendar.read").unwrap(),
    ];
    if config.drive_sync_enabled {
        scopes.push(application::ExternalCapabilityId::new("file.read").unwrap());
    }
    ...
    let credentials = Arc::new(GoogleCredentialSourceAdapter::new(
        repository.clone(),
        oauth,
    ));
    let accounts = Arc::new(GoogleAccountResolverAdapter::new(repository));
    ...
    let store = Arc::new(std::sync::Mutex::new(
        adapters_google::GoogleSyncStore::open(objective_db_path)?,
    ));
    let mut manager = adapters_google::GoogleSyncManager::new(
        store.clone(),
        format!("daemon-{}", uuid::Uuid::new_v4()),
        clock.clone(),
        adapters_google::GoogleSyncManagerConfig::default(),
    )?;
    ...
    for (identity, grant) in active_bindings {
        if grant
            .scopes
            .contains(&application::ExternalCapabilityId::new("mail.read").unwrap())
        {
            manager.register(adapters_google::GoogleSyncRegistration {
                principal: identity.principal_id.clone(),
                account_id: identity.id,
                stream: adapters_google::SyncStream::GmailHistory,
                initial_cursor: None,
                cursor_generation: 1,
                poller: Arc::new(adapters_google::GmailHistoryPoller(
        ...
        if grant
            .scopes
            .contains(&application::ExternalCapabilityId::new("calendar.read").unwrap())
        {
            manager.register(adapters_google::GoogleSyncRegistration {
                principal: identity.principal_id.clone(),
                account_id: identity.id,
                stream: adapters_google::SyncStream::Calendar,
                initial_cursor: None,
                cursor_generation: 1,
                poller: Arc::new(adapters_google::CalendarDeltaPoller(
        ...
        if drive_enabled
            && grant
                .scopes
                .contains(&application::ExternalCapabilityId::new("file.read").unwrap())
        {
            manager.register(adapters_google::GoogleSyncRegistration {
                principal: identity.principal_id,
                account_id: identity.id,
                stream: adapters_google::SyncStream::DriveChanges,
                initial_cursor: None,
                cursor_generation: 1,
                poller: Arc::new(adapters_google::DriveChangesPoller(DriveSynchronizer::new(
...
/// Host admin glue exposing the Google sync manager as a background worker.
/// Stays in the host composition because it implements the host
/// `admin_service::BackgroundWorkerPort`; the sync infrastructure lives in the
/// `adapters-google` crate.
pub struct GoogleSyncWorkerPort {
    handle: Mutex<Option<adapters_google::GoogleSyncHandle>>,
}

impl GoogleSyncWorkerPort {
    pub fn new(handle: adapters_google::GoogleSyncHandle) -> Self {
        Self {
...
impl crate::host::admin_service::BackgroundWorkerPort for GoogleSyncWorkerPort {

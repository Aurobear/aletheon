...
use crate::composition::RuntimeIdentityBinder;
use crate::host::exec_session::ExecSessionBuilder;
use ::contracts::{
...
enum ExecExecutionResult {
    Completed(
        ...
        Arc<crate::host::exec_session::ExecSessionFacts>,
    ),
    Failed(anyhow::Error),
    DeadlineExceeded,
    Interrupted,
...
pub async fn run_exec_streaming(
    ...
    let mut builder = ExecSessionBuilder::new(working_dir.clone())
        ...
        .with_inference(Arc::new(adapters_inference::CoreRpcClient::new(

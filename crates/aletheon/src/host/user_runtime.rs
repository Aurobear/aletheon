...
use ::contracts::paths::UserRuntimePaths;
use anyhow::Context;
...
use crate::daemon::server::{process_inherited_listener, UnixServer};
...
impl UserRuntimeConfig {
    pub fn load(
        ...
        let loaded = crate::config::load_for_host(None, config_path)?;
    ...
    fn from_loaded(
        ...
        let integrations = loaded
            .preflight_integrations(&crate::config::EnvironmentCredentialResolver)
        ...
        let request = DaemonConfig {
            ...
            conscious_arbitration_mode: crate::daemon::parse_conscious_arbitration_mode(
            ...
            robot,
            // RA-03 Session writer mode from `[bootstrap] session_writer`.
            session_writer: app.bootstrap.session_writer,
        };
        Ok(Self {
            request,
            paths,
    ...
    pub fn fixture_at(root: impl Into<PathBuf>) -> Self {
        ...
        let loaded = crate::config::merge_layers(std::iter::empty())
...
fn apply_execd_override(config: &mut crate::config::GrokHardeningConfig, cli_enabled: bool) {
    config.execd |= cli_enabled;
}

pub struct UserRuntime {
    request_handler: RequestHandler,
    server: Option<UnixServer<crate::daemon::handler::RequestHandler>>,
    paths: UserRuntimePaths,
...
impl UserRuntime {
    pub async fn bootstrap(
        ...
        let clock: Arc<dyn ::contracts::Clock> = Arc::new(SystemClock::new());

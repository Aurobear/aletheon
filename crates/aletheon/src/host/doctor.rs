...
impl DoctorReport {
    ...
    pub fn standalone(loaded_config: &LoadedConfig) -> Self {
        ...
        let manifest_path = std::env::var_os("ALETHEON_DEPLOYMENT_MANIFEST")
            ...
            .unwrap_or_else(|| expand_home_path(&data_dir).join("deployment-manifest.json"));
...
fn expand_home_path(path: &std::path::Path) -> std::path::PathBuf {
    let Some(value) = path.to_str() else {
        return path.to_owned();
    };
    if value == "~" {
        return dirs::home_dir().unwrap_or_else(|| path.to_owned());
    }
    value
        .strip_prefix("~/")

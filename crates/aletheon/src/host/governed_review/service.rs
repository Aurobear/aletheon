...
use cognit::ports::inference::{CoreInferenceRequest, InferencePort};
...
pub use crate::config::GovernedReviewLimits;
...
fn redacted_error(error: &str) -> String {
    let governed = application::data_governance::scrub_for_projection(
        error,
        application::data_governance::ContentTrust::ExternalUntrusted,
    );
    let max = application::governed_review::MAX_REVIEW_ERROR_BYTES;

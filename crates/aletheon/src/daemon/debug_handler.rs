...
/// Narrow consumer port for the daemon transport / connection dispatcher.
/// `DebugHandler` is owned by the composition root; the transport and the
/// request handler depend only on this port, not on the concrete handler.
#[async_trait::async_trait]
pub trait DebugHandlerPort: Send + Sync {
    async fn handle_method(&self, method: &str, id: &Value, params: &Value) -> Option<Value>;
    async fn take_pending_subscriber_rx(&self) -> Option<mpsc::Receiver<DebugEvent>>;
}
...
#[async_trait::async_trait]
impl DebugHandlerPort for DebugHandler {
    async fn handle_method(&self, method: &str, id: &Value, params: &Value) -> Option<Value> {
        self.handle_method(method, id, params).await
    }
    async fn take_pending_subscriber_rx(&self) -> Option<mpsc::Receiver<DebugEvent>> {
        self.take_pending_subscriber_rx().await
    }
}
...
impl DebugHandler {
    ...
    async fn handle_perf(&self, id: &Value) -> Value {
        ...
        let prefix_cache = runtime::prefix_cache_observability::prefix_shape_metrics();
        let tool_calls = {
            ...
            "result": {
                "graph": {
                    "nodes": [
                        ...
                        {"id": "harness",     "label": "HarnessCognitiveSession", "type": "core"},
                        ...
                        {"id": "session_mgr", "label": "ContextWorkingSet", "type": "core"},
                    ...
                    "edges": [
                        ...
                        {"from": "handler",    "to": "harness",     "label": "chat"},
                        ...
                        {"from": "harness",    "to": "llm",         "label": "completion"},
                        {"from": "harness",    "to": "tools",       "label": "tool_call"},
                        {"from": "harness",    "to": "event_sink",  "label": "events"},
    ...
    harness [label="HarnessCognitiveSession"];
    ...
    session_mgr [label="ContextWorkingSet"];
    ...
    harness -> llm [label="completion"];
    harness -> tools [label="tool_call"];
    harness -> event_sink [label="events"];

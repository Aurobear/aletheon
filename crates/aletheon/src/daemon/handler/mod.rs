...
pub struct RequestHandler {
    ...
    pub(crate) thread_authority: Arc<application::thread_authority::ThreadAuthorityStore>,
    ...
    pub(crate) workspace_trust: Arc<crate::host::workspace_trust::WorkspaceTrustResolver>,
...
impl CommandUseCases for DaemonCommandUseCases {
    async fn submit_prompt(
        &self,
        intent: &::contracts::contract::command::ClientIntent,
        prompt: &::contracts::contract::command::SubmitPromptIntent,
    ) -> anyhow::Result<CommandOutput> {
        let thread_id = match &prompt.session_id {
            Some(session_id) => ::contracts::ThreadId(session_id.0.clone()),
            None => {
                self.handler
        ...
        if let Some((code, message)) = rpc_error_parts(&response) {
            return Ok(CommandOutput::new(
                ...
                ::contracts::contract::command::CommandOutputV1::Rejected(
                    ::contracts::contract::command::CommandRejectionV1 { code, message },
                ),
            ));
        }
        Ok(CommandOutput::new(
            ...
            ::contracts::contract::command::CommandOutputV1::PromptCompleted(
                application::command_dispatcher::prompt_completion_from_rpc_result(
    ...
    async fn status(
        &self,
        intent: &::contracts::contract::command::ClientIntent,
        status: &::contracts::contract::command::StatusIntent,
    ) -> anyhow::Result<CommandOutput> {
        let Some(session_id) = status.session_id.as_ref() else {
            return Ok(CommandOutput::new(
                intent.correlation_id.clone(),
                ::contracts::contract::command::CommandOutputV1::Rejected(
                    ::contracts::contract::command::CommandRejectionV1 {
                        code: -32602,
        ...
        match self.handler.status_projection(&session_id.0).await {
            Ok(projection) => Ok(CommandOutput::new(
                intent.correlation_id.clone(),
                ::contracts::contract::command::CommandOutputV1::StatusProjected(projection),
            )),
...
impl RequestHandler {
    ...
    pub(crate) async fn resolve_versioned_approval(
        ...
        let decision = match request.decision {
            ::contracts::protocol::client::ApprovalDecisionRequest::Approve => {
                application::approval::ApprovalDecision::Approve
            }
            ::contracts::protocol::client::ApprovalDecisionRequest::Reject => {
                application::approval::ApprovalDecision::Reject {
        ...
        self.ports
            .approvals
            .resolve(application::approval::ResolveApprovalRequest {
                context: application::approval::ApprovalContext {
    ...
    async fn handle_workspace_trust_evaluate(
        ...
        let decision = self
            ...
            .evaluate(
                connection.principal_id.clone(),
                crate::host::workspace_trust::workspace_identity(workspace.cwd()),
                application::workspace_trust::ClientMode::Interactive,
                crate::host::workspace_trust::is_broad_unrecordable_root(workspace.cwd()),
                unix_now(),
            )
            .await;
        let result = match decision {
            application::workspace_trust::WorkspaceTrustDecision::Trusted { granted } => {
                serde_json::json!({"decision":"trusted","sources":granted})
            }
            application::workspace_trust::WorkspaceTrustDecision::Restricted { blocked } => {
                serde_json::json!({"decision":"restricted","sources":blocked})
            }
            application::workspace_trust::WorkspaceTrustDecision::PromptRequired { findings } => {
    ...
    async fn handle_workspace_trust_grant(
        ...
        let granted = match serde_json::from_value::<
            Vec<application::workspace_trust::ExecutableConfigSource>,
    ...
    pub async fn handle(
        ...
        if matches!(
            ...
            let session_id = ::contracts::SessionId(
                params
                    .get("session_id")
                    .and_then(|v| v.as_str())
                    .unwrap_or_default()
    ...
    async fn handle_client_intent(
        ...
        let intent =
            match serde_json::from_value::<::contracts::contract::command::ClientIntent>(params) {
                Ok(intent) => intent,
                Err(error) => {
                    return rpc_error(&id, -32602, format!("invalid ClientIntent: {error}"))
                }
            };
    ...
    pub(super) async fn handle_legacy_status(
        ...
        let session_id = request["params"]
            ...
            .map(|value| ::contracts::SessionId(value.to_owned()));
        let intent = ::contracts::contract::command::ClientIntent::v1(
            ::contracts::contract::command::ClientSurface::Tui,
            connection.principal_id.clone(),
            format!("legacy-status:{}", rpc_id_fragment(&id)),
            ::contracts::contract::command::ClientCommand::Status(
                ::contracts::contract::command::StatusIntent { session_id },
            ),
        );
        ...
        match dispatcher.dispatch(intent).await {
            Ok(output) => match output.output {
                ::contracts::contract::command::CommandOutputV1::StatusProjected(status) => {
    ...
    pub(super) async fn handle_chat(
        ...
        let intent = ::contracts::contract::command::ClientIntent::v1(
            ...
            ::contracts::contract::command::ClientCommand::SubmitPrompt(
                ::contracts::contract::command::SubmitPromptIntent {
                    content: message.to_owned(),
                    session_id: Some(::contracts::SessionId(thread_id.0)),
                    workspace,
                    requirements,
                    task_kind,
                    permission_mode,
    ...
    pub(crate) async fn execute_explicit_chat(
        ...
        let mut context = ::contracts::PrincipalContext::new(
            connection.principal_id.clone(),
        ...
        let trust_decision = self
            ...
            .evaluate(
                ...
                crate::host::workspace_trust::workspace_identity(context.workspace.cwd()),
                application::workspace_trust::ClientMode::Headless,
                crate::host::workspace_trust::is_broad_unrecordable_root(context.workspace.cwd()),
            ...
            .await;
        context.repo_hooks_trusted = crate::host::workspace_trust::source_is_granted(
            &trust_decision,
            application::workspace_trust::ExecutableConfigSource::RepoHooks,
        );
        tracing::debug!(
    ...
    /// Typed Gateway admission boundary. It performs the same authenticated
    /// context/trust binding as the synchronous compatibility facade, then
    /// returns only after Runtime has minted the canonical Turn identity.
    /// Execution and terminal settlement continue in the supervised turn
    /// task; callers must use projection/events for completion.
    pub(crate) async fn submit_explicit_chat(
        &self,
        connection: &super::server::ConnectionContext,
        id: serde_json::Value,
        message: String,
        ...
        anyhow::ensure!(
            !thread_id.0.trim().is_empty() && !message.trim().is_empty(),
            "thread_id and message are required"
        );
        let mut context = ::contracts::PrincipalContext::new(
            connection.principal_id.clone(),
            connection.os_principal,
        ...
        self.bind_thread_authority(&context, None)
            .map_err(|error| anyhow::anyhow!(error.to_string()))?;
        let trust_decision = self
            .workspace_trust
            .evaluate(
    ...
    fn bind_thread_authority(
        &self,
        context: &::contracts::PrincipalContext,
        model_policy: Option<String>,
    ) -> Result<(), application::thread_authority::ThreadAuthorityError> {
        use application::thread_authority::{ThreadAuthorityKey, ThreadSettings};
...
fn parse_turn_requirements(
...
) -> Result<Vec<::contracts::TurnRequirement>, String> {
    if value.is_null() {
        return Ok(Vec::new());
    }
    let requirements: Vec<::contracts::TurnRequirement> = serde_json::from_value(value.clone())
    ...
    for requirement in &requirements {
        let value = match requirement {
            ::contracts::TurnRequirement::InvokeAgentRuntime { runtime_id } => runtime_id,
            ::contracts::TurnRequirement::InvokeCapability { name } => name,
            ::contracts::TurnRequirement::ObserveTerminal { .. } => continue,
...
fn parse_task_kind(value: &serde_json::Value) -> Result<Option<::contracts::TaskKind>, String> {
    if value.is_null() {
        return Ok(None);
    }
...
impl LegacySessionThreadAdapter {
    fn thread_id(session_id: String) -> ::contracts::ThreadId {
...
fn resolve_requested_workspace(
    params: &serde_json::Value,
) -> Result<::contracts::WorkspacePolicy, String> {
    resolve_requested_workspace_with_mode(params, ::contracts::permission::HostPermissionMode::Safe)
}
...
fn resolve_requested_workspace_with_mode(
    ...
    ::contracts::WorkspaceSelection::new(
...
fn parse_host_permission_mode(
    value: &serde_json::Value,
) -> ::contracts::permission::HostPermissionMode {
    match value.as_str() {
        Some("full" | "unrestricted") => ::contracts::permission::HostPermissionMode::Full,
        Some("developer" | "dev") => ::contracts::permission::HostPermissionMode::Developer,
        _ => ::contracts::permission::HostPermissionMode::Safe,
    }
}
...
mod working_dir_tests {
    ...
    fn accepts_typed_agent_runtime_requirement() {
        ...
        assert_eq!(
            requirements,
            vec![::contracts::TurnRequirement::InvokeAgentRuntime {
    ...
    fn permission_mode_parser_is_fail_closed() {
        ...
        assert_eq!(
            ...
            ::contracts::permission::HostPermissionMode::Developer
        );
        assert_eq!(
...
#[async_trait::async_trait]
impl crate::host::unix_server::ConnectionDispatcher for RequestHandler {
    async fn handle(
        &self,
        connection: &crate::host::unix_server::ConnectionContext,
        request: serde_json::Value,
    ) -> serde_json::Value {
        RequestHandler::handle(self, connection, request).await
    }

    async fn set_notify_channel(&mut self, tx: mpsc::Sender<String>) {
        RequestHandler::set_notify_channel(self, tx).await
    }

    fn try_increment_connections(&self) -> bool {
        RequestHandler::try_increment_connections(self)
    }

    fn decrement_connections(&self) {
        RequestHandler::decrement_connections(self)
    }
    ...
    fn notify_tx(&self) -> Option<mpsc::Sender<String>> {
        self.notify_tx.clone()
    }
    ...
    async fn protocol_session_list_for(
        ...
        RequestHandler::protocol_session_list_for(self, principal).await
    }
    ...
    async fn protocol_event_page_for(
        ...
        RequestHandler::protocol_event_page_for(self, principal, session_id, cursor).await
    }
    ...
    async fn protocol_snapshot_for(
        ...
        RequestHandler::protocol_snapshot_for(self, principal, session_id).await
    }
    ...
    async fn protocol_events_after_for(
        ...
        RequestHandler::protocol_events_after_for(self, principal, session_id, cursor).await
    }
    ...
    async fn handle_typed(
        ...
        let app = crate::daemon::handler::typed_gateway::DaemonTypedApplication::from_parts(
            self.ports.clone(),
            std::sync::Arc::new(self.clone()),
            self.thread_authority.clone(),
            self.grok_hardening.clone(),
            connection.clone(),
        );
        let typed =
            gateway::server::handlers::typed::TypedRouteHandler::new(std::sync::Arc::new(app));
        typed.dispatch_wire(request).await
    }
    ...
    async fn execute_explicit_chat(
        ...
        RequestHandler::execute_explicit_chat(
            self,
            connection,
            id,
            message,
    ...
    async fn resolve_versioned_approval(
        ...
        RequestHandler::resolve_versioned_approval(self, connection, request).await
    }
    ...
    async fn cancel_versioned_turn(
        ...
        RequestHandler::cancel_versioned_turn(self, connection, request).await
    }
    ...
    async fn memory_observe(
        ...
        RequestHandler::memory_observe(self, connection, request).await
    }
    ...
    async fn memory_receipt(
        ...
        RequestHandler::memory_receipt(self, connection, request).await
    }
    ...
    async fn memory_recall(
        ...
        RequestHandler::memory_recall(self, connection, request).await
    }
    ...
    async fn memory_feedback(
        ...
        RequestHandler::memory_feedback(self, connection, request).await
    }
    ...
    async fn memory_workspace_preview_bind(
        ...
        RequestHandler::memory_workspace_preview_bind(self, connection, request).await
    }
    ...
    async fn memory_workspace_bind(
        ...
        RequestHandler::memory_workspace_bind(self, connection, request).await
    }
    ...
    async fn memory_workspace_unbind(
        ...
        RequestHandler::memory_workspace_unbind(self, connection, request).await
    }
    ...
    async fn memory_maintenance_status(
        ...
        RequestHandler::memory_maintenance_status(self, request).await
    }
    ...
    {
        RequestHandler::memory_maintenance_run(self, connection, request).await
    }
    ...
    async fn cleanup_disconnected_connection(
        ...
        RequestHandler::cleanup_disconnected_connection(self, connection_id).await
    }

    fn debug_handler(&self) -> std::sync::Arc<dyn crate::daemon::debug_handler::DebugHandlerPort> {
        RequestHandler::debug_handler(self)
    }
}

...
use crate::daemon::server::ConnectionContext;
use application::approval::ApprovalDecision;
use application::approval::{ApprovalContext, ResolveApprovalRequest};
use std::path::Path;
use std::sync::Arc;
...
impl TypedApplicationPort for DaemonTypedApplication {
    ...
    async fn submit_approval(
        ...
        match pending {
            Ok(_scope_subject) => {
                ...
                self.ports
                    ...
                    .resolve_transient_approval(
                        crate::host::admin_service::TransientApprovalRequest {
            ...
            Err(crate::host::admin_service::PendingApprovalError::WrongOwner) => {
                Err(ProtocolError::Server(
                    "approval is not owned by this authenticated connection".into(),
                ))
            }
            Err(crate::host::admin_service::PendingApprovalError::NotFound) => {

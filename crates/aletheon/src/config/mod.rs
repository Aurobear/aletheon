//! Aletheon-owned application configuration.
//!
//! Domain crates define typed sub-configuration values. This crate discovers,
//! merges, validates, and reports application layers; composition roots consume
//! its typed result without routing through Aletheon.
...
pub struct ProfileOverride {
    ...
    pub approval_policy: Option<::contracts::AgentApprovalPolicy>,
    /// Runtime capabilities this role needs in addition to capabilities
    /// explicitly requested by an `agent_spawn` caller.
    pub runtime_capabilities: Vec<::contracts::AgentRuntimeCapability>,
}
...
pub struct AppConfig {
    ...
    #[serde(default)]
    pub sandbox_profiles: ::contracts::SandboxProfiles,
    /// Host-owned outbound network authority for built-in tools.
    #[serde(default)]
    pub network_policy: corpus::security::network_policy::NetworkPolicy,
    #[serde(default)]
    ...
    for mut layer in layers {
        ...
        let project_profiles = if layer.source.kind == ConfigSourceKind::Project {
            layer
                ...
                .cloned()
                .map(|value| value.try_into::<::contracts::SandboxProfiles>())
                .transpose()
        ...
        let lower_profiles = project_profiles.as_ref().map(|_| {
            merged
                ...
                .try_into::<::contracts::SandboxProfiles>()

...
impl ConsciousProcessor for MnemosyneProcessor {
    ...
    async fn on_broadcast(
        ...
        for selected in &broadcast.selected {
            if let ::contracts::WorkspaceContent::Observation(observation) = &selected.content {
                ...
                let observed = ::contracts::wall_to_datetime(self.adapter.clock.wall_now());
        ...
        let result = self
            ...
            .recall(mnemosyne::RecallRequest {
                ...
                current_at: Some(::contracts::wall_to_datetime(self.adapter.clock.wall_now())),

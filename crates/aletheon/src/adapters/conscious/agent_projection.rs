...
use agora::conscious_core_ports::CandidateSubmission;
use application::agent::AgentCandidateProjectionContext;
use runtime::{AgentRuntimeEvent, AgentRuntimeEventSink as AgentEventSink};

pub use runtime::AgentCandidateSubmissionPort;
...
pub struct AgentCandidateProjector {
    input: AgentCandidateProjectionContext,
    clock: Arc<dyn Clock>,
}

impl AgentCandidateProjector {
    pub fn new(input: AgentCandidateProjectionContext, clock: Arc<dyn Clock>) -> Self {
        Self { input, clock }
    }
    ...
    fn submission(
        ...
        let candidate = WorkspaceCandidate {
            ...
            provenance: WorkspaceProvenance {
                ...
                source_refs: std::iter::once(event_ref.clone())
                    ...
                    .chain(self.input.broadcast_refs.iter().map(|reference| {
        ...
        Ok(CandidateSubmission {
            candidate,
            cause: agora::conscious_core_ports::CandidateCause::AgentRuntimeEvent {
...
fn event_ids(event: &AgentRuntimeEvent) -> (AgentId, ProcessId, ::contracts::OperationId) {
    match event {

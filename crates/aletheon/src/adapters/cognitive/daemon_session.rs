...
use application::turn::settings::TurnRuntimeSettings;
...
pub struct DaemonStreamingTurnContext<F> {
    pub config: TurnRuntimeSettings,
    ...
    O: Into<cognit::harness::event_sink::ToolResultEvent> + Send + 'static,
{
    ...
    let harness_config = cognit::harness::HarnessConfig {
        ...
        reflection_tool_call_limit: config.reflection_tool_call_limit,
        circuit_breaker_max_repeats: config.circuit_breaker_max_repeats,
        circuit_breaker_window_size: config.circuit_breaker_window_size,
...
pub async fn execute_cognitive_turn<F, Fut, O>(
    role_graph: Option<super::role_graph::PreparedRoleGraph>,
    request: TurnRequest,
    context: DaemonStreamingTurnContext<F>,
) -> anyhow::Result<TurnResult>
where
    ...
    O: Into<cognit::harness::event_sink::ToolResultEvent> + Send + 'static,
{
    ...
    submit_streaming_daemon_turn(request, context).await
}

pub fn operation_exit_reason(
    result: &anyhow::Result<TurnResult>,
    cancelled: bool,
) -> contracts::OperationExitReason {
    match result {
        ...
        Err(error) if cancelled => contracts::OperationExitReason::Cancelled(
            contracts::CancelReason::Other(error.to_string()),
        ),

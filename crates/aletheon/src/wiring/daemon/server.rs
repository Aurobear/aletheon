...
use gateway::protocol::{
    ApprovalRequestedEvent, Event as TypedGatewayEvent, ProtocolError, RuntimeProgressEvent,
    SessionRef, TurnRef, WireRequest, WireResponse, WireResponseBody, PROTOCOL_VERSION,
};
use gateway::server::handlers::typed::TypedRouteHandler;
...
fn typed_gateway_notification(message: &str) -> Option<TypedGatewayEvent> {
            ...
            tokio::select! {
                ...
                read_result = reader.read_line(&mut line) => {
                    ...
                    if looks_typed {
                        ...
                        let app = crate::wiring::daemon::handler::typed_gateway::DaemonTypedApplication::from_parts(
                            handler.ports.clone(),
                            std::sync::Arc::new(handler.clone()),
                            handler.thread_authority.clone(),
                            handler.grok_hardening.clone(),
                            connection.clone(),
                        );
                        let typed = TypedRouteHandler::new(std::sync::Arc::new(app));
                        let resp_tx = resp_tx.clone();
                        request_tasks.spawn(async move {
                            let response = typed.dispatch_wire(typed_request).await;
                ...
                notification = notify_rx.recv() => {
                    match notification {
                        Some(msg) => {
                            if typed_gateway_started {
                                if let Some(event) = typed_gateway_notification(&msg) {
                                    let response = WireResponse {
                                        version: PROTOCOL_VERSION,
...
mod tests {
    ...
    fn typed_gateway_translates_connection_owned_approval_request() {
        ...
        let event = typed_gateway_notification(&message)
            .expect("approval notification should become a typed event");
        let TypedGatewayEvent::ApprovalRequested(approval) = event else {

...
use ::contracts::types::admission::{ExecutionPermit, UsageReport};
...
use ::contracts::{
    ...
    Context as AbiContext, ExitReason, NamespaceId, OperationKind, OperationRequest, ProcessSignal,
    SpawnSpec,
};
...
use dasein::{Intent, IntentSource, SelfField, SelfFieldOps, Verdict};
use kernel::capability::ToolExecutor;
use kernel::debug_bus::PerfCounter;

use crate::wiring::application::admin_service::ScopedApprovalCache;
...
pub(crate) struct CapabilityResources {
    ...
    pub(crate) capabilities: Arc<RwLock<Vec<::contracts::CapabilityId>>>,
    ...
    pub(crate) extension_decisions:
        Arc<dyn crate::extensions::extension_service::ExtensionDecisionSink>,
}
...
impl TurnToolExecutor {
    ...
    async fn execute(
        ...
        sink: Option<&mut ::contracts::ToolEventSink>,
    ) -> CapabilityResult {
        ...
        // --- PostTool hook ---
        {
            let ctx = HookContext {
                ...
                tool_name: Some(name.clone()),
                tool_input: None,
                tool_result: Some(corpus::hook::HookToolResult {
...
/// Aletheon composition adapter for every non-turn capability caller.
...
pub(crate) struct PreparedCorpus {
    ...
    pub(crate) risk_by_tool: HashMap<String, ::contracts::types::admission::RiskLevel>,
...
impl CapabilityService for ProductionCapabilityService {
    async fn invoke(
        ...
        let context = CapabilityExecutionContext {
            ...
            principal: ::contracts::PrincipalId("external-capability".into()),
            connection_id: ::contracts::ConnectionId::new(),
            thread_id: ::contracts::ThreadId("external-capability".into()),
            // This is an out-of-turn capability invocation.  It must not mint
            // a canonical TurnId; the daemon only assigns turn identities at
            // Runtime admission.  Keep the compatibility context explicitly
            // non-canonical until the caller supplies a real turn context.
            turn_id: ::contracts::TurnId(uuid::Uuid::nil()),

...
use crate::wiring::daemon::legacy_session::LegacySessionSnapshot;
...
impl RequestHandler {
                ...
                let tokens_before = receipt.map_or_else(
                    || {
                        transition
                            ...
                            .messages
                            .iter()
                            .map(::contracts::Message::estimate_tokens)
                            .sum()
                    },
    ...
    async fn distill_session_facts(&self, snapshot: &LegacySessionSnapshot) {
        for message in snapshot.messages.iter().rev().take(10) {
            if !matches!(message.role, ::contracts::Role::User) {
                continue;
            }
            for block in &message.content {
                let ::contracts::ContentBlock::Text { text } = block else {
                    continue;
                };
    ...
    pub(super) async fn handle_session_switch(
        ...
        match self.ports.sessions.switch(target.to_string()).await {
            ...
            Err(crate::wiring::daemon::legacy_session::LegacySessionError::NotFound(_)) => {

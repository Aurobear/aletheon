...
) -> Result<(runtime::PromptId, u64), &'static str> {
    let prompt_id = params
        .get("prompt_id")
        .and_then(serde_json::Value::as_str)
        .and_then(|value| uuid::Uuid::parse_str(value).ok())
        .map(runtime::PromptId)
...
fn queue_op_response(id: &serde_json::Value, result: runtime::QueueOpResult) -> serde_json::Value {
    match result {
        runtime::QueueOpResult::Ok { new_version } => {
            json!({"jsonrpc":"2.0", "id":id, "result":{"status":"ok", "new_version":new_version}})
        }
        runtime::QueueOpResult::Conflict { current } => json!({
            "jsonrpc":"2.0", "id":id,
            "error":{"code":-32045, "message":"prompt version conflict", "data":{"current":current}}
        }),
        runtime::QueueOpResult::Rejected { reason } => json!({
            "jsonrpc":"2.0", "id":id,
...
impl RequestHandler {
    ...
    pub(super) async fn handle_workspace_rewind(
        ...
        let outcome = self
            ...
            .await;
        if outcome == application::workspace_checkpoint::RestoreOutcome::Completed {
                ...
                for checkpoint in checkpoints {
                    let through_sequence = match self
                        ...
                        .session_sequence_through_turn(
                            ::contracts::SessionId(session_id.to_owned()),
                            match uuid::Uuid::parse_str(&checkpoint.turn_id) {
                                Ok(turn_id) => ::contracts::TurnId(turn_id),
                                Err(error) => {
                                    return json!({
                                        "jsonrpc": "2.0",
                                        "id": id,
                    ...
                    entries.push(json!({
                        ...
                        "finalized": matches!(checkpoint.finalize_state, application::workspace_checkpoint::CheckpointFinalizeState::Finalized),
                    }));
                }
                json!({
                    "jsonrpc": "2.0",
    ...
    pub(super) async fn handle_turn_cancel(
        ...
        let result = if self.grok_hardening.prompt_queue {
            ...
            let turn_id = match uuid::Uuid::parse_str(turn_id) {
                Ok(value) => ::contracts::TurnId(value),
                Err(error) => {
                    return json!({
                        "jsonrpc": "2.0",
                        "id": id,
...
mod workspace_rewind_tests {
    ...
    fn prompt_mutation_requires_uuid_and_expected_version() {
        ...
        .unwrap();
        assert_eq!(parsed, (runtime::PromptId(id), 4));
    ...
    fn rejected_prompt_operation_is_not_reported_as_success() {
        let response = queue_op_response(
            &serde_json::json!(9),
            runtime::QueueOpResult::Rejected {

...
use crate::wiring::application::goal_service::{GoalAction, GoalServiceError};
use serde_json::json;
...
) -> Result<::contracts::GoalId, serde_json::Value> {
    request["params"]["goal_id"]
        .as_i64()
        .filter(|value| *value > 0)
        .map(::contracts::GoalId)
        .ok_or_else(|| {
            json!({
                "jsonrpc": "2.0",
                "id": id,
...
impl RequestHandler {
    ...
    pub(super) async fn handle_goal_create(
        ...
        match self
            ...
            .goals
            .create_goal(
                ::contracts::PrincipalId(session_id.clone()),
                session_id,
                scope.into(),
                spec,
            )

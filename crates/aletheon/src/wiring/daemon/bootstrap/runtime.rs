...
use ::contracts::AgentControlPort;
use corpus::Registry;
use tracing::info;

use adapters_agent_profile::MarkdownAgentProfileLoader;
...
pub(super) struct ProfileLoadResult {
    pub registry: Arc<crate::wiring::adapters::runtime::AgentProfileRegistry>,
    pub profiles: HashMap<String, ::contracts::AgentProfile>,
...
pub(super) async fn load_agent_profiles(
    ...
    profiles_config: &crate::config::AgentProfilesConfig,
) -> anyhow::Result<ProfileLoadResult> {
    let mut loader = MarkdownAgentProfileLoader::new();
...
async fn load_agent_profiles_from_loader(
    loader: MarkdownAgentProfileLoader,
    inference: Arc<dyn InferencePort>,
    default_llm: Arc<dyn LlmProvider>,
    _definitions: &[::contracts::ToolDefinition],
    profile_definitions: &[::contracts::ToolDefinition],
    config: &crate::config::CognitiveRuntimeConfig,
    profiles_config: &crate::config::AgentProfilesConfig,
) -> anyhow::Result<ProfileLoadResult> {
    ...
    let catalog = profile_definitions
        ...
        .collect::<HashMap<_, _>>();
    let registry = Arc::new(crate::wiring::adapters::runtime::AgentProfileRegistry::default());
    ...
    for role in loader.list() {
        ...
        let approval_policy = overrides
            ...
            .unwrap_or(::contracts::AgentApprovalPolicy::AutoApprove);
        ...
        let profile = ::contracts::AgentProfile {
            id: ::contracts::AgentProfileId(role.name.clone()),
            ...
            tool_timeout_ms,
            inheritable: true,
            parent_restriction: ::contracts::ParentRestriction::SameOrSafer,
        };
        registry.register(crate::wiring::adapters::runtime::ResolvedAgentProfile {
...
fn expand_tool_selectors(
    selectors: &[String],
    catalog: &HashMap<String, ::contracts::ToolDefinition>,
...
fn derive_risk_tier(
    ...
    match max_level {
        0 => ::contracts::RiskTier::ReadOnly,
        1 => ::contracts::RiskTier::Sandboxed,
        2 => ::contracts::RiskTier::System,
        _ => ::contracts::RiskTier::Unrestricted,
    }
}
...
pub(super) async fn register_agent_tools(
    tools: crate::wiring::domain::ToolRegistryHandle,
    ...
    let explicit =
        corpus::tools::tools::agent_control::AgentControlTools::new(agent_control.clone())
            .with_profile_names(profiles.keys().cloned());
    ...
    if !profiles.is_empty() {
        ...
        let agent_tool = corpus::tools::tools::agent_tool::AgentTool::new(
            profiles,
            agent_control,
            crate::wiring::adapters::runtime::NativeCognitRuntime::runtime_id(),
        );
...
use crate::wiring::adapters::runtime::ProviderWorkerRuntime;
...
pub(crate) async fn register_goal_runtimes(
    ...
    tools: Vec<::contracts::ToolDefinition>,
    capability: Arc<dyn CapabilityService>,
    clock: Arc<dyn Clock>,
) -> anyhow::Result<
    Vec<(
        ::contracts::RuntimeId,
        Arc<dyn runtime::DelegateTaskBackend>,
    )>,
> {
    if !config.enabled {
        return Ok(Vec::new());
    ...
    let routes = [
        (worker, ::contracts::CognitiveRole::Worker),
        (reviewer, ::contracts::CognitiveRole::Reviewer),
    ];
    ...
    for (route, role) in routes {
        ...
        let runtime_id = ::contracts::RuntimeId(route.runtime_id.clone());
    ...
    for (runtime_id, runtime) in prepared {
        registered.push((runtime_id, runtime as Arc<dyn runtime::DelegateTaskBackend>));
    }
    Ok(registered)
}

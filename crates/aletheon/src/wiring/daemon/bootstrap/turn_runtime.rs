    ...
    for (point, scripts) in [
        ...
        for (index, script) in scripts.iter().enumerate() {
            registry.register(corpus::hook::registry::RegisteredHook {
                ...
                source: "config".into(),
                script_path: Some(
                    crate::wiring::daemon::handler::format::expand_tilde(script).into(),
                ),
                point,
                priority: 100,
                timeout_ms: None,
...
pub(super) struct TurnRuntimeResources {
    ...
    pub(crate) capabilities: crate::wiring::daemon::handler::tool_executor::CapabilityResources,
    pub(crate) admission: Arc<dyn AdmissionController>,
    pub(crate) sessions: Arc<
        Mutex<
            HashMap<
                String,
                Arc<Mutex<crate::wiring::daemon::context_working_set::ContextWorkingSet>>,
            >,
        >,
    >,
...
impl ProductionTurnSessions {
    async fn manager(
        ...
        let manager = crate::wiring::daemon::context_working_set::compose_context_working_set(
            &self.data_dir,
            session_id.clone(),
            self.context_window,
    ...
    async fn budget_snapshot(
        &self,
        manager: &crate::wiring::daemon::context_working_set::ContextWorkingSet,
...
impl TurnSessionStatePort for ProductionTurnSessions {
    ...
    async fn begin_user(
        ...
        profile: ResolvedTurnProfile,
        context_costs: TurnContextBudgetCosts,
    ) -> anyhow::Result<crate::wiring::application::turn_runtime_ports::BeginUserResult> {
        let (session_id, manager) = self.manager(requested_session_id).await?;
        ...
        self.hooks
            ...
            .await;
        Ok(
            crate::wiring::application::turn_runtime_ports::BeginUserResult {
                session_id,
                turn_count,
    ...
    async fn finish(
        ...
        profile: ResolvedTurnProfile,
        context_costs: TurnContextBudgetCosts,
    ) -> anyhow::Result<crate::wiring::application::turn_runtime_ports::FinishUserResult> {
        let (session_id, manager_handle) = self.manager(requested_session_id).await?;
        let mut manager = manager_handle.lock().await;
        if !succeeded {
            return Ok(
                crate::wiring::application::turn_runtime_ports::FinishUserResult {
...
struct ProductionTurnApprovals {
    ...
    pending: crate::wiring::application::admin_service::PendingApprovals,
}
...
struct ProductionGovernedCapabilities {
    resources: crate::wiring::daemon::handler::tool_executor::CapabilityResources,
...
pub(super) struct ProductionActiveAgentProfile {
    ...
    profiles: Arc<crate::wiring::adapters::runtime::AgentProfileRegistry>,
}
...
mod tests {
    ...
    fn resolved_turn_profile_carries_behavior_and_authorization() {
        let profile = crate::wiring::application::turn_runtime_ports::ResolvedTurnProfile {

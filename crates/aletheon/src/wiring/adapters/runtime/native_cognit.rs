...
use crate::wiring::application::admin_service::{AdminServiceError, AgentProfileCatalogPort};
use std::time::Duration;

use ::contracts::cognitive_workflow::CognitiveTaskRuntimeBinding;
use ::contracts::{
...
use crate::wiring::application::{CapabilityExecutionContext, CapabilityService};
...
pub struct NativeCognitRuntimeResources {
    ...
    pub conscious_actions: Option<
        Arc<dyn crate::wiring::application::governed_capability::GovernedActionLoopResolver>,
    >,
...
impl NativeCognitRuntime {
    ...
    async fn execute(
        ...
        principal_context.turn_id = Some(::contracts::TurnId::new());
        let mut services = NativeTurnServices {
            ...
            execution: CapabilityExecutionContext {
                agent: Some(::contracts::AgentToolContext {
        ...
        let final_output = loop {
            ...
            let next = input.inbox.try_recv().await.filter(|payload| {
                payload.kind == ::contracts::AgentMessageKind::Input && payload.start_turn
            });
        ...
        let result = AgentResult {
            ...
            usage: AttemptUsage {
                ...
                cost_usd: None,
                elapsed_ms,
                observability: ::contracts::attempt::RuntimeObservability {
...
impl LlmProvider for MeteredLlm {
    ...
    async fn complete_stream(
        ...
        Ok(Box::pin(stream.map(move |chunk| {
            if let Ok(::contracts::StreamChunk::Usage { usage }) = &chunk {
...
fn harness_config(profile: &AgentProfile, budget: &::contracts::AgentBudget) -> HarnessConfig {
    HarnessConfig {
...
fn control_error(kind: AgentControlErrorKind, message: impl Into<String>) -> AgentControlError {
    ...
    runtime::compaction::truncate_utf8_bytes(&mut message, MAX_ERROR_BYTES);

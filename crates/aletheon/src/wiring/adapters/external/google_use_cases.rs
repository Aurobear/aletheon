...
use application::{ExternalCapabilityId, ExternalIdentityState, GrantState, LOCAL_OWNER_PRINCIPAL};
use std::sync::Arc;

use ::contracts::{Clock, PrincipalId};
...
pub struct ProductionExternalSourceUseCases {
    ...
    capabilities: Arc<RwLock<Vec<::contracts::CapabilityId>>>,
    clock: Arc<dyn Clock>,
}
...
impl ProductionExternalSourceUseCases {
    ...
    async fn register_read_tools(&self, google: &Arc<GoogleIntegration>) -> anyhow::Result<()> {
        ...
        let credentials = Arc::new(
            crate::wiring::adapters::external::GoogleCredentialSourceAdapter::new(
                repository.clone(),
                google.oauth(),
            ),
        );
        let accounts = Arc::new(
            crate::wiring::adapters::external::GoogleAccountResolverAdapter::new(repository),
        );
    ...
    async fn tool_registered(&self, name: &str) -> anyhow::Result<bool> {
        let grant = corpus::ExtensionGrant {
            ...
            agent_id: None,
            capabilities: vec![::contracts::CapabilityId(name.into())],
    ...
    async fn grant_tool(&self, name: &str) {
        let capability = ::contracts::CapabilityId(name.into());
...
fn safe_account(
    ...
    grant: &application::CapabilityGrant,
) -> serde_json::Value {
    json!({

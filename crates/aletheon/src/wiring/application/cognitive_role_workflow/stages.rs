...
pub(super) fn latest_change_set(
    packet: &AgentTaskPacket,
) -> anyhow::Result<&::contracts::cognitive_workflow::CognitiveChangeSetReceipt> {
    packet
        .selected_artifacts
        .iter()
        .rev()
...
) -> anyhow::Result<&'a ::contracts::cognitive_workflow::CognitiveValidationRecord> {
...
pub(super) fn validate_review_artifact<'a>(
    ...
    envelope: &'a CognitiveArtifactEnvelope,
    task_roots: &[String],
    expected_findings: Option<&[::contracts::cognitive_workflow::ReviewFinding]>,
) -> anyhow::Result<&'a [::contracts::cognitive_workflow::ReviewFinding]> {
    let change = latest_change_set(packet)?;
...
pub(super) fn unresolved_finding_scope(
    findings: &[::contracts::cognitive_workflow::ReviewFinding],
    task_roots: &[String],

...
use cognit::domain::{EvolutionLogEntry, ReflectionEntry, ReflectionTrigger};
...
use ::contracts::{
    Clock, OperationId, OperationResult, PrincipalContext, PrincipalId, ProcessId, SessionId,
    ThreadId,
};
...
use crate::wiring::application::admin_service::BackgroundWorkerPort;
use crate::wiring::application::session_service::{InterruptOutcome, ResumeResult, SessionService};
use crate::wiring::application::DaemonTurnOrchestrator;
use application::health::{ComponentHealth, HealthRegistry, ProductionHealth};
use application::workflow::WorkflowDef;
use platform::workflow_store::WorkflowStore;
...
#[async_trait]
pub trait CognitiveRuntimePort: Send + Sync {
...
pub struct ProductionHealthUseCases {
    ...
    agent_recovery: crate::wiring::application::agent_control::AgentRecoveryReport,
}

pub struct ProductionHealthResources {
    pub runtime_port: Arc<dyn CognitiveRuntimePort>,
    ...
    pub started_at: ::contracts::MonoTime,
...
impl HealthUseCases for ProductionHealthUseCases {
    ...
    async fn health(&self) -> RequestHealth {
        ...
        platform::storage_health::refresh_storage(
            &self.registry,
            &data_root,
            minimum_free_bytes,
            backup_required,
...
pub struct ProductionSessionLifecycle {
    ...
    approvals: crate::wiring::application::admin_service::ScopedApprovalCache,
    cancel_token: Arc<Mutex<Option<CancellationToken>>>,
    lifecycle: Arc<runtime::lifecycle_contributors::LifecycleRegistry>,
            ...
            .dispatch_if_enabled(
                ...
                runtime::lifecycle_contributors::LifecycleInput {
                    ...
                    principal_id: ::contracts::PrincipalId(
                        application::LOCAL_OWNER_PRINCIPAL.into(),
                    ),
                    thread_id: ::contracts::ThreadId(session_id.into()),
                    turn_id: None,
            ...
            match effect {
                ...
                LifecycleEffect::EmitEvent { schema, payload } => {
                    if let Some(bus) = &self.event_bus {
                        bus.publish_event(
                            ::contracts::SchemaId::from(schema.as_str()),
...
impl SessionLifecycleUseCases for ProductionSessionLifecycle {
    ...
    async fn finish(&self, session_id: String, turn_count: usize) -> anyhow::Result<()> {
        self.dispatch(
            runtime::lifecycle_contributors::LifecyclePhase::BeforeSessionEnd,
            &session_id,
        )
        .await?;
        ...
        self.dispatch(
            runtime::lifecycle_contributors::LifecyclePhase::AfterSessionEnd,
            &session_id,
        )
        .await?;
        Ok(())
    ...
    async fn start(&self, session_id: String, clear_approvals: bool) -> anyhow::Result<()> {
        self.dispatch(
            runtime::lifecycle_contributors::LifecyclePhase::BeforeSessionStart,
            &session_id,
        )
        .await?;
        if clear_approvals {
        ...
        self.corpus.execute_hook(&context).await;
        self.dispatch(
            runtime::lifecycle_contributors::LifecyclePhase::AfterSessionStart,
            &session_id,
        )
        .await?;
        Ok(())
...
pub trait TurnUseCases: Send + Sync {
    async fn watchdog_snapshot(
        &self,
    ) -> crate::wiring::application::turn_coordinator::TurnWatchdogSnapshot {
        Default::default()
    }
    async fn execute(
        &self,
    ...
    /// Admit a turn and return the Runtime-minted identity while execution
    /// continues asynchronously. Implementations must not mint a second
    /// transport-side id or expose a projected text as an admission receipt.
    async fn submit(
        &self,
        id: serde_json::Value,
        message: String,
        ...
        anyhow::bail!("async turn admission is not available on this route")
    }
    ...
    async fn cancel_active_for_connection(
        &self,
        _connection_id: ::contracts::ConnectionId,
    ) -> usize {
        0
    }
...
impl WorkflowUseCases for ProductionWorkflowUseCases {
    async fn save(&self, name: String, definition: WorkflowDef) -> anyhow::Result<()> {
        self.store()?.save(&name, &definition)
    }
    async fn load(&self, name: String) -> anyhow::Result<WorkflowDef> {

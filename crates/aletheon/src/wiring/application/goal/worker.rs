...
use super::{
    ...
    ObjectiveStore, RetryDecision, RetryPolicy,
};
use ::contracts::{AttemptUsage, Clock, CognitiveRole, GoalState, GoalWaitReason, RuntimeId};
use anyhow::Context;
use gateway::ports::{GoalProgress, GoalProgressKind};
use kernel::chronos::SystemClock;
use platform::storage_quota::StorageQuota;
...
impl GoalWorker {
    pub fn new_with_executor(
        store: Arc<Mutex<ObjectiveStore>>,
        executor: Arc<dyn super::AttemptExecutor>,
        worker_runtime: RuntimeId,
        reviewer_runtime: RuntimeId,
        progress_tx: mpsc::Sender<GoalProgress>,
    ) -> Self {

...
pub struct TurnCoordinator {
    ...
    host_acceptance: Option<Arc<agora::host_acceptance::HostAcceptanceController>>,
    runtime_turn_writer: Arc<dyn runtime::TurnLifecycleWriter>,
...
impl TurnCoordinator {
    ...
    pub fn from_components(
        ...
        Self::from_components_with_writer(
            kernel,
            store,
            grok_hardening,
            Arc::new(runtime::RuntimeTurnWriter::new()),
        )
    }

    fn from_components_with_writer(
        kernel: Arc<KernelRuntime>,
        store: Arc<dyn SessionAppendStore>,
        grok_hardening: GrokHardeningConfig,
        runtime_turn_writer: Arc<dyn runtime::TurnLifecycleWriter>,
    ...
    /// Production composition constructor. The Runtime turn writer is
    /// supplied at construction time so the coordinator cannot briefly own a
    /// second process-local writer before a later builder mutation replaces
    /// it. `from_components` remains available for isolated compatibility
    /// fixtures that intentionally exercise the legacy in-memory seam.
    pub fn from_components_with_runtime_turn_writer(
        kernel: Arc<KernelRuntime>,
        store: Arc<dyn SessionAppendStore>,
        grok_hardening: GrokHardeningConfig,
        runtime_turn_writer: Arc<dyn runtime::TurnLifecycleWriter>,
    ) -> Self {
        Self::from_components_with_writer(kernel, store, grok_hardening, runtime_turn_writer)
    }
    ...
    pub fn with_host_acceptance(
        mut self,
        controller: Arc<agora::host_acceptance::HostAcceptanceController>,
    ) -> Self {
    ...
    pub fn session_input(&self) -> Arc<application::session_input::SessionInputCoordinator> {
        ...
        Fut: Future<Output = Result<TurnExecution>> + Send,
    {
        ...
        self.kernel
            ...
            .await
            .context("starting turn operation")?;
        request.operation_id = operation.id;
        let runtime_session = runtime::SessionId(request.context.thread_id.0.clone());
        ...
        if self.canonical_session_guard
            && self
                .store
                .load_session(&::contracts::SessionId(runtime_session.0.clone()))
                .await
                ...
                .is_none()
        {
            let _ = self
                .kernel
                .fail_operation(operation.id, "canonical session not found")
                .await;
        ...
        Fut: Future<Output = Result<TurnExecution>> + Send,
    {
        ...
        match execution {
            Ok(execution) => {
                ...
                // SessionAppendStore item below is only the compatibility
    ...
    async fn finish_cancelled_execution(
        ...
        match &reason {
            CancelReason::DeadlineExceeded => self
                .runtime_turn_writer
                .timeout(
                    &runtime::SessionId(session_id.0.clone()),
                    &runtime::TurnId(turn_id.0.to_string()),
                )
                .await

...
impl AgentHostAdapter {
    ...
    /// been registered; only fixtures without a supervisor use the legacy
    /// entry.
    pub(crate) async fn spawn_via_runtime(
        ...
        let Some(supervisor) = &self.runtime_agent_supervisor else {
            return self.spawn_legacy(request, None).await;
        };
    ...
    pub(crate) async fn spawn_from_runtime(
        ...
        self.spawn_legacy_with_launcher(request, Some(identity), None)
            .await
    }
    ...
    /// the legacy AgentExecutionRegistry.
    pub(crate) async fn spawn_from_runtime_with_launcher(
        ...
        self.spawn_legacy_with_launcher(request, Some(identity), Some(launcher))
            .await
    }

    pub(crate) async fn spawn_legacy(
        &self,
        request: AgentSpawnRequest,
        runtime_identity: Option<runtime::DelegateReceipt>,
    ) -> Result<AgentHandle, AgentControlError> {
        self.spawn_legacy_with_launcher(request, runtime_identity, None)
            .await
    }

    async fn spawn_legacy_with_launcher(
        &self,
        ...
        let launcher = match launcher_override {
            ...
            None if runtime_admitted => {
                ...
                // back to the legacy catalog after identity admission: that

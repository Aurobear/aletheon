...
use super::{
    AgentMessageRecord, AgentResourceLease, AgentRunProjection, AgentRunRecord,
...
fn runtime_agent(agent: AgentId) -> runtime::AgentRunId {
    runtime::AgentRunId(agent.0.to_string())
}

fn projected_runtime_session(run: &AgentRunRecord) -> runtime::SessionId {
...
fn terminal_for(
    ...
    error: Option<&str>,
) -> Option<runtime::TurnTerminal> {
    match status {
        AgentRunStatus::Succeeded => Some(runtime::TurnTerminal::Completed),
        AgentRunStatus::Cancelled | AgentRunStatus::Interrupted => {
            Some(runtime::TurnTerminal::Interrupted)
        }
        AgentRunStatus::Failed => Some(runtime::TurnTerminal::Failed {
            message: error
                .map(str::to_owned)
...
fn runtime_delivery(value: AgentMessageDeliveryState) -> runtime::AgentMailboxDelivery {
    match value {
        AgentMessageDeliveryState::Pending => runtime::AgentMailboxDelivery::Pending,
        AgentMessageDeliveryState::Delivered => runtime::AgentMailboxDelivery::Delivered,
        AgentMessageDeliveryState::Rejected => runtime::AgentMailboxDelivery::Rejected,
    }
}
...
fn runtime_error(error: runtime::RuntimeError) -> AgentControlError {
    ...
    async fn append_message(
        ...
        self.supervisor
            .record_mailbox_delivery(
                ...
                runtime::AgentMailboxDelivery::Pending,
            )
            .await
            .map_err(runtime_error)?;
            ...
            Err(error) => {
                let _ = self
                    ...
                    .record_mailbox_delivery(
                        ...
                        format!("{:?}", payload.kind),
                        payload.correlation_id.map(|value| value.to_string()),
                        runtime::AgentMailboxDelivery::Rejected,
                    )
                    .await;
                return Err(error);
            }
...
mod tests {
    use super::*;
    use ::contracts::{
        AgentBudget, AgentContextFork, AgentHandle, AgentProfileId, AgentSnapshot,
        AgentSpawnRequest, AgoraSpaceId, OperationId, ProcessId, RuntimeId, RuntimeResumability,
    };
    use adapters_sqlite::runtime_agent::SqliteAgentRunProjection;
    use std::sync::Mutex;

    #[derive(Default)]
    struct RecordingSink(Mutex<Vec<runtime::AgentStreamEvent>>);

    #[async_trait]
    impl runtime::AgentEventSink for RecordingSink {
        async fn append(
            &self,
            event: runtime::AgentStreamEvent,
    ...
    fn projection(
        ...
        RuntimeAgentRunProjection,
        Arc<runtime::RuntimeAgentSupervisor>,
    ) {
        let supervisor = Arc::new(
            runtime::RuntimeAgentSupervisor::new(runtime::DelegateBackendRegistry::new())
                .with_event_sink(sink.clone()),
        );
        let projection = Arc::new(SqliteAgentRunProjection::in_memory().unwrap());
        (
            RuntimeAgentRunProjection::new(projection, supervisor.clone()),
            supervisor,
        )
    }

    #[tokio::test]
    async fn create_appends_runtime_admission_before_sql_projection() {
        let sink = Arc::new(RecordingSink::default());
        let (projection, supervisor) = projection(&sink);
        let run = queued_run();

        projection.create(&run).await.unwrap();
        ...
        assert!(matches!(
            ...
            runtime::RuntimeEvent::AgentRunAccepted { agent_run, .. }
                if agent_run.0 == run.agent_id().0.to_string()
        ));
        assert!(supervisor
            .terminal(&runtime::AgentRunId(run.agent_id().0.to_string()))
            .is_none());
    }

    #[tokio::test]
    async fn projection_failure_is_fenced_as_runtime_failure() {
        let sink = Arc::new(RecordingSink::default());
        let (projection, supervisor) = projection(&sink);
        let run = queued_run();

        projection.create(&run).await.unwrap();
        let error = projection.create(&run).await.unwrap_err();
        ...
        assert!(matches!(
            ...
            runtime::RuntimeEvent::AgentRunSettled {
                terminal: runtime::TurnTerminal::Failed { .. },
                ..
            }
        ));
        assert!(matches!(
            supervisor.terminal(&runtime::AgentRunId(run.agent_id().0.to_string())),
            Some(runtime::TurnTerminal::Failed { .. })
        ));
    }

    #[tokio::test]
    async fn terminal_transition_appends_runtime_before_projection_transition() {
        let sink = Arc::new(RecordingSink::default());
        let (projection, _supervisor) = projection(&sink);
        let run = queued_run();
        ...
        let transitioned = projection
            ...
            .await
            .unwrap();
        assert_eq!(transitioned.status(), AgentRunStatus::Failed);
        ...
        assert!(matches!(
            ...
            runtime::RuntimeEvent::AgentRunSettled {
                terminal: runtime::TurnTerminal::Failed { message },
                ..

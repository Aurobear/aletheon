...
//!
//! Defines the minimal transport and typed Application ports that decouple
//! the channel router from the daemon runtime, plus a thin router
//! that classifies inbound messages and hands them to a
//! [`CapabilityRegistry`] of typed handlers (`capability/`).

use std::sync::Arc;

use crate::channel::{ConversationId, InboundMessage, MessageContent, MessageId, OutboundMessage};
use ::contracts::{ApprovalSnapshot, ApprovalStatus};

use super::capability::chat::ChatHandler;
use super::capability::goal::GoalHandler;
use super::capability::greeting::GreetingHandler;
use super::effect::OutboundEffect;
use super::intent::{classify_intent, Intent};
use super::notify::render_approval_notification;
use super::ports::ChannelInsertOutcome;
use super::ports::{
    ChannelApprovalCallbackPort, ChannelApprovalDeliveryPort, ChannelChatAdapter,
    ChannelGoalCommandPort, ChannelProjectionStore, ChannelTurnApplicationPort, GoalProgress,
};
use super::registry::{CapabilityHandler, CapabilityRegistry, HandlerContext};
...
///
/// Owns a [`ChannelProjectionStore`] for persistence and delegates capability
...
pub struct ChannelRouter {
    store: Box<dyn ChannelProjectionStore>,
    registry: CapabilityRegistry,
    approval_callback_port: Option<Arc<dyn ChannelApprovalCallbackPort>>,
    approval_delivery_port: Option<Arc<dyn ChannelApprovalDeliveryPort>>,
}
...
impl ChannelRouter {
    ...
    pub fn new<S>(store: S, turn_executor: Arc<dyn ChannelTurnApplicationPort>) -> Self
    where
        S: ChannelProjectionStore + 'static,
    {
        let cwd = std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("/"));
        let workspace = ::contracts::WorkspacePolicy::from_resolved_roots(cwd, Vec::new())
            .expect("the process working directory is an absolute workspace");
        Self::new_with_workspace(store, turn_executor, workspace)
    }

    pub fn new_with_workspace<S>(
        store: S,
        turn_executor: Arc<dyn ChannelTurnApplicationPort>,
        ...
        S: ChannelProjectionStore + 'static,
    {
        ...
        let chat = Arc::new(ChannelChatAdapter::new(turn_executor, workspace));
        registry.register(Arc::new(ChatHandler::new(chat)));
    ...
    pub fn with_registry<S>(store: S, registry: CapabilityRegistry) -> Self
    where
        S: ChannelProjectionStore + 'static,
    {
        Self {
            store: Box::new(store),
            registry,
            approval_callback_port: None,
            approval_delivery_port: None,
        }
    }
    ...
    pub fn with_goal_command_port(self, executor: Arc<dyn ChannelGoalCommandPort>) -> Self {
        self.with_capability(Arc::new(GoalHandler::new(executor)))
    }

    pub fn with_approval_delivery_port(
        mut self,
        port: Arc<dyn ChannelApprovalDeliveryPort>,
    ) -> Self {
        self.approval_delivery_port = Some(port);
        self
    }

    pub fn with_approval_callback_port(
        mut self,
        port: Arc<dyn ChannelApprovalCallbackPort>,
    ) -> Self {
        self.approval_callback_port = Some(port);
        self
    }
    ...
    pub async fn notify_approval(
        ...
        let port = self
            .approval_delivery_port
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("approval delivery projection is not configured"))?;
    ...
    /// Forward an approval callback to the Application adapter.
    async fn execute_approval_action(
        ...
        let port = self
            .approval_callback_port
            .clone()
            .ok_or_else(|| anyhow::anyhow!("approval callback application is not configured"))?;
        port.execute(principal, channel, action_data, now_ms).await
    }
    ...
    pub async fn process(
        ...
        match self.store.insert_inbound(message)? {
            ChannelInsertOutcome::Duplicate => return Ok(()),
            ChannelInsertOutcome::Inserted => { /* continue processing */ }
        }
        ...
        match transport.send(&outbound).await {
            Ok(provider_msg_id) => {
                ...
                self.store
                    .mark_outbound_sent(&message.correlation_id, &provider_msg_id)?;
            }
            Err(e) => {
                // 10b. Mark outbox row as failed so it can be retried
                //     independently of the already-completed inbox turn.
                self.store
                    .mark_outbound_failed(&message.correlation_id, &e.to_string())?;
            }
        }
    ...
    fn reject_inbound(
        ...
        self.store.reject_inbound(channel, message_id, next_cursor)
    }
    ...
    fn fail_inbound(&mut self, channel: &str, message_id: &str, error: &str) -> anyhow::Result<()> {
        self.store.fail_inbound(channel, message_id, error)
    }
    ...
    pub async fn recover_pending_inbox(
        ...
        for msg in &pending {
            ...
            match transport.send(&outbound).await {
                Ok(provider_msg_id) => {
                    self.store
                        .mark_outbound_sent(&msg.correlation_id, &provider_msg_id)?;
                }
                Err(e) => {
                    self.store
                        .mark_outbound_failed(&msg.correlation_id, &e.to_string())?;
                }
            }
            count += 1;
        }
            ...
            match transport.send(outbound).await {
                Ok(provider_id) => {
                    ...
                    if outbound.correlation_id.starts_with("approval:") {
                        if let Some(port) = &self.approval_delivery_port {
                ...
                Err(e) => {
                    self.store
                        .mark_outbound_failed(&outbound.correlation_id, &e.to_string())?;
                    if outbound.correlation_id.starts_with("approval:") {
                        if let Some(port) = &self.approval_delivery_port {
    ...
    #[cfg(test)]
    #[allow(dead_code)]
    pub fn store(&self) -> &dyn ChannelProjectionStore {
        self.store.as_ref()
    }
}
...
fn build_outbound(
    ...
    let content = match routed {
        ...
        Intent::Greeting => MessageContent::Text {
            text: ai_reply
                .unwrap_or("Hello! I am Aletheon. How can I help you today?")
                .to_string(),
        },

// crates/aletheon-runtime/src/core/reflection.rs
...
impl ReflectionEngine {
    ...
    pub fn reflect(&mut self, context: &ReflectionContext) -> ReflectionResult {
        ...
        let status_str = match &recommendation {
            ...
            ReflectionRecommendation::AdjustStrategy(s) => format!("Adjusting: {}", s),
            ReflectionRecommendation::Stop(reason) => format!("Stopping: {:?}", reason),
        };
        ...
        let summary = format!(
            "Reflection: {} tool calls, {} errors ({:.0}%). Spec: {}. {}",
...
mod tests {
    ...
    fn test_high_error_rate_stops() {
        ...
        let result = engine.reflect(&make_ctx(10, 6));
        assert!(matches!(
    ...
    fn test_agent_enhancement() {
        ...
        let ctx = ReflectionContext {
            ...
            constraints: vec![],
            test_failures: vec![],
            unexpected_outputs: vec!["Added rate limiting".into()],
        };
        let result = engine.reflect(&ctx);
    ...
    fn test_on_track() {
        let mut engine = ReflectionEngine::new(5);
        let result = engine.reflect(&make_ctx(3, 0));

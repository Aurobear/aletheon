...
use crate::SkillProgressSink;
use ::contracts::types::embodiment::SkillProgress;
use ::contracts::{
    EnvelopeV2, EnvelopeV2Delivery, EnvelopeV2Target, MessageId, NamespaceId, OperationId, SchemaId,
};
use async_trait::async_trait;
use runtime::{
    EventId, EventIdentity, EventPayload, EventTreeId, EventVisibility, UnsequencedEvent,
};
...
/// `::contracts::TurnEventSink` as a typed `TurnEvent::EmbodimentProgress`, carrying
/// the real operation id injected by `BoundedProgressSink`.
pub struct EventEmbodimentProgress {
    sink: Arc<dyn ::contracts::TurnEventSink>,
}

impl EventEmbodimentProgress {
    pub fn new(sink: Arc<dyn ::contracts::TurnEventSink>) -> Self {
        Self { sink }
    }
}
...
impl EmbodimentProgressPort for EventEmbodimentProgress {
    async fn record(&self, progress: SkillProgress) {
        self.sink
            .emit(::contracts::TurnEvent::EmbodimentProgress {
...
#[async_trait]
impl ::contracts::TurnEventSink for TracingTurnEventSink {
    async fn emit(&self, event: ::contracts::TurnEvent) {
        match &event {
            ::contracts::TurnEvent::EmbodimentProgress {
                operation_id,
                skill,
                fraction,
                note,
...
/// `runtime::EventSpine` (rooted at the daemon session tree) so session/UI
...
impl SpineTurnEventSink {
    pub fn new(spine: Arc<dyn runtime::EventSpine>, session_id: impl Into<String>) -> Self {
        Self {
            spine,
...
#[async_trait]
impl ::contracts::TurnEventSink for SpineTurnEventSink {
    async fn emit(&self, event: ::contracts::TurnEvent) {
        let ::contracts::TurnEvent::EmbodimentProgress {
            operation_id,
            skill,
            fraction,
            note,
...
pub struct DeferredTurnEventSink {
    inner: tokio::sync::RwLock<Option<Arc<dyn ::contracts::TurnEventSink>>>,
}
...
impl DeferredTurnEventSink {
    ...
    pub async fn bind(&self, sink: Arc<dyn ::contracts::TurnEventSink>) {
...
#[async_trait]
impl ::contracts::TurnEventSink for DeferredTurnEventSink {
    async fn emit(&self, event: ::contracts::TurnEvent) {
...
mod tests {
    use super::*;
    use ::contracts::types::embodiment::SkillId;
    use ::contracts::{MonoTime, TurnEventSink};
    use runtime::event_spine::{EventPosition, SpineEvent, TreeSequence};
    use std::sync::Mutex;

    #[derive(Default)]
    struct RecordingSink(Mutex<Vec<::contracts::TurnEvent>>);
    ...
    #[async_trait]
    impl ::contracts::TurnEventSink for RecordingSink {
        async fn emit(&self, event: ::contracts::TurnEvent) {
    ...
    async fn event_sink_forwards_skill_progress_with_real_operation_id() {
        ...
        let operation_id = ::contracts::OperationId::new();
        progress
    ...
    async fn spine_sink_appends_progress_to_session_tree() {
        ...
        let operation_id = ::contracts::OperationId::new();
        sink.emit(::contracts::TurnEvent::EmbodimentProgress {
            operation_id,
        ...
        assert_eq!(
            event.tree_id,
            runtime::EventTreeId::for_root_session("session-1"),
    ...
    async fn deferred_sink_logs_before_bind_and_forwards_after() {
        ...
        let operation_id = ::contracts::OperationId::new();
        let event = ::contracts::TurnEvent::EmbodimentProgress {
            operation_id,
            skill: "kuavo.stance".into(),
            fraction: 0.25,
            note: "starting".into(),

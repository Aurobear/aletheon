...
use adapters_sqlite::schema_migrations as migrations;
use application::ExternalIdentityId;
...
fn subscription_matches(subscription: &GoogleSubscription, event: &ExternalEventEnvelope) -> bool {
    ...
    if query.important_only {
        return matches!(
            &event.event,
            corpus::tools::google::ExternalEvent::MailReceived(change)
                | corpus::tools::google::ExternalEvent::MailUpdated(change)
...
mod tests {
    use super::*;
    use application::ExternalProviderId;
    use corpus::tools::google::{
        ExternalEvent, ExternalEventDraft, ExternalObjectRef, ExternalRecordRef, MailChange,
        MailMessageSummary, OpaqueCursor, OpaqueProviderObjectId,
    };
    ...
    impl Fixture {
        fn new() -> Self {
            ...
            // Seed the canonical external_identities row directly so the sync
            // store's foreign key resolves without importing the host external
            // identity repository.  Run the shared migrations first so the
            // seed table exists in the fresh database.
            let connection = Connection::open(&path).unwrap();
            migrations::run_migrations(&connection).unwrap();
            connection
                .execute(
                    "INSERT INTO external_identities(
                        identity_id,provider,provider_subject,principal_id,email,state,
                        created_at_ms,updated_at_ms,version
                     ) VALUES(?1,'google',?2,?3,?4,'active',1,1,0)",
                    params![account.to_string(), "subject", "owner", "owner@example.com"],
                )
                .unwrap();
            drop(connection);

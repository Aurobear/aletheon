...
use corpus::tools::google::{ExternalEventEnvelope, ExternalEventId};

use std::sync::{Arc, Mutex};
use tokio_util::sync::CancellationToken;

use adapters_sqlite::ChannelStore;
use application::goal::GoalCoordinator;
use corpus::tools::google::ExternalEvent;
use gateway::channel::{ConversationId, MessageContent, OutboundMessage};
use std::path::Path;
...
/// Concrete Google/Gmail event capability seam. Rich provider event DTOs stay
/// with the Google adapter rather than leaking into Gateway or Application.
#[async_trait]
pub trait GoogleEventCapabilityHandler: Send + Sync {
    async fn handle(
        &self,
        event: &ExternalEventEnvelope,
        cancel: &CancellationToken,
    ) -> anyhow::Result<Vec<gateway::effect::OutboundEffect>>;
}
...
pub struct GoogleEventRouter {
    ...
    /// Concrete Gmail ingest capability owned by the Google adapter.
    mail_ingress: Option<Arc<dyn GoogleEventCapabilityHandler>>,
...
impl GoogleEventRouter {
    ...
    /// Register Gmail's external-event handler; `deliver` invokes it directly
    /// for matching events — bypassing duplex `ChannelRouter::process`.
    pub fn with_mail_ingress(mut self, handler: Arc<dyn GoogleEventCapabilityHandler>) -> Self {
        self.mail_ingress = Some(handler);
        self
    }
...
impl GoogleEventSink for GoogleEventRouter {
    async fn deliver(
        ...
        if matches!(event.event, ExternalEvent::MailReceived(_)) {
            if let Some(handler) = &self.mail_ingress {
                handler
                    .handle(event, cancel)
                    .await
                    .map_err(|error| error.to_string())?;
        ...
        for subscription in subscriptions {
            let goal_event = application::goal::GoalExternalEvent {
                account_id: event.account_id,
                event_id: event.id.to_string(),
                object_id: event.object.object_id.clone(),
                source_timestamp_ms: event.source_timestamp_ms,
            };
            self.goals
                .wake_for_external_event(&subscription.principal_id, &goal_event)
                .map_err(|error| error.to_string())?;
...
fn stream_for(event: &ExternalEventEnvelope) -> super::SyncStream {
    match event.event {
        corpus::tools::google::ExternalEvent::MailReceived(_)
        | corpus::tools::google::ExternalEvent::MailUpdated(_)
        | corpus::tools::google::ExternalEvent::MailDeleted(_) => super::SyncStream::GmailHistory,
        corpus::tools::google::ExternalEvent::CalendarEventCreated(_)
        | corpus::tools::google::ExternalEvent::CalendarEventUpdated(_)
        | corpus::tools::google::ExternalEvent::CalendarEventDeleted(_) => {
            super::SyncStream::Calendar
        }
        corpus::tools::google::ExternalEvent::FileCreated(_)
        | corpus::tools::google::ExternalEvent::FileUpdated(_)
        | corpus::tools::google::ExternalEvent::FileDeleted(_) => super::SyncStream::DriveChanges,
    }
}

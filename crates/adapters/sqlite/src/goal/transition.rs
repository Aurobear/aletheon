...
use super::{GoalId, GoalSnapshot, GoalState, GoalWaitReason, ObjectiveStore, ProcessId};
use serde_json;
use std::fmt;
...
impl ObjectiveStore {
    /// Atomically replace the runtime process link and append its versioned event.
    pub fn set_process_link(
        ...
        let process_id = process_id.map(|value| value.0.to_string());
        let tx = self.db.unchecked_transaction()?;
        let changed = tx.execute(
            "UPDATE objectives SET process_id = ?1, version = ?2, updated_at = datetime('now')
             WHERE objective_id = ?3 AND version = ?4",
            rusqlite::params![process_id, new_version, id.0, expected_version],
        )?;
        if changed == 0 {
            tx.rollback()?;
            return match self.get_goal(id)? {
                Some(goal) => Err(GoalTransitionError::VersionConflict {
                    expected: expected_version,
                    actual: goal.version,
                }),

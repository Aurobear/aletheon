...
pub use application::goal::{PersistedCodingJob, PersistedVerificationReport};
...
impl ObjectiveStore {
    ...
    pub fn persist_coding_job(
        ...
        self.artifacts.write_atomic(&diff_artifact_ref, diff)?;
        ...
        if let Err(error) = result {
            let _ = self.artifacts.remove(&diff_artifact_ref);
            return Err(error);
        }
    ...
    pub fn load_coding_job(&self, job_id: CodingJobId) -> Result<Option<PersistedCodingJob>> {
        ...
        let diff = self
            .artifacts
            .read_bounded(&artifact_ref, MAX_DIFF_ARTIFACT_BYTES)?;

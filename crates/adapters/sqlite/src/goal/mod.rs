//! SQLite persistence adapter for durable Goal state.

mod advance_repository;
mod attempt;
mod attempt_persistence;
pub mod budget;
mod coordinator_repository;
pub mod repository;
mod store;
pub mod summary;
pub mod transition;
...
pub use advance_repository::SqliteGoalAdvanceRepository;
pub use application::goal_attempt::{GoalAttempt, GoalEvaluationFeedback};
pub use application::objective::{Objective, ObjectiveStatus, ObjectiveSummary};
pub use attempt_persistence::SqliteGoalAttemptPersistence;
pub use budget::{GoalBudgetError, GoalBudgetRequest, GoalBudgetReservation};
pub use coordinator_repository::SqliteGoalCoordinatorRepository;
...
pub use transition::GoalTransitionError;
pub use verification::{CodingJobRecoveryRecord, PersistedCodingJob, PersistedVerificationReport};

use anyhow::{Context, Result};
use contracts::{
    GoalBudgetUsage, GoalId, GoalSnapshot, GoalSpec, GoalState, GoalWaitReason, PrincipalId,
    ProcessId,
};
use rusqlite::Connection;

/// SQLite-backed Goal repository. SQL, row mapping and transaction ownership
/// are confined to this adapter.
pub struct ObjectiveStore {
    pub(crate) db: Connection,
    pub(crate) artifact_dir: std::path::PathBuf,
    pub(crate) artifacts: std::sync::Arc<dyn application::goal_artifact::GoalArtifactStore>,
}
...
impl ObjectiveStore {
    ...
    pub fn open(path: &std::path::Path) -> Result<Self> {
        ...
        let artifacts = std::sync::Arc::new(
            platform::goal_artifact::FilesystemGoalArtifactStore::open(&artifact_dir)?,
        );
        Self::open_with_artifacts(path, artifacts)
    }

    pub fn open_with_artifacts(
        path: &std::path::Path,
        artifacts: std::sync::Arc<dyn application::goal_artifact::GoalArtifactStore>,
    ) -> Result<Self> {
        let db = Connection::open(path).context("opening objective store DB")?;
        crate::schema_migrations::run_migrations(&db)?;
        let artifact_dir = artifacts.root().to_path_buf();
        Ok(Self {
            db,
            artifact_dir,
            artifacts,

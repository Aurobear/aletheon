...
impl ChannelStore {
    ...
    /// Mark an unknown sender rejected and advance the provider cursor in one
    /// transaction. The dispatcher must not reach into SQLite directly.
    ...
    /// Mark an inbox message failed without advancing its cursor.
    pub fn fail_inbound(&self, channel: &str, message_id: &str, error: &str) -> Result<()> {
        self.db.execute(

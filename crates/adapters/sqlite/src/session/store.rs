...
impl SessionStore {
    ...
    /// Open the legacy database without schema creation or any write flag.
    /// Runtime cutover uses this reader for one-way migration/audit only.
    ...
    /// Read every legacy record needed by the canonical migration. This method
    /// has no mutation side effects and is also usable by audit tooling.
    pub fn list_records(&self) -> Result<Vec<SessionRecord>> {
        self.list(None, 100_000)
    }

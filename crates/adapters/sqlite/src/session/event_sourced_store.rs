...
use ::contracts::{
    AppendOutcome, EnvelopeV2, EnvelopeV2Delivery, EnvelopeV2Target, ItemId, ItemPayload,
    ItemRecord, MessageId, NamespaceId, PrincipalId, SchemaId, SessionAppendStore,
    SessionForkedEvent, SessionId, SessionPrincipalBoundEvent, SessionReadStore, SessionRecord,
...
/// Startup replay is deliberately bounded. Older committed rows are already
/// represented by the durable read model; a crash window can only require the
/// recent suffix to be replayed. Full-history audit remains an offline tool.
// Keep the startup recovery window bounded to the crash suffix rather than
// replaying the entire historical spine. The canonical read model already
// contains older committed events; replaying those rows only reopens one
// SQLite connection/transaction per item and can hold daemon readiness for
// minutes on a long-lived installation. Full-history rebuild remains an
// explicit offline operation, not a daemon bootstrap requirement.
const RECONCILIATION_MAX_EVENTS: u64 = 8_192;
...
pub async fn reconcile_committed_session_events(
    ...
    let mut after_row_id = through_row_id.saturating_sub(RECONCILIATION_MAX_EVENTS);
...
impl EventSourcedSessionStore {
    ...
    /// Isolated fixture authority. Production composition injects the shared
    /// durable event spine and projection set through [`Self::new`].
    pub fn in_memory(read_model: Arc<dyn SessionProjectionStore>) -> Arc<dyn SessionAppendStore> {
        Arc::new(Self::new(
            read_model,
            Arc::new(
                crate::event_spine::SqliteEventSpine::open(":memory:")
                    .expect("in-memory event spine"),
            ),
            Arc::new(crate::projection_set::DefaultEventProjectionSet::in_memory()),
        ))
    }

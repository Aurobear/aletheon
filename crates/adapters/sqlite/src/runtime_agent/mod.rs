...
use ::contracts::types::agent_control::MAX_LIST_ITEMS;
use ::contracts::{
...
use runtime::agent_repository::{
    AgentMessageRecord, AgentResourceLease, AgentResourceLeaseKind, AgentRunProjection,
    AgentRunRecord, AgentTerminalReceipt,
};
use runtime::{agent_spawn_request_hash, reduce_agent_status_transition, AgentLifecycleEffect};
...
#[derive(Clone)]
pub struct SqliteAgentRunProjection {
    connection: Arc<Mutex<Connection>>,
}

impl std::fmt::Debug for SqliteAgentRunProjection {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("SqliteAgentRunProjection")
            .finish_non_exhaustive()
    }
}

impl SqliteAgentRunProjection {
...
#[async_trait]
impl AgentRunProjection for SqliteAgentRunProjection {
    async fn create(&self, run: &AgentRunRecord) -> Result<(), AgentControlError> {
        ...
        if run.workspace_id != runtime::agent_workspace_id(run.agent_id()) {
            return Err(control_error(
                AgentControlErrorKind::InvalidRequest,
    ...
    async fn runtime_process(
        ...
        connection
            ...
            .map(|(process_id, generation, os_pid, start_time_ticks)| {
                Ok(::contracts::RuntimeProcessId {
                    agent_id: agent,
                    process_id: ProcessId(parse_uuid(&process_id)?),
                    generation,
                    os_pid: ::contracts::OsProcessId(os_pid),
                    start_time_ticks,
                })
            })
            .transpose()
    ...
    async fn compact_terminal(
        ...
        for value in ids {
            ...
            transaction
                .execute(
                    "DELETE FROM agent_terminal_receipts WHERE agent_id=?1",
                    [&value],
                )
                .map_err(persistence)?;
            transaction
...
fn message_kind_wire(kind: &::contracts::AgentMessageKind) -> &'static str {
    use ::contracts::AgentMessageKind::*;
    match kind {
        Input => "input",
        Progress => "progress",
        Result => "result",
...
#[cfg(test)]
mod tests {
    use runtime::AgentRunProjection;

    use super::SqliteAgentRunProjection;

    #[test]
    fn runtime_agent_projection_opens_its_versioned_schema() {
        let projection = SqliteAgentRunProjection::in_memory()
            .expect("Runtime Agent SQLite projection should migrate in memory");
        assert!(format!("{projection:?}").contains("SqliteAgentRunProjection"));
    }

    #[tokio::test]
    async fn terminal_compaction_removes_receipt_before_parent_run() {
        let projection = SqliteAgentRunProjection::in_memory()
            .expect("Runtime Agent SQLite projection should migrate in memory");
        let agent_id = "00000000-0000-0000-0000-000000000001";
        {
            let connection = projection.connection.lock();
            connection
                .execute(
                    "INSERT INTO agent_runs(agent_id,root_agent_id,process_id,operation_id,runtime_id,profile_id,status,request_json,request_hash,created_at_ms,ended_at_ms,version,retain_until_ms,workspace_id,root_process_id,broadcast_refs_json,resumability_json) VALUES(?1,?1,?2,?3,'runtime','profile','succeeded','{}','hash',1,2,0,2,'workspace',?2,'[]','{\"mode\":\"never\"}')",
                    rusqlite::params![
                        agent_id,
            ...
            connection
                .execute(
                    "INSERT INTO agent_terminal_receipts(agent_id,generation,status,recorded_at_ms) VALUES(?1,'generation','succeeded',2)",
                    [agent_id],
                )
                .unwrap();
        }

        let removed = projection.compact_terminal(3, 1).await.unwrap();
        assert_eq!(removed.len(), 1);
        let connection = projection.connection.lock();
        let receipts: i64 = connection
            .query_row("SELECT COUNT(*) FROM agent_terminal_receipts", [], |row| {
                row.get(0)
            })
            .unwrap();

//! External Agent backend adapters for Runtime-owned execution authority.
//!
//! Translates Goal tasks and configured runtime backends into typed Runtime
//! commands. Runtime mints the run/generation, fences the terminal, and owns
//! cancellation admission; this crate keeps only the backends' in-flight
//! execution evidence and provider/process adapters.
...
/// Configured execution binding for one configured Goal runtime. It does
/// not allocate identities or publish terminal events; both remain Runtime
/// responsibilities.
pub struct GoalAttemptBackend {
    task_backend: Arc<dyn runtime::DelegateTaskBackend>,
...
impl DelegateBackend for GoalAttemptBackend {
    async fn spawn(
        ...
        let task_backend = self.task_backend.clone();
        let task = task.clone();
        let task_result = result.clone();
        let task_done = done.clone();
        let task_cancel = cancel.clone();
        tokio::spawn(async move {
            let outcome = task_backend.run_attempt(&task, task_cancel).await;
...
#[async_trait]
impl GoalAttemptPort for RuntimeGoalAttemptExecutor {

...
/// Semantic profile document owned by Runtime; Markdown is only the host
/// serialization adapter.
pub type MarkdownAgentProfile = runtime::AgentProfileDocument;

// ── MarkdownAgentProfileLoader ──────────────────────────────────────────────
...
/// ```
pub struct MarkdownAgentProfileLoader {
    agents: Vec<MarkdownAgentProfile>,
}

impl MarkdownAgentProfileLoader {
    ...
    pub fn get(&self, name: &str) -> Option<&MarkdownAgentProfile> {
        self.agents.iter().find(|a| a.name == name)
    }

    /// All loaded agents.
    pub fn list(&self) -> &[MarkdownAgentProfile] {
        &self.agents
    }
...
impl Default for MarkdownAgentProfileLoader {
    fn default() -> Self {
        Self::new()
    }
}

impl runtime::AgentProfilePort for MarkdownAgentProfileLoader {
    fn profiles(&self) -> Vec<runtime::AgentProfileDocument> {
        self.agents.clone()
    }
}
...
fn parse_agent_md(content: &str, path: &Path) -> Option<MarkdownAgentProfile> {
...
Body"#,
    ...
    fn orphan_legacy_toml_profile_is_rejected() {
        ...
        .unwrap();
        let loader = MarkdownAgentProfileLoader::new();
        let error = loader.validate_legacy_toml_grants(tmp.path()).unwrap_err();
...
Skill body."#,
        ...
        let mut agent_loader = MarkdownAgentProfileLoader::new();

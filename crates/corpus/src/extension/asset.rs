...
use super::package::{PackageId, PermissionRequestSet};
...
pub struct CapabilityDescriptor {
    /// Reuses the existing ::contracts::CapabilityId for identity continuity.
    pub id: CapabilityId,
    pub kind: CapabilityKind,
    /// Reuses the existing ::contracts::RiskLevel for risk continuity.
    pub risk: RiskLevel,
}
...
// Pure compatibility projections from the shared legacy metadata contract.
impl From<crate::ExtensionKind> for AssetKind {
    fn from(kind: crate::ExtensionKind) -> Self {
        match kind {
            crate::ExtensionKind::Tool | crate::ExtensionKind::Plugin => Self::Executable,
            crate::ExtensionKind::Skill => Self::Skill,
            crate::ExtensionKind::Hook => Self::Hook,
            crate::ExtensionKind::Mcp => Self::Connector,
        }
    }
}

impl From<crate::ExtensionOrigin> for AssetOrigin {
    fn from(origin: crate::ExtensionOrigin) -> Self {
        match origin {
            crate::ExtensionOrigin::BuiltIn => Self::BuiltIn,
            crate::ExtensionOrigin::FileSystem { path } => Self::FileSystem { path },
            crate::ExtensionOrigin::Package { package } => Self::Package {
                package,
                version: String::new(),
            },
            crate::ExtensionOrigin::Remote { server } => Self::Package {
...
impl From<&crate::ExtensionDescriptor> for AssetDescriptor {
    fn from(descriptor: &crate::ExtensionDescriptor) -> Self {
        let (package_name, asset_name) = descriptor
            .id
            .as_str()
            .split_once(':')
            .map(|(kind, name)| (format!("legacy:{kind}"), name.to_string()))
            .unwrap_or_else(|| ("legacy:unknown".into(), descriptor.id.as_str().into()));
        Self {
            id: AssetId {
            ...
            origin: descriptor.origin.clone().into(),
            runtime: None,

...
impl ExecdClient {
    ...
    pub async fn process_start(
        ...
        timeout: Duration,
        policy: Option<&::contracts::ResolvedSandboxPolicy>,
    ) -> Result<String> {
        let result = self
            .request(
                "process/start",
...
pub struct ExecdSandboxBackend {
    config: ExecdConfig,
...
impl ExecdSandboxBackend {
    ...
    async fn execute_inner(
        ...
        timeout: Duration,
        sink: Option<&::contracts::ToolEventSink>,
    ...
    async fn execute_process(
                    ...
                    if !chunk.data.is_empty() {
                        ...
                        if let Some(sink) = sink {
                            for line in chunk.data.lines() {
                                let _ =
                                    sink.progress(::contracts::ToolProgress::Text(line.to_owned()));
                            }
                        }
                    }
        ...
        Ok(::contracts::SandboxResult {
            stdout,
            stderr,
            exit_code,
            backend_used: "execd".into(),
            isolation_level: ::contracts::IsolationLevel::Process,
...
fn trusted_tool_invocation(
    descriptor: &::contracts::tool::ToolExecutionDescriptor,
    ...
    match descriptor {
        ::contracts::tool::ToolExecutionDescriptor::EbpfCompile => {
        ...
        ::contracts::tool::ToolExecutionDescriptor::ModuleBuild => {
        ...
        ::contracts::tool::ToolExecutionDescriptor::Script { canonical_path } => {
        ...
        ::contracts::tool::ToolExecutionDescriptor::KernelBuild => Err(
            "kernel build/install descriptor requires an unavailable privileged execution authority"
                .into(),
        ),
        ::contracts::tool::ToolExecutionDescriptor::ModuleLoad => Err(
...
impl crate::security::StructuredToolSandbox for ExecdSandboxBackend {
    ...
    async fn execute(
        ...
        sandbox: &::contracts::SandboxConfig,
    ) -> std::result::Result<::contracts::ToolResult, String> {
        if let Some(descriptor) = descriptor {
            let descriptor_matches_tool = matches!(
                ...
                (
                    ...
                    "kernel_build",
                    ::contracts::tool::ToolExecutionDescriptor::KernelBuild
                ) | (
                    "module_build",
                    ::contracts::tool::ToolExecutionDescriptor::ModuleBuild
                ) | (
                    "module_load",
                    ::contracts::tool::ToolExecutionDescriptor::ModuleLoad
                ) | (_, ::contracts::tool::ToolExecutionDescriptor::Script { .. })
            );
            ...
            return Ok(::contracts::ToolResult {
...
#[async_trait::async_trait]
impl ::contracts::SandboxBackend for ExecdSandboxBackend {
    fn name(&self) -> &str {
        "execd"
    }
    fn isolation_level(&self) -> ::contracts::IsolationLevel {
        ::contracts::IsolationLevel::Process
    }
    fn is_available(&self) -> bool {
        true
    }
    fn capabilities(&self) -> ::contracts::SandboxCapabilities {
        ::contracts::SandboxCapabilities {
...
mod tests {
    ...
    fn script_descriptor_rechecks_canonical_host_path() {
        ...
        let invocation = trusted_tool_invocation(
            &::contracts::tool::ToolExecutionDescriptor::Script {

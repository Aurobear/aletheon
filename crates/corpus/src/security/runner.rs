...
impl ToolRunnerWithGuard {
    ...
    /// underlying tool. This deliberately re-runs the current policy, loop and
    /// output guards so a cached value cannot bypass policy changes or suppress
    /// the per-call audit record. Callers must fail open to ordinary execution
    /// when this method rejects the hit.

...
impl DriverFactory {
    ...
    #[cfg(feature = "input")]
    pub fn try_input() -> Option<Box<dyn crate::driver::input::InputDriver>> {
        #[cfg(target_os = "linux")]
        {
            if std::path::Path::new("/dev/uinput").exists() {
                match crate::driver::input::UinputDriver::create() {
    ...
    #[cfg(feature = "display")]
    pub fn try_display() -> Option<Box<dyn crate::driver::display::DisplayDriver>> {
        match crate::driver::display::X11DisplayDriver::new() {
            Ok(d) => return Some(Box::new(d)),
            Err(e) => tracing::warn!("X11DisplayDriver failed: {e}"),
        }
        // Fallback to framebuffer for headless systems
        match crate::driver::display::FramebufferDriver::new() {
            Ok(d) => return Some(Box::new(d)),
    ...
    #[cfg(feature = "a11y")]
    pub fn try_a11y() -> Option<Box<dyn crate::driver::a11y::A11yDriver>> {
        // AT-SPI2 requires a D-Bus session bus
        if std::env::var("DBUS_SESSION_BUS_ADDRESS").is_ok() {
            match crate::driver::a11y::AtSpiDriver::new() {
                Ok(d) => return Some(Box::new(d)),
    ...
    pub fn try_ocr() -> Option<Box<dyn crate::driver::ocr::OcrDriver>> {
        match crate::driver::ocr::tesseract::TesseractOcrDriver::new() {
            Ok(d) => return Some(Box::new(d)),

...
use runtime::{
    ...
    PromptState, QueueOpResult, QueueSnapshot, MAX_QUEUE_LEN,
};
...
use ::contracts::{
    ConnectionId, EnvelopeV2, EnvelopeV2Delivery, EnvelopeV2Target, MessageId, NamespaceId,
    PrincipalId, SchemaId, ThreadId,
};
...
impl SessionInputCoordinator {
    /// Returns the injected durable queue port for composition-level recovery
    /// and contract verification.
    pub fn queue_store(&self) -> Arc<dyn PromptQueueStore> {
        self.store.clone()
    }
    ...
    pub async fn enqueue_with_target(
        &self,
        principal: PrincipalId,
        connection: ConnectionId,
        thread: ThreadId,
        ...
        execution_target.validate().map_err(anyhow::Error::msg)?;

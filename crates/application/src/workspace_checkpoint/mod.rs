...
pub use service::{
    ...
    WorkspaceCheckpointMetricSample, WorkspaceCheckpointMetrics, WorkspaceCheckpointPort,
    WorkspaceCheckpointService, WorkspaceLeasePort,
};
...
pub use crate::workspace_identity::WorkspaceIdentity;
...
pub trait WorkspaceFilePort: Send + Sync {
    async fn capture(
        &self,
        workspace: &WorkspaceIdentity,
    ...
    async fn protect_current(
        &self,
        workspace: &WorkspaceIdentity,
    ) -> anyhow::Result<Vec<CheckpointFileEntry>>;
    async fn restore(
        &self,
        workspace: &WorkspaceIdentity,
...
#[async_trait::async_trait]
pub trait WorkspaceCheckpointStore: Send + Sync {
    async fn begin(
        &self,
    ...
    async fn finalize(
        &self,
        id: CheckpointId,
        state: CheckpointFinalizeState,
    ) -> anyhow::Result<()>;
    async fn load(
        &self,
        session: &str,
        prompt_index: u64,
    ) -> anyhow::Result<Option<(TurnCheckpoint, Vec<CheckpointFileEntry>)>>;
    async fn load_by_id(
        &self,
        id: CheckpointId,
    ) -> anyhow::Result<Option<(TurnCheckpoint, Vec<CheckpointFileEntry>)>>;
    async fn list(&self, session: &str, limit: usize) -> anyhow::Result<Vec<TurnCheckpoint>>;
    async fn truncate_after(&self, session: &str, prompt_index: u64) -> anyhow::Result<()>;

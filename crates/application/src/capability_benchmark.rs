...
use ::contracts::cognitive_workflow::{AgentTaskPacket, CognitiveRoleOutput};
use ::contracts::{
...
use crate::evaluation_projection::{EvaluationProjectionRecord, EvaluationProjectionSink};
...
pub struct CapabilityReceiptRollup {
    ...
    #[serde(skip)]
    receipt_ids: HashSet<::contracts::EvaluationReceiptId>,
}
...
impl CapabilityRollupProjectionSink {
    ...
    /// Apply an immutable evaluation record to the in-memory materialized view.
    /// Persistence adapters replay durable records through this same method.
    pub fn observe_record(&self, record: &EvaluationProjectionRecord) {
        ...
        if matches!(
            ...
            ::contracts::EvaluationDecision::ObservedPass
                | ::contracts::EvaluationDecision::Accepted
        ) {
            rollup.pass_count += 1;
        }
...
impl runtime::RuntimePreferenceHistory for CapabilityRollupProjectionSink {
...
fn metrics_from_usage(usage: &::contracts::AttemptUsage) -> CapabilityBenchmarkMetrics {
...
mod tests {
    ...
    fn packet() -> AgentTaskPacket {
        ...
        AgentTaskPacket {
            ...
            projection_receipt: AgoraProjectionReceipt {
                ...
                space: ::contracts::AgoraSpaceId("benchmark".into()),
                workspace_version: 1,
    ...
    async fn capability_rollup_uses_receipt_quality_and_separate_usage_dimensions() {
        ...
        let receipt_id = ::contracts::EvaluationReceiptId::new();
        let record = EvaluationProjectionRecord {
            receipt: ::contracts::EvaluationReceiptRef {
                ...
                decision: ::contracts::EvaluationDecision::ObservedPass,
            ...
            context: crate::evaluation_projection::EvaluationProjectionContext {
                ...
                rubric_version: 2,
                process_id: ProcessId::new(),
                metrics: crate::evaluation_projection::EvaluationProjectionMetrics {
                    elapsed_ms: Some(100),

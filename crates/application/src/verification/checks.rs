...
use super::{
    ArchitecturePolicy, VerificationCheckKind, VerificationContext, VerificationWorkspaceReader,
};
use ::contracts::{ChangedFileKind, VerificationCheck};
...
pub fn architecture_review(
    ...
    workspace: &dyn VerificationWorkspaceReader,
) -> VerificationCheck {
    ...
    for changed in &context.changed_files {
        ...
        let source = match workspace.read_utf8(&context.worktree, path) {
            Ok(source) => source,
            Err(_) => {
                findings.push(format!(

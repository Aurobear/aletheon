...
use ::contracts::{
    LlmProvider, LlmResponse, LlmStream, Message, ModelRuntimeFacts, ToolDefinition,
};
...
pub trait InferencePort: Send + Sync {
    ...
    async fn provider_backpressure_metrics(
        &self,
    ) -> Result<HashMap<String, crate::inference::ProviderBackpressureSnapshot>, InferenceError>
    {
...
#[async_trait::async_trait]
impl ::contracts::memory::ProviderBackpressurePort for InferenceProviderBackpressure {
    async fn acquire(
        &self,
        provider_key: &str,
    ) -> anyhow::Result<Box<dyn ::contracts::memory::ProviderRequestPermit>> {
        self.inference

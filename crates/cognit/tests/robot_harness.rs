...
fn terminal_states_dont_transition() {
    ...
    assert_eq!(
        RobotState::Completed.next(&VerificationSignal::Matched),
        RobotState::Completed
    );
    assert_eq!(
        RobotState::Failed.next(&VerificationSignal::Unsafe),
        RobotState::Failed
    );
}
...
fn normal_path_observe_to_completed() {
    ...
    s = s.next(&VerificationSignal::Matched);
    assert_eq!(s, RobotState::Verify);
    s = s.next(&VerificationSignal::Matched);
    assert_eq!(s, RobotState::Settle);
    s = s.next(&VerificationSignal::Matched);
    assert_eq!(s, RobotState::Completed);

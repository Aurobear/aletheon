...
pub(super) async fn run_agent(
                ...
                Ok(resources) => {
                    ...
                    match engine.settle(request, resources).await {
                        ...
                        Err(error) => {
                            ...
                            authoritative_terminal = Some(fabric::SettlementTerminal::Failed {
    ...
    let authoritative_terminal =
        authoritative_terminal.unwrap_or_else(|| fabric::SettlementTerminal::Failed {
    ...
    let (next, result, error, process_exit) = match authoritative_terminal {
        fabric::SettlementTerminal::Completed => (
            AgentRunStatus::Succeeded,
            result,
            None,
            ExitReason::Completed,
        ),
        fabric::SettlementTerminal::Cancelled => (
            AgentRunStatus::Cancelled,
            None,
            error.or_else(|| Some("Agent runtime cancelled".into())),
            ExitReason::Cancelled("Agent runtime cancelled".into()),
        ),
        fabric::SettlementTerminal::Failed { reason } => (
            AgentRunStatus::Failed,
            None,
            Some(reason.clone()),
            ExitReason::Failed(reason),
        ),
        fabric::SettlementTerminal::Recoverable => (

...
pub struct ProductionSessionLifecycle {
    ...
    approvals: crate::application::admin_service::ScopedApprovalCache,
    cancel_token: Arc<Mutex<Option<CancellationToken>>>,
    lifecycle: Arc<crate::application::lifecycle_contributors::LifecycleRegistry>,
...
impl ProductionSessionLifecycle {
    ...
    async fn dispatch(
        &self,
        phase: crate::application::lifecycle_contributors::LifecyclePhase,
        session_id: &str,
    ) -> anyhow::Result<()> {
        let dispatch = self
            .lifecycle
...
fn fabric_lifecycle_phase(
    ...
    use crate::application::lifecycle_contributors::LifecyclePhase as ExecutivePhase;
...
impl SessionLifecycleUseCases for ProductionSessionLifecycle {
    ...
    async fn finish(&self, session_id: String, turn_count: usize) -> anyhow::Result<()> {
        self.dispatch(
            crate::application::lifecycle_contributors::LifecyclePhase::BeforeSessionEnd,
            &session_id,
        )
        .await?;
        ...
        self.dispatch(
            crate::application::lifecycle_contributors::LifecyclePhase::AfterSessionEnd,
            &session_id,
        )
        .await?;
        Ok(())
    ...
    async fn start(&self, session_id: String, clear_approvals: bool) -> anyhow::Result<()> {
        self.dispatch(
            crate::application::lifecycle_contributors::LifecyclePhase::BeforeSessionStart,
            &session_id,
        )
        .await?;
        if clear_approvals {
        ...
        self.corpus.execute_hook(&context).await;
        self.dispatch(
            crate::application::lifecycle_contributors::LifecyclePhase::AfterSessionStart,
            &session_id,
        )
        .await?;
        Ok(())

...
use fabric::{
    AgoraService, AgoraSpaceId, EvaluationDecision, EvaluationReceiptRef, ProcessId, TaskSettlement,
};

use super::cognitive_workspace::CognitiveWorkspaceCoordinator;
...
mod tests {
    use super::*;
    use fabric::cognitive_workflow::{CognitiveRole, CognitiveRoleProfile, CognitiveTaskNode};
    use fabric::{AgoraService, EvaluationContractId, EvaluationReceiptId};

    #[tokio::test]
    async fn u_verify_002_review_finding_modifies_the_canonical_plan_graph() {
        let clock: Arc<dyn fabric::Clock> = Arc::new(kernel::chronos::TestClock::new(1, 1));
        let agora = Arc::new(agora::AgoraRegistry::new(clock));

...
use fabric::{EvaluationReceiptId, EvaluationReceiptRef, ItemPayload, PrincipalId, SessionId};
...
fn has_session_authority(
    store: &crate::application::thread_authority::ThreadAuthorityStore,
    principal_id: &PrincipalId,
    session_id: &SessionId,
) -> Result<bool, crate::application::thread_authority::ThreadAuthorityError> {
    let authority = crate::application::thread_authority::ThreadAuthorityKey::new(
        principal_id.clone(),
...
fn bounded_evidence(
    evidence: Vec<fabric::types::metacognition_evidence::EvidenceItem>,
) -> (
    Vec<fabric::types::metacognition_evidence::EvidenceItem>,
    bool,
) {
...
mod tests {
    ...
    fn item(index: u128) -> fabric::ItemRecord {
        fabric::ItemRecord {
            schema_version: fabric::SESSION_SCHEMA_VERSION,
            id: fabric::ItemId(uuid::Uuid::from_u128(index + 10_000)),
            session_id: SessionId("session-a".into()),
            turn_id: fabric::TurnId(uuid::Uuid::from_u128(1)),
    ...
    fn evaluation_session_authority_is_principal_scoped() {
        use crate::application::thread_authority::{
        ...
        store
            .bind_or_verify(
                &ThreadAuthorityKey::new(principal.clone(), fabric::ThreadId(session_id.0.clone())),
                &ThreadSettings {
                    ...
                    .unwrap(),
                    permission_profile: fabric::PermissionProfileId::workspace_write(),
                    approval_policy: fabric::ApprovalPolicy::OnRequest,
                    model_policy: None,
                },
            )
            .unwrap();

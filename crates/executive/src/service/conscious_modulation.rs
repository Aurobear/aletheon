...
mod tests {
    ...
    fn negate_high_risk_low_relevance_vetoes() {
        let (rec, _) = recommend(RiskLevel::Destructive, Some(CareActionKind::Negate), 0.1, 0.5);
    ...
    fn deliberate_urgent_relevant_reorders() {
        let (rec, _) = recommend(RiskLevel::Sandboxed, Some(CareActionKind::Deliberate), 0.8, 0.9);
    ...
    fn direct_always_proceeds_even_at_max_risk() {
        ...
        let (rec, _) = recommend(RiskLevel::Destructive, Some(CareActionKind::Direct), 0.9, 0.9);

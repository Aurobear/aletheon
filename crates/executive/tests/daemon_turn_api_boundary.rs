...
fn daemon_turn_composition_resources_do_not_escape_the_crate() {
    let orchestrator = source("src/application/daemon_turn/orchestrator.rs");
    ...
    let module = source("src/application/daemon_turn/mod.rs");
...
fn daemon_turn_resources_are_constructed_only_in_bootstrap() {
    ...
    for root in ["src/application", "src/core", "src/host"] {
        ...
        while let Some(directory) = stack.pop() {
            for entry in fs::read_dir(directory).expect("source directory") {
                ...
                if path.is_dir() {
                    ...
                    && !path.starts_with("src/host/daemon/bootstrap")
                {

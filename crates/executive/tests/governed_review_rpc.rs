use executive::application::governed_review::{GovernedReviewLimits, ReviewCapabilities};
use executive::composition::config::{AppConfig, GovernedReviewSettings};

#[test]
fn rpc_dispatch_exposes_exact_governed_lifecycle_methods() {
    let dispatch = include_str!("../src/host/daemon/handler/rpc.rs");
    for method in [
        "review.capabilities",
        "review.submit",
        "review.status",
    ...
    let handler = include_str!("../src/host/daemon/handler/rpc/rpc_review.rs");

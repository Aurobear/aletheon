use executive::application::daemon_turn_engine::map_turn_execution;
use executive::application::turn_coordinator::TurnExecution;

fn execution(stop: fabric::TurnStop, items: Vec<fabric::ItemPayload>) -> TurnExecution {
    let usage = fabric::InferenceUsage::aggregate(items.iter().filter_map(|item| match item {
    ...
    TurnExecution {
        result: fabric::TurnResult {
            output: format!("{stop:?}"),
            failure: (stop == fabric::TurnStop::Failed).then(|| fabric::TurnFailure {
                kind: fabric::TurnFailureKind::Runtime,
...
fn inference_receipt(inference_id: &str, usage: fabric::InferenceUsage) -> fabric::ItemPayload {
    fabric::ItemPayload::InferenceReceipt {
        receipt: fabric::types::inference_receipt::InferenceTerminalReceipt {
            schema_version: fabric::types::inference_receipt::INFERENCE_TERMINAL_RECEIPT_SCHEMA_V1,
...
fn turn_result_stop_mapping_is_exhaustive_and_lossless() {
    for stop in [
        ...
        assert_eq!(mapped.failure.is_some(), stop == fabric::TurnStop::Failed);
    }
}
    ...
    let mapped = map_turn_execution(
        ...
        execution(
            ...
            vec![
                inference_receipt(
                    "inference-1",
                    fabric::InferenceUsage::reported(100, 20, Some(75), Some(25), Some(5)),
                ),
                inference_receipt(
                    "inference-2",
                    fabric::InferenceUsage::reported(60, 10, Some(50), Some(10), Some(2)),
                ),
            ],
        ),
    );

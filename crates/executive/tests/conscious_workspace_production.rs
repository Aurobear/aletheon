...
async fn production_registry_traces_user_observation_action_and_outcome() {
    ...
    let selected = action_loop.select_action(&call, fabric::RiskLevel::ReadOnly).await.unwrap();
    ...
    let child_action = child_loop.select_action(&child_call, fabric::RiskLevel::ReadOnly).await.unwrap();

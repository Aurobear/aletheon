...
use executive::application::extension_snapshot::ExtensionSnapshotCompiler;
...
async fn runtime_view_publishes_complete_snapshots() {
    let view = executive::application::extension_snapshot::ExtensionRuntimeView::default();
    let before = view.load().await;
    let mut replacement =
        executive::application::extension_snapshot::ExtensionRuntimeSnapshot::empty();

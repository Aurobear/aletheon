...
fn range_inclusive_bounds() {
    ...
    assert_eq!(
        outcome_verifier::evaluate(&eo, None, Some(&after), 1).decision,
    ...
    assert_eq!(
        outcome_verifier::evaluate(&eo, None, Some(&after2), 1).decision,
        VerificationDecision::Matched
    );
}
...
fn change_delta_detected() {
    ...
    assert_eq!(
        outcome_verifier::evaluate(&eo, Some(&before), Some(&after), 1).decision,
        VerificationDecision::Matched
    );
}

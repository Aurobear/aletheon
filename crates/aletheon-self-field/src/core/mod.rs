...
#[async_trait]
impl aletheon_abi::SelfFieldOps for SelfField {
    ...
    async fn review(&self, intent: &Intent, ctx: &Context) -> Result<Verdict> {
        ...
        // 2. Policy check (argos-security PolicyEngine)
        ...
        if care_score > 0.8 {
            ...
            if ctx.permissions.max_level() < aletheon_abi::PermissionLevel::SystemChange {

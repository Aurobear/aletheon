...
use aletheon_abi::{Conflict, Resolution};
    ...
    pub fn resolve(&self, conflict: &Conflict) -> Resolution {
        ...
        match priority_a.cmp(&priority_b) {
            std::cmp::Ordering::Greater => Resolution::AcceptA {
                reason: format!("{:?} has higher priority", self.source_name(&conflict.source_a)),
            },
            std::cmp::Ordering::Less => Resolution::AcceptB {
                reason: format!("{:?} has higher priority", self.source_name(&conflict.source_b)),
            },
...
mod tests {
    ...
    fn user_wins() {
        ...
        let conflict = Conflict {
            ...
            source_b: ConflictSource::Brain { proposal: "do Y".to_string(), confidence: 0.9 },
            context: test_ctx(),
        };
    ...
    fn body_critical_wins() {
        let layer = ConflictLayer::new();
        let conflict = Conflict {
            source_a: ConflictSource::User { intent: "proceed".to_string() },
    ...
    fn brain_confidence_wins_over_memory() {
        let layer = ConflictLayer::new();
        let conflict = Conflict {
            source_a: ConflictSource::Brain { proposal: "approach A".to_string(), confidence: 0.95 },
            source_b: ConflictSource::Memory { evidence: "previously used B".to_string() },
            context: test_ctx(),
        };
        let resolution = layer.resolve(&conflict);
        assert!(matches!(resolution, Resolution::AcceptA { .. }));
    ...
    fn equal_priority_compromise() {
        let layer = ConflictLayer::new();
        let conflict = Conflict {
            source_a: ConflictSource::Memory { evidence: "fact A".to_string() },
            source_b: ConflictSource::Self_ { concern: "concern B".to_string() },
            context: test_ctx(),
        };

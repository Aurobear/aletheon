...
impl MemoryBackend for ProceduralMemory {
            ...
            let existing_version: Option<i64> = conn
                .query_row(
                    "SELECT MAX(pe.version) FROM procedural_entries pe
                     INNER JOIN aletheon_memory m ON m.id = pe.memory_id
    ...
    async fn list(&self, filter: &MemoryFilter) -> Result<Vec<MemoryEntry>> {
        self.with_conn(|conn| {
            let mut sql = String::from(
                "SELECT * FROM aletheon_memory WHERE memory_type = 'procedural' AND importance > 0",
            );
            let mut param_values: Vec<Box<dyn rusqlite::types::ToSql>> = Vec::new();
            let mut param_idx = 1;
    ...
    async fn forget(&self, handle: &MemoryHandle) -> Result<()> {
        ...
        self.with_conn(|conn| {
            conn.execute(
                "UPDATE aletheon_memory SET importance = 0.0 WHERE id = ?1",
    ...
    async fn compact(&self, strategy: CompactStrategy) -> Result<CompactResult> {
        self.with_conn(|conn| {
            let before: i64 = conn.query_row(
                "SELECT COUNT(*) FROM aletheon_memory WHERE memory_type = 'procedural'",
                [],
                |r| r.get(0),
            )?;
            ...
            match strategy {
                CompactStrategy::KeepTopN { n } => {
                    let ids_to_remove: Vec<String> = conn
                        .prepare(
                            "SELECT m.id FROM aletheon_memory m
                             INNER JOIN procedural_entries pe ON pe.memory_id = m.id
                ...
                CompactStrategy::PruneBelowImportance { threshold } => {
                    conn.execute(
                        "DELETE FROM aletheon_memory WHERE memory_type = 'procedural' AND importance < ?1",
                        params![threshold],
                    )?;
                    conn.execute(
                        "DELETE FROM procedural_entries WHERE memory_id NOT IN (SELECT id FROM aletheon_memory)",
                        [],
                    )?;
                }
                CompactStrategy::AgeBased {
                    ...
                    conn.execute(
                        "DELETE FROM aletheon_memory WHERE memory_type = 'procedural'
    ...
    async fn stats(&self) -> Result<MemoryStats> {
        self.with_conn(|conn| {
            ...
            let total_size: i64 = conn
                .query_row(
                    "SELECT COALESCE(SUM(LENGTH(content)), 0) FROM aletheon_memory WHERE memory_type = 'procedural'",
                    [],
                    |r| r.get(0),
                )
                .unwrap_or(0);
            let oldest: Option<String> = conn
                .query_row(
                    "SELECT MIN(created_at) FROM aletheon_memory WHERE memory_type = 'procedural'",
                    [],
                    |r| r.get(0),
                )
                .unwrap_or(None);
            let newest: Option<String> = conn
                .query_row(
                    "SELECT MAX(created_at) FROM aletheon_memory WHERE memory_type = 'procedural'",
                    [],
                    |r| r.get(0),
                )
                .unwrap_or(None);
...
mod tests {
    ...
    async fn test_procedural_soft_forget() {
        ...
        // Still counted in stats (aletheon_memory row still exists)
        let stats = mem.stats().await.unwrap();

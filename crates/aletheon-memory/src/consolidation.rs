...
use aletheon_abi::{MemoryBackend, MemoryEntry, MemoryType};
...
pub async fn consolidate(
    ...
    let mut qualified: Vec<(&aletheon_abi::ReflectionEntry, u64, &str)> = Vec::new();
...
fn extract_knowledge(reflection: &aletheon_abi::ReflectionEntry) -> ExtractedKnowledge {
...
mod tests {
    ...
    async fn store_reflection_with_access(
        ...
        if let Some(memory_id) = memory_ids.first() {
            ep.with_conn(|conn| {
                conn.execute(
                    "UPDATE aletheon_memory SET access_count = ?1 WHERE id = ?2",
    ...
    async fn test_promoted_entries_appear_in_semantic() {
        ...
        let query = aletheon_abi::MemoryQuery {

        ...
        self.with_conn(|conn| {
            ...
            if let Some(parent_uuid) = entry.associations.first() {
                let parent_exists: bool = conn
                    .query_row(
                        "SELECT COUNT(*) > 0 FROM aletheon_memory WHERE id = ?1 AND memory_type = 'self'",
    ...
    async fn recall(&self, query: &MemoryQuery) -> Result<Vec<MemoryEntry>> {
        self.with_conn(|conn| {
            let mut sql = String::from(
                "SELECT m.* FROM aletheon_memory m WHERE m.memory_type = 'self'",
            );
    ...
    async fn compact(&self, _strategy: CompactStrategy) -> Result<CompactResult> {
        ...
        self.with_conn(|conn| {
            let count: i64 = conn.query_row(
                "SELECT COUNT(*) FROM aletheon_memory WHERE memory_type = 'self'",
                [],
                |r| r.get(0),
            )?;
    ...
    async fn stats(&self) -> Result<MemoryStats> {
        self.with_conn(|conn| {
            ...
            let total_size: i64 = conn
                .query_row(
                    "SELECT COALESCE(SUM(LENGTH(content)), 0) FROM aletheon_memory WHERE memory_type = 'self'",
                    [],
                    |r| r.get(0),
                )
                .unwrap_or(0);
            let oldest: Option<String> = conn
                .query_row(
                    "SELECT MIN(created_at) FROM aletheon_memory WHERE memory_type = 'self'",
                    [],
                    |r| r.get(0),
                )
                .unwrap_or(None);
            let newest: Option<String> = conn
                .query_row(
                    "SELECT MAX(created_at) FROM aletheon_memory WHERE memory_type = 'self'",
                    [],
                    |r| r.get(0),
                )
                .unwrap_or(None);

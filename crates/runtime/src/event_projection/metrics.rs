...
use crate::{EventPayload, SpineEvent};
use ::contracts::{ItemPayload, ItemRecord};
...
impl EventProjection for MetricsProjection {
    ...
    fn descriptor(&self) -> ProjectionDescriptor {
        ProjectionDescriptor {
            name: "event-metrics",
            version: 1,
            accepted_schemas: super::debug::ALL_EVENT_SCHEMAS,
        }
    }

    fn apply(&self, state: &mut Self::State, event: &SpineEvent) -> Result<(), ProjectionError> {
        state.event_count += 1;
        if event.schema.0 == ::contracts::SchemaId::EVENT_TOOL_OBSERVATION_V1 {
            state.tool_calls += 1;
            if inline(event)
        ...
        if event.schema.0 == ::contracts::SchemaId::TURN_EVENT_V1 {

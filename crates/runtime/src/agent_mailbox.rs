...
use ::contracts::ipc::envelope_v2::{EnvelopeV2, SchemaId};
use ::contracts::{
    AgentControlError, AgentControlErrorKind, AgentMessageKind, AgentMessagePayload,
};
...
#[cfg(test)]
mod tests {
    use super::AgentRuntimeInbox;

    #[test]
    fn bounded_inbox_rejects_zero_capacity() {
        assert!(AgentRuntimeInbox::bounded_channel(0).is_err());
        assert!(AgentRuntimeInbox::bounded_channel(1).is_ok());
    }
}

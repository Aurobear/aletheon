...
use ::contracts::ipc::stream::{TurnEventSender, TurnEventV1};
use ::contracts::types::tool::ToolResult;
use ::contracts::types::tool_stream::{
...
mod tests {
    use super::*;
    use ::contracts::ipc::stream::TurnEventStream;
    use ::contracts::types::tool::{ToolResult, ToolResultMeta};
    use ::contracts::types::tool_stream::{ToolNotification, ToolNotificationKind};
    use proptest::prelude::*;

    fn turn_stream(_capacity: usize) -> (TurnEventStream, TurnEventSender) {
        TurnEventStream::new()
    }
    ...
    async fn governed_notification_reaches_lifecycle_observer_once() {
        let (mut sink, rx) = ::contracts::tool_event_channel_for_call("call-notify");
    ...
    async fn bound_stream_call_id_mismatch_fails_closed() {
        let (mut sink, rx) = ::contracts::tool_event_channel_for_call("call-a");
        sink.terminal(Ok(result())).await;
        let (_stream, sender) = turn_stream(1);

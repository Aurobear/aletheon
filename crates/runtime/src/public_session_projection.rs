...
use crate::{EventPayload, EventVisibility, SpineEvent};
use ::contracts::types::episode_report::{EpisodeReport, EpisodeSettlement, SettledEpisodeReport};
use ::contracts::types::outcome_verification::VerificationDecision;
use ::contracts::types::robot_failure::RobotFailureClass;
use ::contracts::{
    ActivityKind, ActivitySnapshot, ActivityState, ItemPayload, ItemRecord, SessionForkedEvent,
    SessionId, SessionPrincipalBoundEvent, SessionRecord, SessionStatus, TaskPhase,
    TaskRuntimeFacts, TaskSettlement, TaskSnapshot, TaskStepSnapshot, SESSION_SCHEMA_VERSION,
};
use serde::{de::DeserializeOwned, Deserialize, Serialize};

use crate::read_model::{EventProjection, ProjectionDescriptor, ProjectionError};
use crate::session_projection::SessionProjectionStore;
...
impl SessionProjection {
    ...
    pub async fn materialize(
        ...
        match event.schema.0.as_str() {
            ::contracts::SchemaId::EVENT_SESSION_CREATED_V1 => {
            ...
            ::contracts::SchemaId::EVENT_SESSION_PRINCIPAL_BOUND_V1 => {
...
fn project_evaluation_settlement(
...
) -> (Option<TaskSettlement>, Vec<::contracts::ReviewFinding>) {
    use ::contracts::{EvaluationDecision, ReviewFindingSeverity, ReviewFindingStatus};
    ...
    let mut findings = receipt
        ...
        .iter()
        .enumerate()
        .map(|(index, gate)| ::contracts::ReviewFinding {
    ...
    let settlement = match receipt.decision {
        ...
        | EvaluationDecision::ObservedFail => {
            if findings.is_empty() {
                findings.push(::contracts::ReviewFinding {
...
fn turn_phase(items: &[&ItemRecord]) -> TaskPhase {
    ...
    if let Some(classification) = items.iter().rev().find_map(|item| match &item.payload {
        ...
        return match classification {
            ::contracts::TurnRecoveryClassification::Interrupted => TaskPhase::Interrupted,
            ::contracts::TurnRecoveryClassification::Failed => TaskPhase::Failed,
        };
    }
    if let Some(status) = items.iter().rev().find_map(|item| match &item.payload {
        ...
        return match status {
            ::contracts::TurnTerminalStatus::Completed => TaskPhase::Completed,
            ::contracts::TurnTerminalStatus::Interrupted => TaskPhase::Interrupted,
            ::contracts::TurnTerminalStatus::Failed => TaskPhase::Failed,
        };
    }
    ...
    // A failed capability is an activity terminal, not a Turn terminal. The
    // cognitive loop may inspect that result, correct its arguments, and still
    // complete the Turn. Only the authoritative TurnSettlement/recovery facts
    // above, or an unrecovered terminal inference failure, settle this step.
    if items.iter().any(|item| match &item.payload {
        ItemPayload::InferenceReceipt { receipt } => {
            receipt.status == ::contracts::types::inference_receipt::InferenceTerminalStatus::Failed
        }
        _ => false,
    }) {
            ...
            ItemPayload::ToolResult {
                ...
                if *is_error {
                    activity.progress = Some(serde_json::json!({
                        "error": bounded_activity_error(content, 512),
                    }));
                }
            }
            ItemPayload::CapabilityReceipt { receipt } => {
                let state = match receipt.status {
                    ::contracts::CapabilityTerminalStatus::Succeeded => ActivityState::Completed,
                    ::contracts::CapabilityTerminalStatus::Failed
                    | ::contracts::CapabilityTerminalStatus::TimedOut => ActivityState::Failed,
                    ::contracts::CapabilityTerminalStatus::Cancelled => ActivityState::Cancelled,
                };
...
fn bounded_activity_error(value: &str, max_chars: usize) -> String {
...
fn project_runtime_facts(items: &[ItemRecord]) -> Option<TaskRuntimeFacts> {
    ...
    let sum = |select: fn(&::contracts::InferenceUsage) -> Option<u64>| {
    ...
    let cache_telemetry = if receipts.is_empty() {
        ...
        .iter()
        .all(|receipt| receipt.usage.cache_telemetry == ::contracts::CacheTelemetry::Reported)
    {
        ::contracts::CacheTelemetry::Reported
    } else if receipts
        .iter()
        .all(|receipt| receipt.usage.cache_telemetry == ::contracts::CacheTelemetry::Unsupported)
    {
        ::contracts::CacheTelemetry::Unsupported
    } else {
        ::contracts::CacheTelemetry::Unknown
    };
    let cache_known = cache_telemetry == ::contracts::CacheTelemetry::Reported;
    Some(TaskRuntimeFacts {
        ...
        cumulative_usage: ::contracts::InferenceUsage {
            total_input_tokens,
            output_tokens,
...
mod tests {
    ...
    fn context_budget_projection() -> ::contracts::ContextBudgetProjection {
    ...
    fn robot_receipt(safety_denied: bool) -> SettledEpisodeReport {
        let expected = ::contracts::types::expected_outcome::ExpectedOutcome {
        ...
        let attempt = ::contracts::types::episode_report::AttemptRecord {
            attempt: 1,
            attempt_id: "attempt-1".into(),
            operation_id: (!safety_denied).then(|| "operation-1".into()),
            request: Some(::contracts::types::embodiment::SkillRequest {
            ...
            evidence_refs: if safety_denied {
                ...
                vec![::contracts::types::embodiment::EvidenceRef {
        ...
        let failure = safety_denied.then(|| {
            ::contracts::types::robot_failure::RobotFailure::new(
                RobotFailureClass::ExecutionRejected,
                "Safety Supervisor denied high-risk hardware permit",
            )
        });
        let report = ::contracts::types::episode_report::build_report(
            ::contracts::types::episode_report::EpisodeReportInput {
                ...
                safe_stop: safety_denied.then_some(
                    ::contracts::types::episode_report::SafeStopReceipt {
                ...
                selected_frames: if safety_denied {
                    ...
                    vec![::contracts::types::frame::FrameRef {
                        uri: format!("artifact://sha256/{FRAME_DIGEST}"),
    ...
    fn u_input_004_shell_receipt_projects_a_terminal_command_activity() {
        ...
        let item = ItemRecord {
            ...
            payload: ItemPayload::CapabilityReceipt {
                receipt: ::contracts::CapabilityTerminalReceipt {
                    ...
                    started_at: ::contracts::MonoTime(10),
                    finished_at: ::contracts::MonoTime(20),
                    exit_code: Some(0),
                    error_class: None,
                    artifact_ids: vec![],
                    evidence_ids: vec![],
    ...
    #[test]
    fn failed_tool_projects_a_bounded_public_error_reason() {
        let session_id = SessionId("failed-tool-session".into());
        let turn_id = ::contracts::TurnId::new();
        let record = |sequence, payload| ItemRecord {
            schema_version: SESSION_SCHEMA_VERSION,
            id: ::contracts::ItemId::new(),
        ...
        let items = vec![
            ...
            record(
                ...
                ItemPayload::ToolResult {
                    call_id: "spawn-1".into(),
                    content: format!("capacity {}", "x".repeat(700)),
                    is_error: true,
                    permit_id: None,
                    audit_id: None,
                },
        ...
        let activities = project_activities("task", &items);
        let error = activities[0]
            .progress
            .as_ref()
            .and_then(|progress| progress.get("error"))
            .and_then(serde_json::Value::as_str)
            .unwrap();
        assert_eq!(activities[0].state, ActivityState::Failed);
        assert_eq!(error.chars().count(), 512);
        assert!(error.ends_with('…'));
        assert_eq!(
            turn_phase(&items.iter().collect::<Vec<_>>()),
            TaskPhase::Active
        );
    }
}
...
impl EventProjection for SessionProjection {
    ...
    fn apply(&self, state: &mut Self::State, event: &SpineEvent) -> Result<(), ProjectionError> {
        ...
        match event.schema.0.as_str() {
            ::contracts::SchemaId::EVENT_SESSION_CREATED_V1 => {
                Self::apply_session_created(state, event)
            }
            ::contracts::SchemaId::EVENT_SESSION_FORKED_V1 => {
                Self::apply_session_forked(state, event)
            }
            ::contracts::SchemaId::EVENT_SESSION_PRINCIPAL_BOUND_V1 => Ok(()),
            ::contracts::SchemaId::TURN_EVENT_V1 => Self::apply_item(state, event),
            _ => Ok(()),
        }
    }
}
...
fn is_legacy_evaluation_projection_event(event: &SpineEvent) -> bool {
    event.schema.0 == ::contracts::SchemaId::TURN_EVENT_V1
...
mod host_settlement_tests {
    ...
    ) -> ::contracts::EvaluationReceiptRef {
        ::contracts::EvaluationReceiptRef {
    ...
    fn failed_tests_override_model_completion_claims() {
        ...
        assert_eq!(findings[0].status, ::contracts::ReviewFindingStatus::Open);
        assert!(findings[0]
            .repair_link
            .as_deref()
            .unwrap()

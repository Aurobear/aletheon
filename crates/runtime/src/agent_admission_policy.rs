//! Runtime-owned bounded Agent admission policy (RA-05).

use std::collections::{HashMap, VecDeque};
use std::sync::Arc;
use std::time::Duration;

use ::contracts::BudgetController;
use ::contracts::{
    AgentControlError, AgentControlErrorKind, AgentId, AttemptUsage, BudgetRequest,
    BudgetReservationId, BudgetScopeId, BudgetScopeKind, BudgetTransferReceipt, UsageReport,
};
...
/// Runtime-owned policy values. Host configuration adapters convert their
/// external configuration into this contract before composition.
...
impl Default for AgentAdmissionPolicy {
    fn default() -> Self {
        Self {
...
impl AgentAdmissionPolicy {
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        if let Some(child) = self.max_child_cost_micro {
            let root = self.root_max_cost_micro.ok_or_else(|| {
                anyhow::anyhow!(
                    "finite child cost allowance requires a finite root rollout allowance"
                )
            })?;
            anyhow::ensure!(
                child <= root,
...
/// Bounded, fair, root-scoped admission implementation. Runtime owns the
/// policy state machine; the budget controller remains an injected port.
pub struct BoundedAgentAdmission {
    config: AgentAdmissionPolicy,

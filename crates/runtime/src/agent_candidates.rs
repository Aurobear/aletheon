//! Runtime-owned Agent candidate submission contract (RA-05).
//!
//! Agent lifecycle observations may be projected into a conscious workspace,
//! but the projection contract must not be owned by Executive's orchestration
//! module. Runtime owns the bounded candidate envelope and admission port;
//! Agora/conscious implementations remain host/application adapters.

use ::contracts::dasein::{SelfEventId, SelfVersion};
use ::contracts::{BroadcastEpoch, ProcessorId, WorkspaceCandidate};
use async_trait::async_trait;

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum CandidateCause {
    ExternalObservation {
        event_ref: String,
    },
    ProcessorResponse {
        processor: ProcessorId,
        source_epoch: BroadcastEpoch,
    },
    DaseinTransition {
        event_id: SelfEventId,
        version: SelfVersion,
    },
    GovernedActionOutcome {
        permit_id: String,
        operation_id: ::contracts::OperationId,
    },
    GovernedActionProposal {
        operation_id: ::contracts::OperationId,
        call_id: String,
    },
    AgentRuntimeEvent {
        agent_id: ::contracts::AgentId,
        operation_id: ::contracts::OperationId,
        event_kind: String,
    },
}

impl CandidateCause {
    pub fn required_source_ref(&self) -> String {
        match self {
            Self::ExternalObservation { event_ref } => event_ref.clone(),
            Self::ProcessorResponse {
                processor: _,
                source_epoch,
            } => format!("broadcast_epoch:{}", source_epoch.0),
            Self::DaseinTransition { event_id, version } => {
                format!("dasein:{}:v{}", event_id.0, version.0)
            }
            Self::GovernedActionOutcome {
                permit_id,
                operation_id,
            } => format!("capability:{permit_id}:{}", operation_id.0),
            Self::GovernedActionProposal {
                operation_id,
                call_id,
            } => format!("action-call:{}:{call_id}", operation_id.0),
            Self::AgentRuntimeEvent {
                agent_id,
                operation_id,
                event_kind,
            } => format!(
                "agent-runtime:{}:{}:{event_kind}",
                agent_id.0, operation_id.0
            ),
        }
    }

    pub fn validate(&self) -> anyhow::Result<()> {
        match self {
            Self::ExternalObservation { event_ref } => anyhow::ensure!(
                !event_ref.trim().is_empty() && event_ref.len() <= 32 * 1024,
                "external event reference is invalid"
            ),
            Self::ProcessorResponse { processor, .. } => processor.validate()?,
            Self::DaseinTransition { .. } => {}
            Self::GovernedActionOutcome { permit_id, .. } => anyhow::ensure!(
                !permit_id.trim().is_empty() && permit_id.len() <= 1024,
                "capability permit reference is invalid"
            ),
            Self::GovernedActionProposal { call_id, .. } => anyhow::ensure!(
                !call_id.trim().is_empty() && call_id.len() <= 1024,
                "governed action call reference is invalid"
            ),
            Self::AgentRuntimeEvent { event_kind, .. } => anyhow::ensure!(
                !event_kind.trim().is_empty() && event_kind.len() <= 128,
                "Agent runtime event kind is invalid"
            ),
        }
        Ok(())
    }
}

#[derive(Debug, Clone)]
pub struct CandidateSubmission {
    pub candidate: WorkspaceCandidate,
    pub cause: CandidateCause,
}

impl CandidateSubmission {
    pub fn validate(&self) -> anyhow::Result<()> {
        self.candidate.validate()?;
        self.cause.validate()?;
        let required = self.cause.required_source_ref();
        match &self.cause {
            CandidateCause::ProcessorResponse {
                processor,
                source_epoch,
            } => {
                let full = format!("broadcast:{}:{}", self.candidate.space.0, source_epoch.0);
                anyhow::ensure!(
                    self.candidate.provenance.source_refs.contains(&full),
                    "processor submission lacks source broadcast reference"
                );
                anyhow::ensure!(
                    self.candidate
                        .provenance
                        .source_refs
                        .contains(&format!("processor:{}", processor.0)),
                    "processor submission lacks processor identity reference"
                );
            }
            _ => anyhow::ensure!(
                self.candidate.provenance.source_refs.contains(&required),
                "candidate submission lacks its causal source reference"
            ),
        }
        Ok(())
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum CandidateAdmissionStatus {
    Accepted,
    Duplicate,
    RejectedCapacity,
    RejectedSourceQuota,
    RejectedWrongSpace,
    RejectedInvalid,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CandidateSubmissionReceipt {
    pub candidate_id: ::contracts::ContentId,
    pub status: CandidateAdmissionStatus,
    pub detail: Option<String>,
}

/// Runtime-to-conscious projection port. Runtime may submit a candidate to an
/// adapter, but it never selects, commits, or broadcasts that candidate.
#[async_trait]
pub trait AgentCandidateSubmissionPort: Send + Sync {
    async fn submit_agent_candidate(
        &self,
        submission: CandidateSubmission,
        recipient: ::contracts::ProcessId,
        root: ::contracts::ProcessId,
    ) -> anyhow::Result<CandidateSubmissionReceipt>;
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn candidate_cause_rejects_unbounded_or_empty_references() {
        assert!(CandidateCause::ExternalObservation {
            event_ref: String::new()
        }
        .validate()
        .is_err());
        assert!(CandidateCause::AgentRuntimeEvent {
            agent_id: ::contracts::AgentId::new(),
            operation_id: ::contracts::OperationId::new(),
            event_kind: String::new(),
        }
        .validate()
        .is_err());
        assert!(CandidateCause::ExternalObservation {
            event_ref: "event-1".into()
        }
        .validate()
        .is_ok());
    }

    #[test]
    fn runtime_event_cause_has_stable_source_reference() {
        let cause = CandidateCause::AgentRuntimeEvent {
            agent_id: ::contracts::AgentId::new(),
            operation_id: ::contracts::OperationId::new(),
            event_kind: "terminal".into(),
        };
        let reference = cause.required_source_ref();
        assert!(reference.starts_with("agent-runtime:"));
        assert!(reference.ends_with(":terminal"));
    }
}

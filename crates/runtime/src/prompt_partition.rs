...
use ::contracts::{
    canonicalize_tool_definitions, tool_schema_digest, Message, Role, ToolDefinition,
};
    ...
    let (tool_bytes, tool_digest) = canonicalize_tool_definitions(tools)
        .and_then(|canonical| {
            let bytes = serde_json::to_vec(&canonical).map_err(|error| {
                ::contracts::ToolDefinitionCanonicalizationError::Serialization(error.to_string())
            })?;

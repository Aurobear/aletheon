...
use ::contracts::{Clock, PermissionProfileId, PrincipalId, WorkspacePolicy, WorkspaceSelection};
...
impl MemoryGatewayService {
    pub fn intake_ledger(&self) -> Arc<MemoryIntakeLedger> {
        self.ledger.clone()
    }

    pub fn binding_registry(&self) -> Arc<WorkspaceMemoryBindingRegistry> {
        self.bindings.clone()
    }
    ...
    pub async fn bind_workspace(
        ...
        let binding = tokio::task::spawn_blocking(move || {
            ...
            {
                return Err(crate::WorkspaceMemoryBindingError::Invalid);
            }
    ...
    pub async fn unbind_workspace(
        ...
        let binding = tokio::task::spawn_blocking(move || {
            ...
            registry
                ...
                .ok_or(crate::WorkspaceMemoryBindingError::Corrupt)
        })
        .await
    ...
    async fn verify_active_binding(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            Ok::<_, crate::WorkspaceMemoryBindingError>(unchanged)
        })
        .await
    ...
    fn resolve_workspace_key(&self, working_dir: &Path) -> anyhow::Result<WorkspaceMemoryKey> {
        ...
        let identity = platform::workspace_identity::workspace_identity(workspace.cwd());
...
fn classification_sensitivity(
    value: ::contracts::data_governance::DataClassification,
) -> MemorySensitivityV1 {
    use ::contracts::data_governance::DataClassification;
    match value {
...
fn memory_item(item: crate::RecallItem) -> MemoryRecallItemV1 {
    MemoryRecallItemV1 {
...
fn evidence(value: crate::EvidenceLevel) -> MemoryEvidenceV1 {
    match value {
        crate::EvidenceLevel::AliasHit => MemoryEvidenceV1::AliasHit,
        crate::EvidenceLevel::ExactTitleMatch => MemoryEvidenceV1::ExactTitleMatch,
        crate::EvidenceLevel::HighVectorMatch => MemoryEvidenceV1::HighVectorMatch,
        crate::EvidenceLevel::KeywordExact => MemoryEvidenceV1::KeywordExact,
        crate::EvidenceLevel::WeakSemantic => MemoryEvidenceV1::WeakSemantic,
    }
}
